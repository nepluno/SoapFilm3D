...
// ---------------------------------------------------------
///
/// Apply an impulse along the given normal to the specified vertices, weighted
/// by the given barycentric coordinates
///
// ---------------------------------------------------------
...
void CollisionPipeline::apply_impulse(const Vec4d& alphas,
  ...
  double friction_impulse =
      min(m_friction_coefficient * delta_rv_normal, mag(pre_rv_tangential));
  ...
  //
  //        double i = impulse_magnitude / (s0*s0*inv_m0 + s1*s1*inv_m1 +
  //        s2*s2*inv_m2 + s3*s3*inv_m3);
  //
  //        if (i == std::numeric_limits<double>::infinity() || i ==
  ...
  //        {
  //            std::cout << "big impulse: " << i << " axis = " << j << "
  //            impulse magnitude = " << impulse_magnitude << " s = " << s0 << "
  //            " << s1 << " " << s2 << " " << s3 << " m = " << inv_m0 << " " <<
  //            inv_m1 << " " << inv_m2 << " " << inv_m3 << std::endl; std::cout
  //            << "post impulse velocities: " << v0[j] << " " << v1[j] << " "
  //            << v2[j] << " " << v3[j] << std::endl;
  //        }
  //
  //        //
  ...
  //        //
  //
  //        double friction_i = friction_impulse / (s0*s0*inv_m0 + s1*s1*inv_m1
  //        + s2*s2*inv_m2 + s3*s3*inv_m3);
  //
  ...
  for (int j = 0; j < 3; j++) {
    ...
    denom += normal[j] * normal[j] *
             (s0 * s0 * inv_m0 + s1 * s1 * inv_m1 + s2 * s2 * inv_m2 +
              s3 * s3 * inv_m3);
  }
  ...
  m_surface.set_newposition(
      e0, m_surface.get_position(e0) + dt * m_surface.m_velocities[e0]);
  m_surface.set_newposition(
      e1, m_surface.get_position(e1) + dt * m_surface.m_velocities[e1]);
  m_surface.set_newposition(
      e2, m_surface.get_position(e2) + dt * m_surface.m_velocities[e2]);
  m_surface.set_newposition(
      e3, m_surface.get_position(e3) + dt * m_surface.m_velocities[e3]);
...
void CollisionPipeline::apply_edge_edge_impulse(const Collision& collision,
                                                double impulse_magnitude,
                                                double dt) {
  ...
  apply_impulse(alphas, collision.m_vertex_indices, impulse_magnitude,
                collision.m_normal, dt);
}
...
void CollisionPipeline::apply_triangle_point_impulse(const Collision& collision,
                                                     double impulse_magnitude,
                                                     double dt) {
  assert(!collision.m_is_edge_edge);
...
void CollisionPipeline::add_triangle_candidates(
    size_t t, bool return_solid, bool return_dynamic,
  ...
  m_broadphase.get_potential_vertex_collisions(
      tmin, tmax, return_solid, return_dynamic, candidate_vertices);
...
void CollisionPipeline::add_edge_candidates(
    size_t e, bool return_solid, bool return_dynamic,
    CollisionCandidateSet& collision_candidates) {
  Vec3d emin, emax;
  ...
  m_broadphase.get_potential_edge_collisions(emin, emax, return_solid,
                                             return_dynamic, candidate_edges);
...
void CollisionPipeline::add_point_candidates(
    size_t v, bool return_solid, bool return_dynamic,
    CollisionCandidateSet& collision_candidates) {
  Vec3d vmin, vmax;
  ...
  m_broadphase.get_potential_triangle_collisions(
      vmin, vmax, return_solid, return_dynamic, candidate_triangles);
...
// ---------------------------------------------------------
///
/// Add collision candidates for a specified vertex and all elements incident on
/// the vertex
///
// ---------------------------------------------------------

void CollisionPipeline::add_point_update_candidates(
    size_t v, CollisionCandidateSet& collision_candidates) {
  // Avoid solid-vs-solid tests during sequential impulses phase

  if (m_surface.vertex_is_all_solid(v)) {
    return;
  }
  ...
  std::vector<size_t>& incident_triangles =
      m_surface.m_mesh.m_vertex_to_triangle_map[v];
  std::vector<size_t>& incident_edges =
      m_surface.m_mesh.m_vertex_to_edge_map[v];

  for (size_t i = 0; i < incident_triangles.size(); i++) {
    add_triangle_candidates(incident_triangles[i], true, true,
...
// ---------------------------------------------------------
///
/// Apply impulses to all proximal elements in the list of potentially proximal
/// elements
///
// ---------------------------------------------------------

void CollisionPipeline::process_proximity_candidates(
    double dt, CollisionCandidateSet& candidates) {
    ...
    if (candidate[2] == 1) {
      ...
      if (e0[0] != e1[0] && e0[0] != e1[1] && e0[1] != e1[0] &&
        ...
        check_edge_edge_proximity(
            m_surface.get_position(e0[0]), m_surface.get_position(e0[1]),
            m_surface.get_position(e1[0]), m_surface.get_position(e1[1]),
        ...
        if (distance < m_surface.m_proximity_epsilon && distance > 0) {
          ...
          double relvel =
              dot(normal, s0 * m_surface.m_velocities[e0[0]] +
          ...
          Collision proximity(true, Vec4st(e0[0], e0[1], e1[0], e1[1]), normal,
                              Vec4d(s0, 1.0 - s0, s2, 1.0 - s2), dt * relvel);
      ...
      if (tri[0] != v && tri[1] != v && tri[2] != v) {
        ...
        check_point_triangle_proximity(
            m_surface.get_position(v), m_surface.get_position(tri[0]),
            m_surface.get_position(tri[1]), m_surface.get_position(tri[2]),
        ...
        if (distance == 0) {
          ...
          bool col = LosTopos::point_triangle_collision(
              d, d, 0, a, a, 1, b, b, 2, c, c, 3, s1, s2, s3, normal, rel_disp);
          std::cout << "collision = " << col << std::endl;
          std::cout << "s = " << s1 << " " << s2 << " " << s3 << std::endl;
          std::cout << "normal = " << normal << " rel_disp = " << rel_disp
                    << std::endl;
          LosTopos::Vec3d x1 = a;
          LosTopos::Vec3d x2 = b;
          LosTopos::Vec3d x3 = c;
          ...
          std::cout << "normal = " << normal << " mag = " << mag(normal)
                    << std::endl;
        }
        ...
        if (distance < m_surface.m_proximity_epsilon && distance > 0) {
          ...
          Collision proximity(false, Vec4st(v, tri[0], tri[1], tri[2]), normal,
                              Vec4d(1.0, s1, s2, s3), dt * relvel);
...
bool CollisionPipeline::detect_segment_segment_collision(
    const Vec3st& candidate, Collision& collision) {
  ...
  if (m_surface.edge_is_all_solid(candidate[0]) &&
      m_surface.edge_is_all_solid(candidate[1])) {
    return false;
  }
  ...
  if (segment_segment_collision(
          m_surface.get_position(a), m_surface.get_newposition(a), a,
          m_surface.get_position(b), m_surface.get_newposition(b), b,
          m_surface.get_position(c), m_surface.get_newposition(c), c,
          m_surface.get_position(d), m_surface.get_newposition(d), d, s0, s2,
          normal, rel_disp)) {
    collision = Collision(true, Vec4st(a, b, c, d), normal,
                          Vec4d(s0, (1 - s0), s2, (1 - s2)), rel_disp);
    return true;
  }
...
bool CollisionPipeline::detect_point_triangle_collision(const Vec3st& candidate,
                                                        Collision& collision) {
  ...
  if (point_triangle_collision(
          m_surface.get_position(v), m_surface.get_newposition(v), v,
          m_surface.get_position(sorted_tri[0]),
          m_surface.get_newposition(sorted_tri[0]), sorted_tri[0],
          m_surface.get_position(sorted_tri[1]),
          m_surface.get_newposition(sorted_tri[1]), sorted_tri[1],
          m_surface.get_position(sorted_tri[2]),
          m_surface.get_newposition(sorted_tri[2]), sorted_tri[2], s1, s2, s3,
          normal, rel_disp))

  {
    collision =
        Collision(false, Vec4st(v, sorted_tri[0], sorted_tri[1], sorted_tri[2]),
                  normal, Vec4d(1, s1, s2, s3), rel_disp);
    return true;
  }
...
void CollisionPipeline::process_collision_candidates(
    double dt, CollisionCandidateSet& candidates, bool add_to_new_candidates,
    CollisionCandidateSet& new_candidates, ProcessCollisionStatus& status) {
  ...
  while (false == candidates.empty() && i++ < max_iteration) {
    ...
    if (candidate[2] == 1) {
      ...
      if (detect_segment_segment_collision(candidate, collision)) {
        ...
        double impulse =
            IMPULSE_MULTIPLIER * (desired_relative_velocity - relvel);
        ...
        if (!status.overflow && add_to_new_candidates) {
          add_point_update_candidates(collision.m_vertex_indices[0],
                                      new_candidates);
          add_point_update_candidates(collision.m_vertex_indices[1],
                                      new_candidates);
          add_point_update_candidates(collision.m_vertex_indices[2],
                                      new_candidates);
          add_point_update_candidates(collision.m_vertex_indices[3],
                                      new_candidates);
        }
      }
    } else {
  ...
  if (m_surface.m_verbose && max_iteration > 0 && i >= max_iteration) {
    std::cout << "CollisionPipeline::process_collision_candidates: "
...
void CollisionPipeline::test_collision_candidates(
    CollisionCandidateSet& candidates, std::vector<Collision>& collisions,
...
// ---------------------------------------------------------
///
/// Check if any collision exists in the set of candidates.  Stop when the first
/// collision is found.
///
// ---------------------------------------------------------

bool CollisionPipeline::any_collision(CollisionCandidateSet& candidates,
                                      Collision& collision) {
...
void CollisionPipeline::dynamic_point_vs_solid_triangle_collisions(
  ...
  for (size_t i = 0; i < m_surface.get_num_vertices(); ++i) {
    ...
    process_collision_candidates(dt, point_collision_candidates,
                                 collect_candidates,
                                 update_collision_candidates, status);
  }
}
...
void CollisionPipeline::dynamic_triangle_vs_all_point_collisions(
    double dt, bool collect_candidates,
    CollisionCandidateSet& update_collision_candidates,
    ProcessCollisionStatus& status) {
  for (size_t i = 0; i < m_surface.m_mesh.num_triangles(); ++i) {
    ...
    process_collision_candidates(dt, triangle_collision_candidates,
                                 collect_candidates,
                                 update_collision_candidates, status);
  }
}
...
void CollisionPipeline::dynamic_edge_vs_all_edge_collisions(
    double dt, bool collect_candidates,
    CollisionCandidateSet& update_collision_candidates,
    ProcessCollisionStatus& status) {
  for (size_t i = 0; i < m_surface.m_mesh.m_edges.size(); ++i) {
    ...
    process_collision_candidates(dt, edge_collision_candidates,
                                 collect_candidates,
                                 update_collision_candidates, status);
  }
}
...
// ---------------------------------------------------------
///
/// Detect and fix all collisions, sweeping over all mesh primitives a number of
/// times.
///
// ---------------------------------------------------------
...
bool CollisionPipeline::handle_collisions(double dt) {
  ...
  for (int pass = 0; pass < MAX_PASS; ++pass) {
    // if last time through the loop, fill out the update_collision_candidates
    // array when the loop exits, we will wind down this array
    ...
    dynamic_point_vs_solid_triangle_collisions(
        dt, collect_candidates, update_collision_candidates, status);
    ...
    dynamic_triangle_vs_all_point_collisions(
        dt, collect_candidates, update_collision_candidates, status);
    ...
    dynamic_edge_vs_all_edge_collisions(dt, collect_candidates,
                                        update_collision_candidates, status);
    ...
    if (!collision_found) {
      if (verbose) {
        std::cout << "no collision found this pass, returning early"
                  << std::endl;
      }
      return true;
    }
  ...
  //    std::sort( update_collision_candidates.begin(),
  //    update_collision_candidates.end(), CollisionCandidateSetLT );
  //    CollisionCandidateSet::iterator new_end =
  //    std::unique(update_collision_candidates.begin(),
  //    update_collision_candidates.end());
  //    update_collision_candidates.erase(new_end,
  //    update_collision_candidates.end());

  std::set<Vec3st, CCollisionCandidateSetLT> candidate_set(
      update_collision_candidates.begin(), update_collision_candidates.end());
  update_collision_candidates.assign(candidate_set.begin(),
                                     candidate_set.end());

  // now wind down the update_collision_candidates list
  ProcessCollisionStatus status;
  status.overflow = false;
  process_collision_candidates(dt, update_collision_candidates, true,
                               update_collision_candidates, status);
  ...
  if (m_surface.m_verbose && !ok) {
    ...
    g_stats.add_to_int(
        "CollisionPipeline::handle_collisions:didnt_resolve_all_collisions", 1);
  }
  ...
  if (status.overflow) {
    ...
    if (m_surface.m_verbose) {
      ...
      g_stats.add_to_int(
          "CollisionPipeline::handle_collisions:overflowed_candidate_list", 1);
    }
  }
...
bool CollisionPipeline::detect_collisions(std::vector<Collision>& collisions) {
  ...
  ProcessCollisionStatus status;
  test_collision_candidates(collision_candidates, collisions, status);
...
// ---------------------------------------------------------
///
/// Detect continuous collisions among elements in the given ImpactZones, and
/// adjacent to the given ImpactZones.
///
// ---------------------------------------------------------

bool CollisionPipeline::detect_new_collisions(
    const std::vector<ImpactZone> impact_zones,
  ...
  for (size_t i = 0; i < impact_zones.size(); ++i) {
    for (size_t j = 0; j < impact_zones[i].m_collisions.size(); ++j) {
      add_unique(zone_vertices,
                 impact_zones[i].m_collisions[j].m_vertex_indices[0]);
      add_unique(zone_vertices,
                 impact_zones[i].m_collisions[j].m_vertex_indices[1]);
      add_unique(zone_vertices,
                 impact_zones[i].m_collisions[j].m_vertex_indices[2]);
      add_unique(zone_vertices,
                 impact_zones[i].m_collisions[j].m_vertex_indices[3]);
    }
  }
  ...
  for (size_t i = 0; i < zone_vertices.size(); ++i) {
    for (size_t j = 0;
         j < mesh.m_vertex_to_triangle_map[zone_vertices[i]].size(); ++j) {
      add_unique(zone_triangles,
                 mesh.m_vertex_to_triangle_map[zone_vertices[i]][j]);
    }
  }
  ...
  for (size_t i = 0; i < zone_vertices.size(); ++i) {
    for (size_t j = 0; j < mesh.m_vertex_to_edge_map[zone_vertices[i]].size();
         ++j) {
...

{
  ...
  if (segment_segment_collision(
          m_surface.get_position(e0), m_surface.get_newposition(e0), e0,
          m_surface.get_position(e1), m_surface.get_newposition(e1), e1,
          m_surface.get_position(t0), m_surface.get_newposition(t0), t0,
          m_surface.get_position(t1), m_surface.get_newposition(t1), t1, s0, s2,
          normal, rel_disp))

  {
    Collision new_collision(true, Vec4st(e0, e1, t0, t1), normal,
                            Vec4d(s0, (1 - s0), s2, (1 - s2)), rel_disp);
    ...
    bool check_hit =
        detect_segment_segment_collision(check_candidate, check_collision);
    assert(check_hit);
  }
  ...
  if (segment_segment_collision(
          ...
          m_surface.get_position(t2), m_surface.get_newposition(t2), t2, s0, s2,
          normal, rel_disp))

  {
    Collision new_collision(true, Vec4st(e0, e1, t1, t2), normal,
                            Vec4d(s0, (1 - s0), s2, (1 - s2)), rel_disp);
    collisions.push_back(new_collision);
  ...
  {
    Collision new_collision(true, Vec4st(e0, e1, t2, t0), normal,
                            Vec4d(s0, (1 - s0), s2, (1 - s2)), rel_disp);
    collisions.push_back(new_collision);
  ...
  if (point_triangle_collision(
          ...
          m_surface.get_position(t0), m_surface.get_newposition(t0), t0,
          m_surface.get_position(t1), m_surface.get_newposition(t1), t1,
          m_surface.get_position(t2), m_surface.get_newposition(t2), t2, s1, s2,
          s3, normal, rel_disp)) {
    Collision new_collision(false, Vec4st(e0, t0, t1, t2), normal,
                            Vec4d(1.0, s1, s2, s3), rel_disp);
    collisions.push_back(new_collision);
  }
  ...
  if (point_triangle_collision(
          ...
          m_surface.get_position(t2), m_surface.get_newposition(t2), t2, s1, s2,
          s3, normal, rel_disp)) {
    Collision new_collision(false, Vec4st(e1, t0, t1, t2), normal,
                            Vec4d(1.0, s1, s2, s3), rel_disp);
    collisions.push_back(new_collision);
  }
  ...
  for (size_t i = 0; i < collisions.size(); ++i) {
    ...
    std::cout << "\n ======== Collision: is_edge_edge: " << coll.m_is_edge_edge
              << ", indices: " << coll.m_vertex_indices << std::endl;

    if (coll.m_is_edge_edge) {
      size_t edge0 = m_surface.m_mesh.get_edge_index(coll.m_vertex_indices[0],
                                                     coll.m_vertex_indices[1]);
      std::cout << "edge0: " << edge0 << std::endl;

      size_t edge1 = m_surface.m_mesh.get_edge_index(coll.m_vertex_indices[2],
                                                     coll.m_vertex_indices[3]);
      ...
      for (size_t j = 0; j < collision_candidates0.size(); ++j) {
        if (collision_candidates0[j][0] == edge1 ||
            collision_candidates0[j][1] == edge1) {
          edge1_found = true;
        }
      }

      if (!edge1_found) {
        std::cout << "broadphase didn't find edge " << edge1 << std::endl;
      }
      ...
      for (size_t j = 0; j < collision_candidates1.size(); ++j) {
        if (collision_candidates1[j][0] == edge0 ||
            collision_candidates1[j][1] == edge0) {
          edge0_found = true;
        }
      }

      if (!edge0_found) {
        std::cout << "broadphase didn't find edge " << edge0 << std::endl;
      }

    } else {
      size_t vert = coll.m_vertex_indices[0];
      size_t collision_tri = m_surface.m_mesh.get_triangle_index(
          coll.m_vertex_indices[1], coll.m_vertex_indices[2],
          coll.m_vertex_indices[3]);
      ...
      for (size_t j = 0; j < v_collision_candidates.size(); ++j) {
        if (v_collision_candidates[j][0] == collision_tri ||
            v_collision_candidates[j][1] == collision_tri) {
          tri_found = true;
        }
      }

      if (!tri_found) {
        std::cout << "broadphase didn't find tri " << collision_tri
                  << std::endl;
      }

      CollisionCandidateSet t_collision_candidates;
      add_triangle_candidates(collision_tri, true, true,
                              t_collision_candidates);
      bool vert_found = false;
      for (size_t j = 0; j < t_collision_candidates.size(); ++j) {
        if (t_collision_candidates[j][0] == vert ||
            t_collision_candidates[j][1] == vert) {
          vert_found = true;
        }
      }

      if (!vert_found) {
        std::cout << "broadphase didn't find vertex " << vert << std::endl;
      }
    }
  }
}
...
// ---------------------------------------------------------
///
/// Determine if the vertices specified by the Collision object are still in
/// collision
///
// ---------------------------------------------------------

bool CollisionPipeline::check_if_collision_persists(
    const Collision& collision) {
  const Vec4st& vs = collision.m_vertex_indices;

  if (collision.m_is_edge_edge) {
    return segment_segment_collision(
        m_surface.get_position(vs[0]), m_surface.get_newposition(vs[0]), vs[0],
