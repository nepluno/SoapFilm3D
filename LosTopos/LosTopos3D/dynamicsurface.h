...
//
//  The most important function is integrate(), which advances the mesh vertices
//  from m_positions to m_newpositions, while performing collision detection and
//  resolution.
//
// ---------------------------------------------------------
...
// Broad-phase collision detector.  Avoids performing collision detection
// between far-away primitives.
class BroadPhase;
...
struct Intersection {
  /// Constructor
  ///
  Intersection(size_t edge_index, size_t triangle_index)
      : m_edge_index(edge_index), m_triangle_index(triangle_index) {}
...
// --------------------------------------------------------
///
/// A surface mesh.  Essentially consists of a NonDestructiveTriMesh object
/// coupled with a set of vertex locations in 3D space.
///
// --------------------------------------------------------

class DynamicSurface {
 public:
  /// Create a DynamicSurface object from the specified vertices, triangles, and
  /// vertex masses.
  ///

  DynamicSurface(const std::vector<Vec3d>& vs, const std::vector<Vec3st>& ts,
                 ...
                 bool in_collision_safety = true, bool in_verbose = false);
 ...
 public:
  /// Advance from current state to a collision-free state as close as possible
  /// to predicted state.
  ///
  ...
  /// Break up the triangle mesh into connected components, determine surface
  /// IDs for all vertices.
  ///
  void partition_surfaces(std::vector<size_t>& surface_ids,
  ...
  /// Get an estimate for the surface normal at the specified vertex. Computed
  /// using an unweighted average of the normals of incident triangles.
  ///
  inline Vec3d get_vertex_normal(size_t vertex) const;

  /// Get an estimate for the surface normal at the specified vertex. Computed
  /// using angleweighted pseudonormal
  ///
  ...
  /// using angleweighted pseudonormal
  ///
  inline Vec3d get_vertex_normal_angleweighted_by_label(size_t vertex,
                                                        int label) const;

  /// Get an estimate for the surface mean curvature normal at the specified
  /// vertex, using Meyer's method.
  ///
  inline Vec3d get_vertex_curvature_normal(size_t vertex, int label = -1) const;

  /// Compute all vertex normals, using an unweighted average of incident
  /// triangle normals.
  ///
  void get_all_vertex_normals(std::vector<Vec3d>& normals) const;

  /// Get an estimate for the surface normal at the specified vertex. Computed
  /// using a weighted average as described in [Max 1999].
  ///
  ...
  /// Get the average edge length, disregarding edges with vertices marked as
  /// solid
  ///
  ...
  /// Compute the total surface area using predicted vertex locations
  /// (m_newpositions)
  ///
  ...
  /// Compute the volume enclosed by the surface, using predicted vertex
  /// locations (m_newpositions)
  ///
  inline double get_predicted_volume() const;

  /// Compute the distance from the given point to the surface.  Also return the
  /// index of the closest triangle.
  ///
  double distance_to_surface(const Vec3d& p,
                             size_t& out_closest_triangle) const;
  ...
  unsigned int compute_rank_from_triangles(
      const std::vector<size_t>& tris) const;

  /// Get edge dihedral angle
  double get_largest_dihedral(size_t edge) const;
  double get_largest_dihedral(size_t edge,
                              const std::vector<Vec3d>& cached_normals) const;

  /// Determine which region the point is inside by ray-casting and looking at
  /// the normal of the first intersection, and comparing that with the
  /// triangle's labeling
  ...
  /// Delete and rebuild the broad phase object, using AABBs defined from
  /// m_positions.
  ///
  void rebuild_static_broad_phase();

  /// Delete and rebuild the broad phase object, using AABBs defined from
  /// m_positions and m_newpositions.
  ///
  void rebuild_continuous_broad_phase();

  /// Assume that the specified vertex has moved.  Update the broadphase entries
  /// of the vertex, its incident edges and incident triangles.
  ///
  void update_static_broad_phase(size_t vertex_index);

  /// Assume that the specified vertex has moved.  Update the broadphase entries
  /// of the vertex, its incident edges and incident triangles, using current
  ...
  /// Get the padded AABB of the specified vertex using current and predicted
  /// positions.
  ///
  void vertex_continuous_bounds(size_t v, Vec3d& xmin, Vec3d& xmax) const;

  /// Get the padded AABB of the specified edge using current and predicted
  /// positions.
  ///
  void edge_continuous_bounds(size_t e, Vec3d& xmin, Vec3d& xmax) const;

  /// Get the padded AABB of the specified triangle using current and predicted
  /// positions.
  ///
  ...
  /// Get the set of triangles intersected by the given segment. Also returns
  /// the barycentric coordinate along the segment of each intersection.
  ///
  ...
  ///
  size_t get_number_of_triangle_intersections(
      const Vec3d& segment_point_a, const Vec3d& segment_point_b) const;

  /// Using exact intersection testing, count the number of triangle intersected
  /// by the given segment.
  ///
  size_t get_number_of_triangle_intersections_exact(
      const Vec3d& segment_point_a, const Vec3d& segment_point_b) const;

  /// Test the given triangle against all other triangles in the mesh for
  /// intersection.
  ///
  ...
  /// Fire an assert if the mesh contains a self-intersection. Uses m_positions
  /// as the vertex locations.
  ///
  void assert_mesh_is_intersection_free(bool degeneracy_counts_as_intersection);

  /// Using m_newpositions as the vertex locations, fire an assert if the mesh
  /// contains a self-intersection.
  ///
  void assert_predicted_mesh_is_intersection_free(
      bool degeneracy_counts_as_intersection);

  /// Returns the number of vertices in the mesh, including any vertices marked
  /// as deleted
  ///
  ...
  /// Set the current positions of all vertices in the mesh, from a C-array of
  /// doubles
  ///
  ...
  /// Ensure that no mesh elements intersect, during mesh moving and mesh
  /// maintenance
  ///
...
inline double DynamicSurface::get_triangle_area(size_t v0, size_t v1,
                                                size_t v2) const {
...
inline Vec3d triangle_normal(const Vec3d& v0, const Vec3d& v1,
                             const Vec3d& v2) {
  Vec3d u = v1 - v0;
  Vec3d v = v2 - v0;
...
// --------------------------------------------------------
///
/// Compute the normal of a triangle specified by a triangle index, and the
/// relevant region
///
// --------------------------------------------------------

inline Vec3d DynamicSurface::get_triangle_normal_by_region(size_t tri,
                                                           int region) const {
...
inline Vec3d DynamicSurface::get_triangle_normal(size_t v0, size_t v1,
                                                 size_t v2) const {
...
inline Vec3d DynamicSurface::get_triangle_barycenter(
    size_t triangle_index) const {
  const Vec3st& tri = m_mesh.get_triangle(triangle_index);
  return 1.0 / 3.0 *
         (get_position(tri[0]) + get_position(tri[1]) + get_position(tri[2]));
}
...
inline double DynamicSurface::get_min_triangle_area(
    size_t& triangle_index) const {
...
// --------------------------------------------------------
///
/// Compute surface normal at the specified vertex (unweighted average of
...
// --------------------------------------------------------
///
/// Compute surface normal at the specified vertex (angle-weighted
/// pseudo-normal) per region.
///
// --------------------------------------------------------

inline Vec3d DynamicSurface::get_vertex_normal_angleweighted_by_label(
    size_t vertex_index, int label) const {
  ...
  for (unsigned int i = 0; i < inc_tris.size(); ++i) {
    ...
    if (tri_label[0] != label && tri_label[1] != label)
      continue;  // no matching label, skip it.

    if (curr_tri[0] == curr_tri[1]) {
      continue;
    }
    ...
    if (tri_label[0] == label)
      normal *=
          -1;  // flip the normal in the case of label being on the reverse.
...
inline Vec3d DynamicSurface::get_vertex_normal_angleweighted(
...
// --------------------------------------------------------
///
/// Compute surface normal at the specified vertex (weighted according to [Max
/// 1999]).
///
// --------------------------------------------------------
...
inline Vec3d DynamicSurface::get_vertex_curvature_normal(size_t vertex_index,
                                                         int region) const {
  Vec3d mean_curvature_normal(0, 0, 0);
  for (unsigned int i = 0;
       i < m_mesh.m_vertex_to_triangle_map[vertex_index].size(); ++i) {
  ...
  for (unsigned int i = 0;
    ...
    vertex_area +=
        get_triangle_area(m_mesh.m_vertex_to_triangle_map[vertex_index][i]) /
        3.0;

    // Don't bother using Meyer's wacky area calculation
    // vertex_area += mixed_area( vertex_index,
    // surface_tracker->m_mesh.m_vertex_to_triangle_map[vertex_index][i],
    // *surface_tracker );
  }
...
inline double DynamicSurface::get_edge_length(size_t edge_index) const {
  return mag(get_position(m_mesh.m_edges[edge_index][1]) -
             get_position(m_mesh.m_edges[edge_index][0]));
}
...
inline double DynamicSurface::get_volume() const {
  ...
  for (size_t t = 0; t < tris.size(); ++t) {
    ...
    volume += inv_six * triple(get_position(tri[0]), get_position(tri[1]),
                               get_position(tri[2]));
  }
  return volume;
}
...
inline double DynamicSurface::get_predicted_volume() const {
  ...
  for (size_t t = 0; t < tris.size(); ++t) {
    ...
    const Vec3st& tri = tris[t];
    volume += inv_six * triple(get_newposition(tri[0]), get_newposition(tri[1]),
                               get_newposition(tri[2]));
  }
  return volume;
}
...
// --------------------------------------------------------
///
/// Return true if the specified vertex is solid (should be treated as having
/// infinite mass).
///
// --------------------------------------------------------
...
inline Vec3c DynamicSurface::vertex_is_solid_3(size_t v) const {
  assert(v < m_masses.size());
  return (Vec3c(vertex_is_solid(v, 0), vertex_is_solid(v, 1),
                vertex_is_solid(v, 2)));
}

inline bool DynamicSurface::vertex_is_all_solid(size_t v) const {
  return (vertex_is_solid(v, 0) && vertex_is_solid(v, 1) &&
          vertex_is_solid(v, 2));
}
...
// --------------------------------------------------------
///
/// Return true if either end vertex of the specified edge is solid (should be
...
// --------------------------------------------------------
///
/// Return true if any corner vertex of the specified triangle is solid (should
/// be treated as having infinite mass).
///
// --------------------------------------------------------

inline bool DynamicSurface::triangle_is_any_solid(size_t t) const {
  const Vec3st& tri = m_mesh.get_triangle(t);
  return (vertex_is_any_solid(tri[0]) || vertex_is_any_solid(tri[1]) ||
          vertex_is_any_solid(tri[2]));
}

// --------------------------------------------------------
///
/// Return true if both end vertices of the specified edge is solid (should be
/// treated as having infinite mass).
///
// --------------------------------------------------------
...
// --------------------------------------------------------
///
/// Return true if all corner vertices of the specified triangle is solid
/// (should be treated as having infinite mass).
///
// --------------------------------------------------------

inline bool DynamicSurface::triangle_is_all_solid(size_t t) const {
  const Vec3st& tri = m_mesh.get_triangle(t);
  return (vertex_is_all_solid(tri[0]) && vertex_is_all_solid(tri[1]) &&
          vertex_is_all_solid(tri[2]));
}

// ---------------------------------------------------------
///
/// Returns the number of vertices in the mesh, including any vertices marked as
/// deleted
///
// ---------------------------------------------------------
...
/// Vertex velocity, for remeshing only, not for collisions
///
inline void DynamicSurface::set_all_remesh_velocities(
...
inline bool check_edge_triangle_intersection_by_index(
    size_t edge_a, size_t edge_b, size_t triangle_a, size_t triangle_b,
    size_t triangle_c, const std::vector<Vec3d>& m_positions, bool verbose) {
  ...
  return segment_triangle_intersection(
      m_positions[edge_a], edge_a, m_positions[edge_b], edge_b,
      m_positions[triangle_a], triangle_a, m_positions[triangle_b], triangle_b,
      m_positions[triangle_c], triangle_c, DEGEN_COUNTS_AS_INTERSECTION,
      verbose);
}

// --------------------------------------------------------
///
/// Returns true if the an edge from one of the triangles intersects the other
/// triangle. NOTE: Using this routine will produce duplicate checks.  Better to
/// use check_edge_triangle_intersection where possible.
///
// --------------------------------------------------------

inline bool check_triangle_triangle_intersection(
    Vec3st triangle_a, Vec3st triangle_b, const std::vector<Vec3d>& positions) {
  if (triangle_a[0] == triangle_a[1] || triangle_b[0] == triangle_b[1]) {
    return false;
  }

  if (check_edge_triangle_intersection_by_index(
          triangle_a[0], triangle_a[1], triangle_b[0], triangle_b[1],
          triangle_b[2], positions, false)) {
    return true;
  }

  if (check_edge_triangle_intersection_by_index(
          triangle_a[1], triangle_a[2], triangle_b[0], triangle_b[1],
          triangle_b[2], positions, false)) {
    return true;
  }

  if (check_edge_triangle_intersection_by_index(
          triangle_a[2], triangle_a[0], triangle_b[0], triangle_b[1],
          triangle_b[2], positions, false)) {
    return true;
  }

  if (check_edge_triangle_intersection_by_index(
          triangle_b[0], triangle_b[1], triangle_a[0], triangle_a[1],
          triangle_a[2], positions, false)) {
    return true;
  }

  if (check_edge_triangle_intersection_by_index(
          triangle_b[1], triangle_b[2], triangle_a[0], triangle_a[1],
          triangle_a[2], positions, false)) {
    return true;
  }

  if (check_edge_triangle_intersection_by_index(
          triangle_b[2], triangle_b[0], triangle_a[0], triangle_a[1],
          triangle_a[2], positions, false)) {
    return true;
  }
