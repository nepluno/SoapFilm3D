...
void NonDestructiveTriMesh::clear() {
  ...
  for (size_t i = 0; i < m_vds.size(); i++) m_vds[i]->resize(0);
  for (size_t i = 0; i < m_eds.size(); i++) m_eds[i]->resize(0);
  for (size_t i = 0; i < m_fds.size(); i++) m_fds[i]->resize(0);
}
...
///
void NonDestructiveTriMesh::nondestructive_renumber_triangle(
    size_t tri, const Vec3st& verts) {
  ...
  // For any verts of the tri that have changed, update their neighbor
  // relationships

  // Check all old vertices of the triangle
  for (unsigned int i = 0; i < 3; i++) {
    // If the vertex isn't in the new triangle, remove the triangle from the
    // vertex's neighbours.
    size_t cur_old_vert = old_verts[i];

    // if the old vertex *is* in the new tri, skip it
    if (cur_old_vert == verts[0] || cur_old_vert == verts[1] ||
        cur_old_vert == verts[2])
      continue;
  ...
  // Update the triangle->edge map, m_triangle_to_edge_map
  // We're going to be dumb about edges for now. Delete them all and then re-add
  // them.
  Vec3st& te = m_triangle_to_edge_map[tri];
  ...
  for (unsigned int i = 0; i < 3; i++) {
    ...
    // if the new vertex *wasn't* in the old tri, add the triangle to the new
    // vertex
...
void NonDestructiveTriMesh::nondestructive_remove_vertex(size_t vtx) {
  ...
  for (size_t i = 0; i < m_vertex_to_edge_map[vtx].size(); ++i) {
    assert(m_edges[m_vertex_to_edge_map[vtx][i]][0] ==
           m_edges[m_vertex_to_edge_map[vtx][i]][1]);
  }
...
size_t NonDestructiveTriMesh::nondestructive_add_edge(size_t vtx0,
                                                      size_t vtx1) {
...
void NonDestructiveTriMesh::nondestructive_remove_edge(size_t edge_index) {
  // vertex 0
  {
    std::vector<size_t>& vertex_to_edge_map =
        m_vertex_to_edge_map[m_edges[edge_index][0]];
  ...
  {
    std::vector<size_t>& vertex_to_edge_map =
        m_vertex_to_edge_map[m_edges[edge_index][1]];
    for (int i = 0; i < (int)vertex_to_edge_map.size(); ++i) {
      if (vertex_to_edge_map[i] == edge_index) {
        vertex_to_edge_map.erase(vertex_to_edge_map.begin() + i);
        --i;
...
// --------------------------------------------------------
///
/// Find edge specified by two vertices.  Return edges.size if the edge is not
/// found.
///
// --------------------------------------------------------
  ...
  for (size_t e0 = 0; e0 < edges0.size(); e0++) {
    ...
    if (!edge_is_deleted(edge0)) {
      std::vector<size_t>::const_iterator it =
          std::find(edges1.begin(), edges1.end(), edge0);
      if (it != edges1.end()) {
        return edge0;
      }
    }
...
// --------------------------------------------------------
///
/// Find triangle specified by three vertices.  Return triangles.size if the
/// triangle is not found.
///
// --------------------------------------------------------

size_t NonDestructiveTriMesh::get_triangle_index(size_t vtx0, size_t vtx1,
                                                 size_t vtx2) const {
...
void NonDestructiveTriMesh::clear_deleted_triangles(
    std::vector<Vec2st>* defragged_triangle_map) {
...
void NonDestructiveTriMesh::test_connectivity() const {
  ...
  for (size_t i = 0; i < m_vertex_to_triangle_map.size(); ++i) {
    ...
    for (size_t j = 0; j < m_vertex_to_triangle_map[i].size(); ++j) {
      ...
      assert(m_tris[inc_triangle][0] == i || m_tris[inc_triangle][1] == i ||
             m_tris[inc_triangle][2] == i);
    }
  }
  ...
  for (size_t i = 0; i < m_edge_to_triangle_map.size(); ++i) {
    ...
    for (size_t j = 0; j < m_edge_to_triangle_map[i].size(); ++j) {
      ...
      if (m_tris[triangle_index][0] == m_edges[i][0] ||
          m_tris[triangle_index][0] == m_edges[i][1]) {
        ++num_common_verts;
      }
      if (m_tris[triangle_index][1] == m_edges[i][0] ||
          m_tris[triangle_index][1] == m_edges[i][1]) {
        ++num_common_verts;
      }
      if (m_tris[triangle_index][2] == m_edges[i][0] ||
          m_tris[triangle_index][2] == m_edges[i][1]) {
        ++num_common_verts;
      }
  ...
  for (size_t i = 0; i < m_triangle_to_edge_map.size(); ++i) {
    ...
    assert(edge0[0] == m_tris[i][0] || edge0[0] == m_tris[i][1] ||
           edge0[0] == m_tris[i][2]);
    assert(edge0[1] == m_tris[i][0] || edge0[1] == m_tris[i][1] ||
           edge0[1] == m_tris[i][2]);
    assert(edge1[0] == m_tris[i][0] || edge1[0] == m_tris[i][1] ||
           edge1[0] == m_tris[i][2]);
    assert(edge1[1] == m_tris[i][0] || edge1[1] == m_tris[i][1] ||
           edge1[1] == m_tris[i][2]);
    assert(edge2[0] == m_tris[i][0] || edge2[0] == m_tris[i][1] ||
           edge2[0] == m_tris[i][2]);
    assert(edge2[1] == m_tris[i][0] || edge2[1] == m_tris[i][1] ||
           edge2[1] == m_tris[i][2]);
  }
}
