...
//
//  Functions supporting the "edge collapse" operation: removing short edges
//  from the mesh.
//
// ---------------------------------------------------------
...
// --------------------------------------------------------
///
/// Edge collapser constructor.  Takes a SurfTrack object and curvature-adaptive
/// parameters.
///
// --------------------------------------------------------

EdgeCollapser::EdgeCollapser(SurfTrack& surf, bool use_curvature,
...
void EdgeCollapser::get_moving_triangles(
    size_t source_vertex, size_t destination_vertex,
    std::vector<size_t>& moving_triangles) {
  moving_triangles.clear();

  for (size_t i = 0;
       i < m_surf.m_mesh.m_vertex_to_triangle_map[source_vertex].size(); ++i) {
    moving_triangles.push_back(
        m_surf.m_mesh.m_vertex_to_triangle_map[source_vertex][i]);
  }
  for (size_t i = 0;
       i < m_surf.m_mesh.m_vertex_to_triangle_map[destination_vertex].size();
       ++i) {
    moving_triangles.push_back(
        m_surf.m_mesh.m_vertex_to_triangle_map[destination_vertex][i]);
  }
}
...
void EdgeCollapser::get_moving_edges(size_t source_vertex,
                                     size_t destination_vertex, size_t,
                                     std::vector<size_t>& moving_edges) {
  moving_edges = m_surf.m_mesh.m_vertex_to_edge_map[source_vertex];
  moving_edges.insert(
      moving_edges.end(),
      m_surf.m_mesh.m_vertex_to_edge_map[destination_vertex].begin(),
...
bool EdgeCollapser::collapse_edge_pseudo_motion_introduces_collision(
    size_t source_vertex, size_t destination_vertex, size_t edge_index,
    const Vec3d&) {
  ...
  // Check for collisions, holding everything static except for the source and
  ...
  for (size_t i = 0; i < moving_triangles.size(); ++i) {
    m_surf.m_collision_pipeline->add_triangle_candidates(
        moving_triangles[i], true, true, collision_candidates);
  }

  // point-triangle candidates
  m_surf.m_collision_pipeline->add_point_candidates(source_vertex, true, true,
                                                    collision_candidates);
  m_surf.m_collision_pipeline->add_point_candidates(destination_vertex, true,
                                                    true, collision_candidates);

  // edge-edge candidates
  for (size_t i = 0; i < moving_edges.size(); ++i) {
    m_surf.m_collision_pipeline->add_edge_candidates(
        moving_edges[i], true, true, collision_candidates);
  }

  // Prune collision candidates containing both the source and destination
  // vertex (they will trivially be collisions )
  ...
  for (size_t i = 0; i < collision_candidates.size(); ++i) {
    ...
    if (candidate[2] == 1) {
      ...
      if (e0[0] == source_vertex || e0[1] == source_vertex ||
          e1[0] == source_vertex || e1[1] == source_vertex) {
        if (e0[0] == destination_vertex || e0[1] == destination_vertex ||
            e1[0] == destination_vertex || e1[1] == destination_vertex) {
          should_delete = true;
        }
      }
      ...
      if (v == source_vertex || tri[0] == source_vertex ||
          tri[1] == source_vertex || tri[2] == source_vertex) {
        if (v == destination_vertex || tri[0] == destination_vertex ||
            tri[1] == destination_vertex || tri[2] == destination_vertex) {
          should_delete = true;
        }
      }
    }
  ...
  Collision collision;
  if (m_surf.m_collision_pipeline->any_collision(collision_candidates,
                                                 collision)) {
    return true;
  }

  // Also check proximity: if any proximity check returns zero distance, this
  // collapse cannot be allowed either. Because the CCD above is geometrically
  // exact, it sometimes returns different result than the proximity check below
  // (proximity
  //  distance = 0, but CCD says no collision). If distance is 0, the subsequent
  //  proximity handling will produce NaNs, so it is problematic too.
  ...
  for (size_t i = 0; i < collision_candidates.size(); i++) {
    ...
    if (candidate[2] == 1) {
      ...
      if (e0[0] != e1[0] && e0[0] != e1[1] && e0[1] != e1[0] &&
        ...
        Vec3d normal;
        check_edge_edge_proximity(
            m_surf.get_newposition(e0[0]), m_surf.get_newposition(e0[1]),
            m_surf.get_newposition(e1[0]), m_surf.get_newposition(e1[1]),
      ...
      if (tri[0] != v && tri[1] != v && tri[2] != v) {
        ...
        check_point_triangle_proximity(
            m_surf.get_newposition(v), m_surf.get_newposition(tri[0]),
            m_surf.get_newposition(tri[1]), m_surf.get_newposition(tri[2]),
...
// --------------------------------------------------------
///
/// Determine if the edge collapse operation would invert the normal of any
/// incident triangles.
///
// --------------------------------------------------------

bool EdgeCollapser::collapse_edge_introduces_normal_inversion(
    size_t source_vertex, size_t destination_vertex, size_t edge_index,
    const Vec3d& vertex_new_position) {
  // Get the set of triangles which are going to be deleted
  std::vector<size_t>& triangles_incident_to_edge =
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index];
  ...
  for (size_t i = 0; i < moving_triangles.size(); i++) {
    ...
    double area = triangle_area(m_surf.get_position(current_triangle[0]),
  ...
  for (size_t i = 0; i < moving_triangles.size(); ++i) {
    // Disregard triangles which will end up being deleted - those triangles
    // incident to the collapsing edge.
    ...
    const Vec3st& current_triangle =
        m_surf.m_mesh.get_triangle(moving_triangles[i]);
    Vec3d old_normal = m_surf.get_triangle_normal(current_triangle);

    Vec3d new_normal;

    double new_area;
    if (current_triangle[0] == source_vertex ||
        current_triangle[0] == destination_vertex) {
      new_normal = triangle_normal(vertex_new_position,
                                   m_surf.get_position(current_triangle[1]),
                                   m_surf.get_position(current_triangle[2]));
      new_area = triangle_area(vertex_new_position,
                               m_surf.get_position(current_triangle[1]),
                               m_surf.get_position(current_triangle[2]));
    } else if (current_triangle[1] == source_vertex ||
               current_triangle[1] == destination_vertex) {
      new_normal = triangle_normal(m_surf.get_position(current_triangle[0]),
                                   vertex_new_position,
                                   m_surf.get_position(current_triangle[2]));
      new_area = triangle_area(m_surf.get_position(current_triangle[0]),
                               vertex_new_position,
                               m_surf.get_position(current_triangle[2]));
    } else {
      assert(current_triangle[2] == source_vertex ||
             current_triangle[2] == destination_vertex);
      new_normal = triangle_normal(m_surf.get_position(current_triangle[0]),
                                   m_surf.get_position(current_triangle[1]),
                                   vertex_new_position);
      new_area = triangle_area(m_surf.get_position(current_triangle[0]),
...
// --------------------------------------------------------
///
/// Determine whether collapsing an edge will introduce an unacceptable change
/// in volume.
///
// --------------------------------------------------------

bool EdgeCollapser::collapse_edge_introduces_volume_change(
    size_t source_vertex, size_t edge_index, const Vec3d& vertex_new_position) {
  //
  // If any incident triangle has a tiny area, collapse the edge without regard
  // to volume change
  //

  const std::vector<size_t>& inc_tris =
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index];
  ...
  const std::vector<size_t>& triangles_incident_to_vertex =
      m_surf.m_mesh.m_vertex_to_triangle_map[source_vertex];
  double volume_change = 0;

  for (size_t i = 0; i < triangles_incident_to_vertex.size(); ++i) {
    const Vec3st& inc_tri =
        m_surf.m_mesh.get_triangle(triangles_incident_to_vertex[i]);
    volume_change += signed_volume(
        vertex_new_position, m_surf.get_position(inc_tri[0]),
        m_surf.get_position(inc_tri[1]), m_surf.get_position(inc_tri[2]));
  }
...
// ---------------------------------------------------------
///
/// Returns true if the edge collapse would introduce a triangle with a min or
/// max angle outside of the specified min or max.
///
// ---------------------------------------------------------

bool EdgeCollapser::collapse_edge_introduces_bad_angle(
    size_t source_vertex, size_t destination_vertex,
    const Vec3d& vertex_new_position) {
  std::vector<size_t> moving_triangles;
  get_moving_triangles(source_vertex, destination_vertex, moving_triangles);
  ...
  int edge_id =
      (int)m_surf.m_mesh.get_edge_index(source_vertex, destination_vertex);
  ...
  for (size_t i = 0; i < moving_triangles.size(); ++i) {
    ...
    double mina = min_triangle_angle(m_surf.get_position(tri[0]),
                                     m_surf.get_position(tri[1]),
                                     m_surf.get_position(tri[2]));
    double maxa = max_triangle_angle(m_surf.get_position(tri[0]),
                                     m_surf.get_position(tri[1]),
                                     m_surf.get_position(tri[2]));

    // assert(mina >= 0); //This was failing. Is it a NaN or really a negative
    // angle?
    assert(mina == mina);
    assert(maxa == maxa);

    if (min_tri_angle < 0 || mina < min_tri_angle) min_tri_angle = mina;
    if (max_tri_angle < 0 || maxa > max_tri_angle) max_tri_angle = maxa;
  }
  ...
  for (size_t i = 0; i < moving_triangles.size(); ++i) {
    ...
    const std::vector<size_t>& triangles_incident_to_edge =
        m_surf.m_mesh.m_edge_to_triangle_map[edge_id];
...
// --------------------------------------------------------
bool EdgeCollapser::get_new_vertex_position_dihedral(
    Vec3d& vertex_new_position, size_t& vertex_to_keep,
    size_t& vertex_to_delete, const size_t& edge, Vec3c& new_vert_solid_label) {
  // rank 1, 2, 3 = smooth, ridge, peak
  // if the vertex ranks don't match, keep the higher rank vertex

  // TODO: We should eliminate the use of the term "rank", since we are
  // technically no longer using ranks to determine features

  int keep_incident_features = m_surf.vertex_feature_edge_count(vertex_to_keep);
  int delete_incident_features =
      m_surf.vertex_feature_edge_count(vertex_to_delete);

  // For now, map these to the existing rank scores
  unsigned int keep_rank =
      keep_incident_features < 2 ? 1 : (keep_incident_features >= 3 ? 3 : 2);
  unsigned int delete_rank = delete_incident_features < 2
                                 ? 1
                                 : (delete_incident_features >= 3 ? 3 : 2);

  bool edge_is_a_feature = m_surf.edge_is_feature(edge);

  bool keep_vert_is_boundary =
      m_surf.m_mesh.m_is_boundary_vertex[vertex_to_keep];
  bool del_vert_is_boundary =
      m_surf.m_mesh.m_is_boundary_vertex[vertex_to_delete];

  // situations where large collapse threshold applies:
  //  1. one of the two vertices has no feature edges
  //  2. both have feature edges, and the edge is a feature, and one of the two
  //  vertices has exactly two feature edges
  bool large_threshold =
      ((keep_rank == 1 || delete_rank == 1) ||
       (((keep_rank == 2 &&
          m_surf.vertex_feature_is_smooth_ridge(vertex_to_keep)) ||
         (delete_rank == 2 &&
          m_surf.vertex_feature_is_smooth_ridge(vertex_to_delete))) &&
        m_surf.edge_is_feature(edge)));
  large_threshold =
      true;  // always use large thresholds for bubbles! because there're no
             // features other than triple junctions that we care about.

  large_threshold = large_threshold ||
                    m_surf.m_aggressive_mode;  // if we are in aggressive mode,
                                               // use the large threshold

  double len = mag(m_surf.get_position(vertex_to_keep) -
                   m_surf.get_position(vertex_to_delete));
  if (!large_threshold && len >= m_t1_pull_apart_distance) {
    if (m_surf.m_verbose) std::cout << "!large thresh and len >= t1_dist\n";
    return false;
  }
  ...
  bool keep_vert_is_manifold =
      !m_surf.m_mesh.is_vertex_nonmanifold(vertex_to_keep);
  bool delete_vert_is_manifold =
      !m_surf.m_mesh.is_vertex_nonmanifold(vertex_to_delete);
  ...
  if (keep_vert_is_any_solid || delete_vert_is_any_solid) {
    ...
    new_vert_solid_label =
        m_surf.m_solid_vertices_callback->generate_collapsed_solid_label(
            m_surf, vertex_to_keep, vertex_to_delete, keep_vert_is_solid,
            delete_vert_is_solid);
  }
  ...
  if (m_surf.m_allow_vertex_movement_during_collapse &&
      !(keep_vert_is_boundary || del_vert_is_boundary) &&
      !(keep_vert_is_any_solid || delete_vert_is_any_solid)) {
    // Ranks dominate (i.e. use ranks to decide collapsing first, and if they
    // match then use nonmanifoldness to decide).
    //-> This is particularly important for outward normal flow: it snaps the
    //non-manifold curve back onto the feature curve produced at merge points.
    // (Other scenarios might(?) work better with nonmanifoldness dominating; so
    // I've left the option in the code for now - Christopher Batty.)
    ...
    if (ranks_dominate) {
      if (keep_rank > delete_rank) {
        ...
        // same ranks, but one is non-manifold; may as well prefer to keep
        // non-manifold points.
        if (!keep_vert_is_manifold && delete_vert_is_manifold) {
          ...
          m_surf.m_subdivision_scheme->generate_new_midpoint(
              edge, m_surf, vertex_new_position);
        }
      }
    } else {
    ...
    if (!keep_vert_is_any_solid) std::swap(vertex_to_keep, vertex_to_delete);

    Vec3d newpos = (m_surf.get_position(vertex_to_keep) +
                    m_surf.get_position(vertex_to_delete)) /
                   2;
    if (!m_surf.m_solid_vertices_callback->generate_collapsed_position(
            m_surf, vertex_to_keep, vertex_to_delete, newpos)) {
...
bool EdgeCollapser::collapse_edge(size_t edge) {
  ...
  if (m_surf.m_aggressive_mode) {
    ...
    for (size_t i = 0;
         i < m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_keep].size(); i++)
      incident_triangles.push_back(
          m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_keep][i]);
    for (size_t i = 0;
         i < m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_delete].size();
         i++)
      incident_triangles.push_back(
          m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_delete][i]);
    bool bad_angle = false;
    for (size_t i = 0; i < incident_triangles.size(); i++)
      if (m_surf.triangle_with_bad_angle(incident_triangles[i]))
        bad_angle = true;
    if (!bad_angle)  // do not collapse in aggressive mode if none of the
                     // involved triangles have bad angles.
      return false;
  }
  ...
  // either we're allowing remeshing of boundary edges, or this edge is not on
  // the boundary.
  assert(m_remesh_boundaries || !edge_is_boundary);

  Vec3d rel_vel = m_surf.get_remesh_velocity(vertex_to_keep) -
                  m_surf.get_remesh_velocity(vertex_to_delete);
  Vec3d edge_vec = m_surf.get_position(vertex_to_keep) -
                   m_surf.get_position(vertex_to_delete) - rel_vel;
  ...
  if (!m_surf.m_t1_transition_enabled && causes_irregular) return false;

  if ((dot(rel_vel, edge_vec) > 0 || edge_len >= m_t1_pull_apart_distance) &&
      causes_irregular && !m_surf.m_aggressive_mode) {
    if (m_surf.m_verbose)
      std::cout << "The collapse will produce irregular junction, but the "
                   "endpoints are moving apart. No need to collapse."
                << std::endl;
    return false;
  }

  if (m_surf.m_verbose) {
    std::cout << "Collapsing edge.  Doomed vertex: " << vertex_to_delete
              << " --- Vertex to keep: " << vertex_to_keep << std::endl;
  }

  // --------------

  // If we're disallowing topology changes, don't let an edge collapse form a
  // degenerate tet

  if (false == m_surf.m_allow_non_manifold) {
    bool would_be_non_manifold = false;

    // for each triangle that *would* be created, make sure that there isn't
    // already a triangle with those 3 vertices

    for (size_t i = 0;
         i < m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_delete].size();
         ++i) {
      Vec3st new_triangle = m_surf.m_mesh.get_triangle(
          m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_delete][i]);
      if (new_triangle[0] == vertex_to_delete) {
        new_triangle[0] = vertex_to_keep;
      }
      if (new_triangle[1] == vertex_to_delete) {
        new_triangle[1] = vertex_to_keep;
      }
      if (new_triangle[2] == vertex_to_delete) {
        new_triangle[2] = vertex_to_keep;
      }

      for (size_t j = 0;
           j < m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_keep].size();
           ++j) {
        if (NonDestructiveTriMesh::triangle_has_these_verts(
                m_surf.m_mesh.get_triangle(
                    m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_keep][j]),
                new_triangle)) {
          if (m_surf.m_verbose) {
            std::cout << "would_be_non_manifold" << std::endl;
          }
    ...
    // Look for a vertex which is adjacent to both vertices on the edge, and
    // which isn't on one of the incident triangles

    const std::vector<size_t>& triangles_incident_to_edge =
        m_surf.m_mesh.m_edge_to_triangle_map[edge];
    std::vector<size_t> third_vertices;

    for (size_t i = 0; i < triangles_incident_to_edge.size(); ++i) {
      const Vec3st& inc_triangle =
          m_surf.m_mesh.get_triangle(triangles_incident_to_edge[i]);
      ...
      for (size_t j = 0; j < adj_vertices1.size(); ++j) {
        if (adj_vertices0[i] == adj_vertices1[j]) {
          ...
          if (!is_on_inc_triangle) {
            // found a vertex adjacent to both edge vertices, which doesn't lie
            // on the incident triangles

            //
            if (m_surf.m_verbose) {
              std::cout << " --- Edge Collapser: found a vertex adjacent to "
                           "both edge vertices, which doesn't lie on the "
                           "incident triangles "
                        << std::endl;
              std::cout << " --- Adjacent vertex: " << adj_vertices0[i]
                        << ", incident triangles: ";
            }
  ...
  {
    const std::vector<size_t>& r_triangles_incident_to_edge =
        m_surf.m_mesh.m_edge_to_triangle_map[edge];

    // Do not collapse edge on a degenerate tet or degenerate triangle
    for (size_t i = 0; i < r_triangles_incident_to_edge.size(); ++i) {
      const Vec3st& triangle_i =
          m_surf.m_mesh.get_triangle(r_triangles_incident_to_edge[i]);

      if (triangle_i[0] == triangle_i[1] || triangle_i[1] == triangle_i[2] ||
          triangle_i[2] == triangle_i[0]) {
        if (m_surf.m_verbose) {
          std::cout << "duplicate vertices on triangle" << std::endl;
        }
        return false;
      }

      for (size_t j = i + 1; j < r_triangles_incident_to_edge.size(); ++j) {
        const Vec3st& triangle_j =
            m_surf.m_mesh.get_triangle(r_triangles_incident_to_edge[j]);

        if (NonDestructiveTriMesh::triangle_has_these_verts(triangle_i,
                                                            triangle_j)) {
          if (m_surf.m_verbose) {
            std::cout << "two triangles share vertices" << std::endl;
          }
  ...
  bool can_collapse = get_new_vertex_position_dihedral(
      vertex_new_position, vertex_to_keep, vertex_to_delete, edge,
      new_vert_solid_label);
  if (!can_collapse) return false;
  ...
  if (m_surf.get_position(m_surf.m_mesh.m_edges[edge][1]) !=
      m_surf.get_position(m_surf.m_mesh.m_edges[edge][0]))
  // if ( mag ( m_surf.get_position(m_surf.m_mesh.m_edges[edge][1]) -
  // m_surf.get_position(m_surf.m_mesh.m_edges[edge][0]) ) > 0 )
  {
    // Change source vertex predicted position to superimpose onto destination
    // vertex
    m_surf.set_newposition(vertex_to_keep, vertex_new_position);
    m_surf.set_newposition(vertex_to_delete, vertex_new_position);

    bool volume_change = collapse_edge_introduces_volume_change(
        vertex_to_delete, edge, vertex_new_position);
    volume_change = false;  // ignore volume changes for bubbles!

    if (volume_change && !m_surf.m_aggressive_mode) {
      // Restore saved positions which were changed by the function we just
      // called.
      m_surf.set_newposition(vertex_to_keep,
                             m_surf.get_position(vertex_to_keep));
      m_surf.set_newposition(vertex_to_delete,
                             m_surf.get_position(vertex_to_delete));

      g_stats.add_to_int("EdgeCollapser:collapse_volume_change", 1);

      if (m_surf.m_verbose) {
        std::cout << "collapse_volume_change" << std::endl;
      }
      return false;
    }

    bool normal_inversion = collapse_edge_introduces_normal_inversion(
        vertex_to_delete, vertex_to_keep, edge, vertex_new_position);

    if (normal_inversion &&
        !m_surf
             .m_aggressive_mode)  //&& (edge_len >= m_t1_pull_apart_distance) )
    {
      // Restore saved positions which were changed by the function we just
      // called.
      m_surf.set_newposition(vertex_to_keep,
      ...
      if (m_surf.m_verbose) {
        std::cout << "normal_inversion" << std::endl;
      }
      return false;
    }

    bool bad_angle = collapse_edge_introduces_bad_angle(
        vertex_to_delete, vertex_to_keep, vertex_new_position);

    if (bad_angle &&
        !m_surf.m_aggressive_mode)  //&& edge_len >= m_t1_pull_apart_distance )
    {
      // Restore saved positions which were changed by the function we just
      // called.
      m_surf.set_newposition(vertex_to_keep,
    ...
    if (m_surf.m_collision_safety) {
      collision = collapse_edge_pseudo_motion_introduces_collision(
          vertex_to_delete, vertex_to_keep, edge, vertex_new_position);
    }
    ...
    if (collision) {
      // edge collapse would introduce collision or change volume too much or
  ...
  for (int i = 0; i < 3; i++) {
    if (new_vert_solid_label[i])
      m_surf.m_masses[vertex_to_keep][i] =
  ...
  // Copy this vector, don't take a reference, as deleting will change the
  // original
  std::vector<size_t> triangles_incident_to_edge =
      m_surf.m_mesh.m_edge_to_triangle_map[edge];
  ...
  for (size_t i = 0; i < triangles_incident_to_edge.size(); ++i) {
    if (m_surf.m_verbose) {
      std::cout << "removing edge-incident triangle: "
                << m_surf.m_mesh.get_triangle(triangles_incident_to_edge[i])
                << std::endl;
    }
  ...
  std::vector<size_t> triangles_incident_to_vertex =
      m_surf.m_mesh.m_vertex_to_triangle_map[vertex_to_delete];

  for (size_t i = 0; i < triangles_incident_to_vertex.size(); ++i) {
    for (size_t local_ind = 0; local_ind < triangles_incident_to_edge.size();
         ++local_ind) {
      assert(triangles_incident_to_vertex[i] !=
             triangles_incident_to_edge[local_ind]);
    }
    ...
    if (m_surf.m_verbose) {
      std::cout << "adding updated triangle: " << new_triangle << std::endl;
    }
    ...
    //
    Vec2i label =
        m_surf.m_mesh.get_triangle_label(triangles_incident_to_vertex[i]);
  ...
  for (size_t i = 0; i < triangles_incident_to_vertex.size(); ++i) {
    if (m_surf.m_verbose) {
      std::cout << "removing vertex-incident triangle: "
                << m_surf.m_mesh.get_triangle(triangles_incident_to_vertex[i])
                << std::endl;
    }
  ...
  if (m_surf.m_mesheventcallback)
    m_surf.m_mesheventcallback->post_collapse(m_surf, edge, vertex_to_keep,
                                              data);
...
bool EdgeCollapser::edge_is_collapsible(size_t edge_index,
  ...
  if (m_surf.edge_is_any_solid(edge_index)) {
    return false;
  }
  ...
  for (size_t i = 0;
       i < m_surf.m_mesh.m_edge_to_triangle_map[edge_index].size(); ++i) {
    ...
    if (rad2deg(cur_angle) < m_surf.m_min_triangle_angle) return true;
  }

  current_length = m_surf.get_edge_length(edge_index);
  if (m_use_curvature) {
    // collapse if we're below the lower limit
    if (current_length < m_min_edge_length) return true;

    // don't collapse if we're near the upper limit, since it can produce edges
    // above the limit
    if (current_length > m_max_edge_length * 0.5) return false;
    ...
    if (current_length < 3 * min_nbr_len) return false;

    double curvature_value =
        get_edge_curvature(m_surf, m_surf.m_mesh.m_edges[edge_index][0],
                           m_surf.m_mesh.m_edges[edge_index][1]);
...
bool EdgeCollapser::collapse_pass() {
  if (m_surf.m_verbose) {
    std::cout << "\n\n\n---------------------- EdgeCollapser: collapsing "
                 "----------------------"
              << std::endl;
    std::cout << "m_min_edge_length: " << m_min_edge_length;
    std::cout << ", m_use_curvature: " << m_use_curvature;
    std::cout << ", m_min_curvature_multiplier: " << m_min_curvature_multiplier
              << std::endl;
    std::cout << "m_surf.m_collision_safety: " << m_surf.m_collision_safety
              << std::endl;
  }
  ...
  if (m_surf.m_verbose) {
    std::cout << sortable_edges_to_try.size() << " candidate edges sorted"
              << std::endl;
...
bool EdgeCollapser::collapse_will_produce_irregular_junction(size_t edge) {
  ...
  for (size_t i = 0; i < mesh.m_vertex_to_triangle_map[a].size(); i++) {
    const Vec2i& l =
        mesh.get_triangle_label(mesh.m_vertex_to_triangle_map[a][i]);
    if (l[0] >= 0) {
  ...
  for (size_t i = 0; i < mesh.m_vertex_to_triangle_map[b].size(); i++) {
    const Vec2i& l =
        mesh.get_triangle_label(mesh.m_vertex_to_triangle_map[b][i]);
    if (l[0] >= 0) {
      if (l[0] >= (int)regions.size()) regions.resize(l[0] + 1, false);
      regions[l[0]] = true;
    }
  ...
  for (int i = 0; i < nr; i++)
    for (int j = i + 1; j < nr; j++) {
      if (regions[i] && regions[j])
        if (!regiongraph[i * nr + j]) irregular = true;
    }
