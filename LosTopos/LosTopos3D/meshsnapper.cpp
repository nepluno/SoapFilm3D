...
//  The opposite of a pinch operation. This was adapted from the edge collapse
//  code.
//
// ---------------------------------------------------------
...
MeshSnapper::MeshSnapper(SurfTrack& surf, bool use_curvature,
                         bool remesh_boundaries, int max_curvature_multiplier)
    : m_surf(surf),
      m_edge_threshold(0.03),
      m_face_threshold(0.025),
      m_facesplitter(surf),
      m_edgesplitter(surf, use_curvature, remesh_boundaries,
                     max_curvature_multiplier) {}
...
bool MeshSnapper::snap_introduces_normal_inversion(
...
bool MeshSnapper::snap_pseudo_motion_introduces_collision(
    size_t source_vertex, size_t destination_vertex, const Vec3d&) {
...
// --------------------------------------------------------
///
/// Snap a pair of vertices by moving the source vertex to the destination
/// vertex
///
// --------------------------------------------------------

bool MeshSnapper::snap_vertex_pair(size_t vertex_to_keep,
                                   size_t vertex_to_delete) {
  ...
  // --------------
  // decide on new vertex position: just take the mass-weighted average of the
  // two points to be snapped, so that constraints marked by infinite masses are
  // respected.
  ...
  for (int i = 0; i < 3; i++) {
    if (m0[i] == std::numeric_limits<double>::infinity() &&
        m1[i] == std::numeric_limits<double>::infinity()) {
      // both vertices are constrained in this direction: this requires the two
      // cooresponding coordindates are equal
      assert(m_surf.get_position(vertex_to_delete)[i] ==
             m_surf.get_position(vertex_to_keep)[i]);
      ...
      vertex_new_position[i] = 0.5 * (m_surf.get_position(vertex_to_delete)[i] +
                                      m_surf.get_position(vertex_to_keep)[i]);
    }
  }
  ...
  if (mag(m_surf.get_position(vertex_to_delete) -
          m_surf.get_position(vertex_to_keep)) > 0) {
    ...
    bool normal_inversion = snap_introduces_normal_inversion(
    ...
    if (m_surf.m_collision_safety) {
      collision = snap_pseudo_motion_introduces_collision(
          vertex_to_delete, vertex_to_keep, vertex_new_position);
    }
  ...
  if (!m_surf.vertex_is_all_solid(vertex_to_keep) &&
      m_surf.vertex_is_all_solid(vertex_to_delete))
  ...
  if (m_surf.m_mesheventcallback)
    m_surf.m_mesheventcallback->pre_snap(m_surf, vertex_to_delete,
                                         vertex_to_keep, &data);
  ...
  for (size_t i = 0; i < triangles_incident_to_vertex.size(); ++i) {
    ...
    if (m_surf.m_mesh.triangle_is_deleted(triangles_incident_to_vertex[i]))
      continue;
  ...
  if (m_surf.m_mesheventcallback)
    m_surf.m_mesheventcallback->post_snap(m_surf, vertex_to_keep,
                                          vertex_to_delete, data);
...
bool MeshSnapper::snap_edge_pair(size_t edge0, size_t edge1) {
  ...
  check_edge_edge_proximity(v0, v1, v2, v3, distance, s0, s2, normal);

  // compute the resulting closest points
  Vec3d midpoint0 = s0 * v0 + (1 - s0) * v1;
  Vec3d midpoint1 = s2 * v2 + (1 - s2) * v3;

  // Check if we're fairly close to an end vertex; if so just use the vertex
  // directly for snapping. otherwise, do a split to create a new point which
  // will then be snapped.

  if (m_surf.m_mesheventcallback) {
    m_surf.m_mesheventcallback->log()
        << "ee snap: edge0 = " << edge0 << ": " << edge_data0[0] << " (" << v0
        << ") -> " << edge_data0[1] << " (" << v1 << ") edge1 = " << edge1
        << ": " << edge_data1[0] << " (" << v2 << ") -> " << edge_data1[1]
        << " (" << v3 << ")" << std::endl;
    m_surf.m_mesheventcallback->log()
        << "s0 = " << s0 << " s2 = " << s2 << " dist = " << distance
        << " midpoint0 = " << midpoint0 << " midpoint1 = " << midpoint1
        << std::endl;
  }
  ...
  if (s0 > 1 - m_edge_threshold) {
    ...
    if (m_surf.m_mesheventcallback)
      m_surf.m_mesheventcallback->log()
          << "attemping to split edge0 at " << midpoint0 << std::endl;

    if (!m_edgesplitter.edge_is_splittable(edge0) ||
        !m_edgesplitter.split_edge(edge0, split_result, false, true, &midpoint0,
  ...
  if (s2 > 1 - m_edge_threshold) {
    ...
    if (m_surf.m_mesheventcallback)
      m_surf.m_mesheventcallback->log()
          << "attempting to split edge1 at " << midpoint1 << std::endl;

    if (!m_edgesplitter.edge_is_splittable(edge1) ||
        !m_edgesplitter.split_edge(edge1, split_result, false, true, &midpoint1,
                                   std::vector<size_t>(1, snapping_vert0),
                                   true))
      return false;
  ...
  if (m_surf.m_mesheventcallback)
    m_surf.m_mesheventcallback->log()
        << "attempting to snap vertex " << snapping_vert0 << " to "
        << snapping_vert1 << std::endl;

  // finally, if the splitting succeeds and we have a good vertex pair, try to
  // snap them.
  bool success = vert_pair_is_snappable(snapping_vert0, snapping_vert1) &&
                 snap_vertex_pair(snapping_vert0, snapping_vert1);

  if (m_surf.m_mesheventcallback)
    m_surf.m_mesheventcallback->log()
        << "snap " << (success ? "succeeded" : "failed") << std::endl;
...
bool MeshSnapper::snap_face_vertex_pair(size_t face, size_t vertex) {
  ...
  check_point_triangle_proximity(v_pos, t0_pos, t1_pos, t2_pos, dist, s0, s1,
                                 s2, normal);

  // Depending on the barycentric coordinates, either snap to one of the face
  // vertices, split an edge and snap to it, or split the face and snap to it.
  if (m_surf.m_mesheventcallback) {
    m_surf.m_mesheventcallback->log()
        << "vf snap: v = " << vertex << " (" << v_pos
        << ") f = " << face_data[0] << " (" << t0_pos << ") " << face_data[1]
        << " (" << t1_pos << ") " << face_data[2] << " (" << t2_pos << ")"
        << std::endl;
    m_surf.m_mesheventcallback->log()
        << "s0 = " << s0 << " s1 = " << s1 << " s2 = " << s2
        << " dist = " << dist
        << " nearest = " << (t0_pos * s0 + t1_pos * s1 + t2_pos * s2)
        << std::endl;
  }
  ...
  if (s0 < m_face_threshold) {
    if (s1 < m_face_threshold) {
      ...
      size_t edge_to_split =
          m_surf.m_mesh.get_edge_index(face_data[1], face_data[2]);
      ...
      if (m_surf.m_mesheventcallback)
        m_surf.m_mesheventcallback->log()
            << "attempting to snap to e12: " << split_point << std::endl;

      if (!m_edgesplitter.edge_is_splittable(edge_to_split) ||
          !m_edgesplitter.split_edge(edge_to_split, result_vertex, false, true,
                                     &split_point, std::vector<size_t>(), true))
        return false;
    ...
    if (s2 < m_face_threshold) {
      ...
      size_t result_vertex;
      size_t edge_to_split =
          m_surf.m_mesh.get_edge_index(face_data[0], face_data[2]);
      ...
      if (m_surf.m_mesheventcallback)
        m_surf.m_mesheventcallback->log()
            << "attempting to snap to e02: " << split_point << std::endl;
    ...
    size_t result_vertex;
    size_t edge_to_split =
        m_surf.m_mesh.get_edge_index(face_data[0], face_data[1]);
    ...
    if (m_surf.m_mesheventcallback)
      m_surf.m_mesheventcallback->log()
          << "attempting to snap to e01: " << split_point << std::endl;
    ...
    if (m_surf.m_mesheventcallback)
      m_surf.m_mesheventcallback->log()
          << "split face: " << split_point << std::endl;

    if (!m_facesplitter.face_is_splittable(face) ||
        !m_facesplitter.split_face(face, result_vertex, true, &split_point))
      return false;
  ...
  if (m_surf.m_mesheventcallback)
    m_surf.m_mesheventcallback->log()
        << "attempting to snap vertex " << snapping_vertex << " to " << vertex
        << std::endl;

  // finally, if the splitting succeeds and we have a good vertex pair, try to
  // snap them.
  bool success = vert_pair_is_snappable(snapping_vertex, vertex) &&
                 snap_vertex_pair(snapping_vertex, vertex);
...
bool MeshSnapper::vert_pair_is_snappable(size_t vert0, size_t vert1) {
  ...
  if (m_surf.m_mesh.vertex_is_deleted(vert0) ||
      m_surf.m_mesh.vertex_is_deleted(vert1))
    return false;

  // shouldn't be calling this on duplicate vertices
  assert(vert0 != vert1);

  // Check if there is an edge connecting the two vertices - if so, we can't do
  // the snap
  size_t edge_id = m_surf.m_mesh.get_edge_index(vert0, vert1);
  if (edge_id != m_surf.m_mesh.m_edges.size()) return false;
...
bool MeshSnapper::edge_pair_is_snappable(size_t edge0, size_t edge1,
  ...
  //(otherwise, we're essentially snapping triangle to be degenerate/zero area
  //in its plane.)
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      if (edge_index != m_surf.m_mesh.m_edges.size()) {
        ...
        for (size_t k = 0;
             k < m_surf.m_mesh.m_edge_to_triangle_map[edge_index].size(); ++k) {
        ...
        // they shared an edge but we couldn't find a potentially offending
        // triangle pair
        ...
        // if the tris are _not_ oriented consistently, flip the normal to make
        // the comparison valid.
        Vec2st shared_edge_data = m_surf.m_mesh.m_edges[edge_index];
        if (m_surf.m_mesh.oriented(shared_edge_data[0], shared_edge_data[1],
                                   tri0_data) !=
            m_surf.m_mesh.oriented(shared_edge_data[1], shared_edge_data[0],
                                   tri1_data))
          normal1 = -normal1;
  ...
  check_edge_edge_proximity(
      m_surf.get_position(edge_data0[0]), m_surf.get_position(edge_data0[1]),
      m_surf.get_position(edge_data1[0]), m_surf.get_position(edge_data1[1]),
      current_length, s0, s2, normal);

  if (current_length < m_surf.m_merge_proximity_epsilon) {
    // check for "dimensional drop-down" cases which would lead to snapping two
    // vertices that already share an edge.
    if (s0 > 1 - m_edge_threshold) {
      if (s2 > 1 - m_edge_threshold) {
        if (m_surf.m_mesh.get_edge_index(edge_data0[0], edge_data1[0]) !=
            m_surf.m_mesh.m_edges.size())
          return false;
      } else if (s2 < m_edge_threshold) {
        if (m_surf.m_mesh.get_edge_index(edge_data0[0], edge_data1[1]) !=
            m_surf.m_mesh.m_edges.size())
          return false;
      }
    } else if (s0 < m_edge_threshold) {
      if (s2 > 1 - m_edge_threshold) {
        if (m_surf.m_mesh.get_edge_index(edge_data0[1], edge_data1[0]) !=
            m_surf.m_mesh.m_edges.size())
          return false;
      } else if (s2 < m_edge_threshold) {
        if (m_surf.m_mesh.get_edge_index(edge_data0[1], edge_data1[1]) !=
            m_surf.m_mesh.m_edges.size())
          return false;
      }
    }
...
bool MeshSnapper::face_vertex_pair_is_snappable(size_t face, size_t vertex,
                                                double& current_length) {
  // skip deleted pairs
  if (m_surf.m_mesh.triangle_is_deleted(face) ||
      m_surf.m_mesh.vertex_is_deleted(vertex))
    return false;

  // face shouldn't contain the vertex to be snapped
  const Vec3st& face_data = m_surf.m_mesh.m_tris[face];
  if (m_surf.m_mesh.triangle_contains_vertex(face_data, vertex)) return false;

  // check if the vertex is contained in a face adjacent to the one it's merging
  // with. if so, require that the angle between the faces be less than 90
  // degrees.
  Vec3d tri_normal = m_surf.get_triangle_normal(face);
  for (int i = 0; i < 3; ++i) {
    size_t edge = m_surf.m_mesh.m_triangle_to_edge_map[face][i];
    for (size_t j = 0; j < m_surf.m_mesh.m_edge_to_triangle_map[edge].size();
         ++j) {
      ...
      if (m_surf.m_mesh.triangle_contains_vertex(other_tri_data, vertex)) {
        ...
        // if the tris are _not_ oriented consistently, flip the normal to make
        // the comparison valid.
        if (m_surf.m_mesh.oriented(m_surf.m_mesh.m_edges[edge][0],
                                   m_surf.m_mesh.m_edges[edge][1], face_data) !=
            m_surf.m_mesh.oriented(m_surf.m_mesh.m_edges[edge][1],
                                   m_surf.m_mesh.m_edges[edge][0],
                                   other_tri_data)) {
  ...
  check_point_triangle_proximity(
      m_surf.get_position(vertex), m_surf.get_position(face_data[0]),
      m_surf.get_position(face_data[1]), m_surf.get_position(face_data[2]),
      current_length, s0, s1, s2, normal);

  if (current_length < m_surf.m_merge_proximity_epsilon) {
    // anticipate the case where we would drop down to vertex snapping
    // but there is already a connecting edge. That should be an edge collapse,
    // not a snap.
    if (s1 < m_face_threshold && s2 < m_face_threshold &&
        m_surf.m_mesh.get_edge_index(face_data[0], vertex) !=
            m_surf.m_mesh.m_edges.size())
      return false;

    if (s0 < m_face_threshold && s2 < m_face_threshold &&
        m_surf.m_mesh.get_edge_index(face_data[1], vertex) !=
            m_surf.m_mesh.m_edges.size())
      return false;

    if (s0 < m_face_threshold && s1 < m_face_threshold &&
        m_surf.m_mesh.get_edge_index(face_data[2], vertex) !=
            m_surf.m_mesh.m_edges.size())
      return false;
...
bool MeshSnapper::snap_pass() {
  if (m_surf.m_verbose) {
    std::cout << "\n\n\n---------------------- MeshSnapper: collapsing "
                 "----------------------"
              << std::endl;
    std::cout << "m_merge_proximity_epsilon: "
              << m_surf.m_merge_proximity_epsilon;
  }
  ...
  for (size_t vertex = 0; vertex < m_surf.get_num_vertices(); ++vertex) {
    ...
    m_surf.m_broad_phase->get_potential_triangle_collisions(
        vmin, vmax, false, true, overlapping_tris);
  ...
  for (size_t edge0 = 0; edge0 < m_surf.m_mesh.m_edges.size(); ++edge0) {
    ...
    m_surf.m_broad_phase->get_potential_edge_collisions(vmin, vmax, false, true,
                                                        overlapping_edges);
  ...
  // TODO: can we update the local geometry after merges to find newly formed
  // proximities and then using a priority queue, make sure to test them
  // immediately if new ones are closer?

  std::sort(sortable_pairs_to_try.begin(), sortable_pairs_to_try.end());

  if (m_surf.m_verbose) {
    std::cout << sortable_pairs_to_try.size() << " candidate pairs sorted"
              << std::endl;
  }
  ...
  if (m_surf.m_mesheventcallback)
    for (size_t si = 0; si < sortable_pairs_to_try.size(); si++)
      m_surf.m_mesheventcallback->log()
          << "Snap pair: " << sortable_pairs_to_try[si].m_length << ", "
          << (sortable_pairs_to_try[si].m_face_vert_proximity ? "vf" : "ee")
          << " pair: " << sortable_pairs_to_try[si].m_index0 << " and "
          << sortable_pairs_to_try[si].m_index1 << std::endl;
  ...
  for (size_t si = 0; si < sortable_pairs_to_try.size(); ++si) {
    ...
    if (m_surf.m_mesheventcallback)
      m_surf.m_mesheventcallback->log()
          << "Snap pair to try: "
          << (sortable_pairs_to_try[si].m_face_vert_proximity ? "vf" : "ee")
          << " pair: " << ind0 << " and " << ind1 << " with distance "
          << sortable_pairs_to_try[si].m_length << std::endl;
