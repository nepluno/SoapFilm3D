...
//
//  A triangle mesh with associated vertex locations and  masses.  Query
//  functions for getting geometry info.
//
// ---------------------------------------------------------
...
DynamicSurface::DynamicSurface(const std::vector<Vec3d>& vertex_positions,
                               ...
                               bool in_collision_safety, bool in_verbose)
    : m_proximity_epsilon(in_proximity_epsilon),
      ...
      m_collision_pipeline(
          NULL),  // allocated and initialized in the constructor body
  ...
  // Some compilers worry about using "this" in the initialization list, so
  // initialize it here
  m_collision_pipeline =
      new CollisionPipeline(*this, *m_broad_phase, in_friction_coefficient);
...
// ---------------------------------------------------------
///
/// Destructor. Frees memory allocated by DynamicSurface for the broad phase and
/// collision pipeline objects.
///
// ---------------------------------------------------------
...
double DynamicSurface::distance_to_surface(const Vec3d& p,
                                           size_t& closest_triangle) const {
  ...
  while (min_distance == BIG_DOUBLE) {
    ...
    m_broad_phase->get_potential_triangle_collisions(xmin, xmax, true, true,
                                                     nearby_triangles);
    ...
    for (size_t j = 0; j < nearby_triangles.size(); ++j) {
      ...
      check_point_triangle_proximity(p, get_position(tri[0]),
                                     get_position(tri[1]), get_position(tri[2]),
                                     curr_distance);
...
// --------------------------------------------------------
///
/// Break up the triangle mesh into connected components, determine surface IDs
/// for all vertices.
///
// --------------------------------------------------------

void DynamicSurface::partition_surfaces(
    std::vector<size_t>& surface_ids,
    std::vector<std::vector<size_t> >& surfaces) const {
  ...
  while (true) {
    ...
    for (next_unassigned_vertex = 0;
         next_unassigned_vertex < surface_ids.size();
         ++next_unassigned_vertex) {
      if (m_mesh.m_vertex_to_edge_map[next_unassigned_vertex].empty()) {
        continue;
      }
    ...
    while (false == open.empty()) {
      ...
      if (m_mesh.m_vertex_to_edge_map[vertex_index].empty()) {
        continue;
      }
      ...
      const std::vector<size_t>& incident_edges =
          m_mesh.m_vertex_to_edge_map[vertex_index];

      for (size_t i = 0; i < incident_edges.size(); ++i) {
        size_t adjacent_vertex = m_mesh.m_edges[incident_edges[i]][0];
        if (adjacent_vertex == vertex_index) {
          adjacent_vertex = m_mesh.m_edges[incident_edges[i]][1];
        }
  ...
  for (size_t i = 0; i < surface_ids.size(); ++i) {
    if (m_mesh.m_vertex_to_edge_map[i].empty()) {
      continue;
    }
    ...
    for (size_t j = 0; j < incident_edges.size(); ++j) {
      ...
      if (adjacent_vertex == i) {
        adjacent_vertex = m_mesh.m_edges[incident_edges[j]][1];
      }
...
void DynamicSurface::get_triangle_intersections(
    const Vec3d& segment_point_a, const Vec3d& segment_point_b,
    std::vector<double>& hit_ss, std::vector<size_t>& hit_triangles,
    bool verbose) const {
  Vec3d aabb_low, aabb_high;
  minmax(segment_point_a, segment_point_b, aabb_low, aabb_high);

  // TODO Use some kind of line-drawing algorithm to find a sequence of
  // acceleration grid cells that overlap/enclose the segment, to avoid having to
  // search massive chunks of the mesh.

  std::vector<size_t> overlapping_triangles;
  m_broad_phase->get_potential_triangle_collisions(aabb_low, aabb_high, true,
                                                   true, overlapping_triangles);
  ...
  for (size_t i = 0; i < overlapping_triangles.size(); ++i) {
    ...
    bool hit = segment_triangle_intersection(
        segment_point_a, dummy_index, segment_point_b, dummy_index + 1, v0,
        t[0], v1, t[1], v2, t[2], sa, sb, bary1, bary2, bary3, false, verbose);
...
// ---------------------------------------------------------
///
/// Run intersection detection against all triangles and return the number of
/// hits.
///
// ---------------------------------------------------------

size_t DynamicSurface::get_number_of_triangle_intersections(
    const Vec3d& segment_point_a, const Vec3d& segment_point_b) const {
  int num_hits = 0;
  int num_misses = 0;
  Vec3d aabb_low, aabb_high;
  minmax(segment_point_a, segment_point_b, aabb_low, aabb_high);
  ...
  for (size_t i = 0; i < overlapping_triangles.size(); ++i) {
    ...
    bool hit = segment_triangle_intersection(
        segment_point_a, dummy_index, segment_point_b, dummy_index + 1, v0,
        t[0], v1, t[1], v2, t[2], degenerate_counts_as_hit);
...
unsigned int DynamicSurface::vertex_primary_space_rank(size_t v,
                                                       int region) const {
  ...
  const std::vector<size_t>& incident_triangles =
      m_mesh.m_vertex_to_triangle_map[v];
  ...
  if (labelset.size() <= 2) {
    ...
    // Otherwise, visit each set of the manifold regions separately, take the
    // max. This seems to work somewhat better than doing all at once in the
    // non-manifold case.
    unsigned int max_rank = 0;
    for (std::set<int>::iterator it = labelset.begin(); it != labelset.end();
         ++it) {
      int cur_label = *it;
...
unsigned int DynamicSurface::compute_rank_from_triangles(
    const std::vector<size_t>& triangles) const {
...
double DynamicSurface::get_largest_dihedral(size_t edge) const {
  ...
  for (size_t i = 0; i < tri_list.size(); ++i) {
    ...
    for (size_t j = i + 1; j < tri_list.size(); ++j) {
      ...
      // possibly flip one normal so the tris are oriented in a matching way, to
      // get the right dihedral angle.
      if (m_mesh.oriented(v0, v1, m_mesh.get_triangle(tri_id0)) !=
          m_mesh.oriented(v1, v0, m_mesh.get_triangle(tri_id1))) {
        norm1 = -norm1;
      }
...
/// Look at all triangle pairs and get the smallest angle, ignoring regions.
double DynamicSurface::get_largest_dihedral(
...
/// is used to determine inside/outside. (This can readily be extended to the
/// multi-material case.) Return -1 if we can't make a safe determination
// ---------------------------------------------------------
int DynamicSurface::test_region_via_ray_and_normal(const Vec3d& p,
                                                   const Vec3d& ray_end) {
...
int DynamicSurface::get_region_containing_point(const Vec3d& p) {
  ...
  // We use voting (for the moment) to enhance robustness, in the absence of a
  // geometrically exact test.
  ...
  if (region != -1) region_counts[region]++;
  ...
  if (consistency < 0.9 || total < 4) {
    ...
    for (int i = 0; i < 7; ++i) {
      float x_dir = randhashf(seed, -1, 1);
      ++seed;
      float y_dir = randhashf(seed, -1, 1);
      ++seed;
      float z_dir = randhashf(seed, -1, 1);
      ++seed;
      Vec3d direction(x_dir, y_dir, z_dir);
      if (mag(direction) > 0.00001) normalize(direction);
    ...
    /*std::cout << "Updated region scores: " << (double)max_val << " out
    of 11.\n"; it = region_counts.begin(); for(;it != region_counts.end(); ++it)
    { std::cout << "   " << it->first  << " got " << it->second << std::endl;
    }*/
    consistency = (double)max_val / (double)total;
    if (consistency < 0.9)
      std::cout << "Warning: Consistency is only " << consistency << " out of "
                << total << " rays.\n";
    assert(consistency > 0.6);  // if we didn't even hit 60% consistently the
                                // same, this labeling is pretty dubious
  }
...
void DynamicSurface::integrate(double desired_dt, double& actual_dt) {
  ...
  if (m_verbose) {
    std::cout << "---------------------- Los Topos: integration and collision "
                 "handling --------------------"
              << std::endl;
  }
    ...
    if (m_collision_safety) {
      ...
      if (!intersections.empty()) {
        ...
        if (all_collisions_handled) {
          std::cout << "Intersection in predicted mesh but handle collisions "
                       "returned ok."
                    << std::endl;
          assert(false);
        }
        ...
        if (m_verbose) {
          std::cout << "Intersection in predicted mesh, cutting timestep."
                    << std::endl;
        }
...
void DynamicSurface::rebuild_static_broad_phase() {
  ...
  if (m_verbose) std::cout << "Rebuilding broad phase\n";
  m_broad_phase->update_broad_phase(*this, false);

  if (m_verbose) std::cout << "Done rebuilding broad phase\n";
}
...
void DynamicSurface::update_static_broad_phase(size_t vertex_index) {
  const std::vector<size_t>& incident_tris =
      m_mesh.m_vertex_to_triangle_map[vertex_index];
  const std::vector<size_t>& incident_edges =
      m_mesh.m_vertex_to_edge_map[vertex_index];
  ...
  m_broad_phase->update_vertex(vertex_index, low, high,
                               vertex_is_all_solid(vertex_index));
  ...
  for (size_t t = 0; t < incident_tris.size(); ++t) {
    ...
    m_broad_phase->update_triangle(incident_tris[t], low, high,
                                   triangle_is_all_solid(incident_tris[t]));
  }
  ...
  for (size_t e = 0; e < incident_edges.size(); ++e) {
    ...
    m_broad_phase->update_edge(incident_edges[e], low, high,
                               edge_is_all_solid(incident_edges[e]));
  }
}

// ---------------------------------------------------------
///
/// Update the broadphase elements incident to the given vertex, using current
/// and predicted vertex positions
///
// ---------------------------------------------------------
...
void DynamicSurface::vertex_static_bounds(size_t v, Vec3d& xmin,
                                          Vec3d& xmax) const {
  if (m_mesh.m_vertex_to_triangle_map[v].empty()) {
...
void DynamicSurface::edge_static_bounds(size_t e, Vec3d& xmin,
                                        Vec3d& xmax) const {
...
void DynamicSurface::triangle_static_bounds(size_t t, Vec3d& xmin,
                                            Vec3d& xmax) const {
  ...
  if (tri[0] == tri[1]) {
    ...
    minmax(get_position(tri[0]), get_position(tri[1]), get_position(tri[2]),
           xmin, xmax);
    xmin -= Vec3d(m_aabb_padding);
    xmax += Vec3d(m_aabb_padding);
  }
...
void DynamicSurface::vertex_continuous_bounds(size_t v, Vec3d& xmin,
                                              Vec3d& xmax) const {
  if (m_mesh.m_vertex_to_triangle_map[v].empty()) {
    xmin = Vec3d(m_aabb_padding);
    xmax = -Vec3d(m_aabb_padding);
...
void DynamicSurface::edge_continuous_bounds(size_t e, Vec3d& xmin,
                                            Vec3d& xmax) const {
  const Vec2st& edge = m_mesh.m_edges[e];
  if (edge[0] == edge[1]) {
    xmin = Vec3d(m_aabb_padding);
    ...
    minmax(get_position(edge[0]), get_newposition(edge[0]),
           get_position(edge[1]), get_newposition(edge[1]), xmin, xmax);
    xmin -= Vec3d(m_aabb_padding);
    xmax += Vec3d(m_aabb_padding);
  }
}
...
void DynamicSurface::triangle_continuous_bounds(size_t t, Vec3d& xmin,
                                                Vec3d& xmax) const {
  const Vec3st& tri = m_mesh.get_triangle(t);
  if (tri[0] == tri[1]) {
    xmin = Vec3d(m_aabb_padding);
    ...
    minmax(get_position(tri[0]), get_newposition(tri[0]), get_position(tri[1]),
           get_newposition(tri[1]), get_position(tri[2]),
           get_newposition(tri[2]), xmin, xmax);
...
static bool aabbs_intersect(const Vec3d& a_xmin, const Vec3d& a_xmax,
                            const Vec3d& b_xmin, const Vec3d& b_xmax) {
  if ((a_xmin[0] <= b_xmax[0] && a_xmin[1] <= b_xmax[1] &&
       a_xmin[2] <= b_xmax[2]) &&
      (a_xmax[0] >= b_xmin[0] && a_xmax[1] >= b_xmin[1] &&
       a_xmax[2] >= b_xmin[2])) {
    return true;
  }
...
///
/// Caution: slow!
/// Check the consistency of the broad phase by comparing against the N^2
/// broadphase.
///
// ---------------------------------------------------------
...
void DynamicSurface::check_static_broad_phase_is_up_to_date() const {
  ...
  for (size_t i = 0; i < get_num_vertices(); ++i) {
    ...
    std::vector<size_t> overlapping_triangles;
    m_broad_phase->get_potential_triangle_collisions(
    ...
    for (size_t j = 0; j < m_mesh.num_triangles(); ++j) {
      if (m_mesh.triangle_is_deleted(j)) {
        continue;
      }
    ...
    assert(overlapping_triangles.size() ==
           brute_force_overlapping_triangles.size());

    std::sort(overlapping_triangles.begin(), overlapping_triangles.end());
    std::sort(brute_force_overlapping_triangles.begin(),
              brute_force_overlapping_triangles.end());
  ...
  for (size_t i = 0; i < m_mesh.m_edges.size(); ++i) {
    ...
    m_broad_phase->get_potential_edge_collisions(aabb_low, aabb_high, true,
                                                 true, overlapping_edges);
    ...
    if (overlapping_edges.size() != brute_force_overlapping_edges.size()) {
      ...
      std::cout << "overlapping_edges.size(): " << overlapping_edges.size()
                << std::endl;
      for (size_t k = 0; k < overlapping_edges.size(); ++k) {
        std::cout << k << ": " << overlapping_edges[k] << std::endl;
      }

      std::cout << "brute_force_overlapping_edges.size(): "
                << brute_force_overlapping_edges.size() << std::endl;
    ...
    std::sort(brute_force_overlapping_edges.begin(),
              brute_force_overlapping_edges.end());
  ...
  for (size_t i = 0; i < m_mesh.num_triangles(); ++i) {
    ...
    m_broad_phase->get_potential_vertex_collisions(aabb_low, aabb_high, true,
                                                   true, overlapping_vertices);
    ...
    for (size_t j = 0; j < get_num_vertices(); ++j) {
      ...
      if (aabbs_intersect(aabb_low, aabb_high, vertex_aabb_low,
                          vertex_aabb_high)) {
        brute_force_overlapping_vertices.push_back(j);
      }
    }

    if (overlapping_vertices.size() !=
        brute_force_overlapping_vertices.size()) {
      std::cout << "triangle " << i << ": " << m_mesh.get_triangle(i)
                << std::endl;
      std::cout << "overlapping_vertices.size(): "
                << overlapping_vertices.size() << std::endl;
      for (size_t k = 0; k < overlapping_vertices.size(); ++k) {
        std::cout << k << ": " << overlapping_vertices[k] << " --- ";
        std::cout << "is deleted: "
                  << m_mesh.vertex_is_deleted(overlapping_vertices[k])
                  << std::endl;
      }

      std::cout << "brute_force_overlapping_vertices.size(): "
                << brute_force_overlapping_vertices.size() << std::endl;
      for (size_t k = 0; k < brute_force_overlapping_vertices.size(); ++k) {
        std::cout << k << ": " << brute_force_overlapping_vertices[k]
                  << " --- ";
        std::cout << "is deleted: "
                  << m_mesh.vertex_is_deleted(
                         brute_force_overlapping_vertices[k])
                  << std::endl;
      }
    }

    assert(overlapping_vertices.size() ==
           brute_force_overlapping_vertices.size());

    std::sort(overlapping_vertices.begin(), overlapping_vertices.end());
    std::sort(brute_force_overlapping_vertices.begin(),
              brute_force_overlapping_vertices.end());
...
/// Caution: slow!
/// Check the consistency of the broad phase by comparing against the N^2
/// broadphase.  Checks using current and predicted vertex positions.
///
// ---------------------------------------------------------
  ...
  for (size_t i = 0; i < m_mesh.num_triangles(); ++i) {
    ...
    if (overlapping_vertices.size() !=
      ...
      for (size_t k = 0; k < brute_force_overlapping_vertices.size(); ++k) {
        ...
        Vec3d lo, hi;
        bool is_solid =
            vertex_is_all_solid(brute_force_overlapping_vertices[k]);
        m_broad_phase->get_vertex_aabb(brute_force_overlapping_vertices[k],
                                       is_solid, lo, hi);
        std::cout << "AABB: " << lo << " - " << hi << std::endl;
        std::cout << "x: " << pm_positions[brute_force_overlapping_vertices[k]]
                  << ", new_x: "
                  << pm_newpositions[brute_force_overlapping_vertices[k]]
                  << std::endl;

        bool query_overlaps_broadphase_aabb =
            aabbs_intersect(aabb_low, aabb_high, lo, hi);
        std::cout << "query_overlaps_broadphase_aabb: "
                  << query_overlaps_broadphase_aabb << std::endl;

        BroadPhaseGrid* grid_bf = static_cast<BroadPhaseGrid*>(m_broad_phase);

        const std::vector<Vec3st>& cells =
            grid_bf->m_dynamic_vertex_grid
                .m_elementidxs[brute_force_overlapping_vertices[k]];
...
// --------------------------------------------------------
///
/// Check a triangle (by index) vs all other triangles for any kind of
/// intersection
///
// --------------------------------------------------------

bool DynamicSurface::check_triangle_vs_all_triangles_for_intersection(
    size_t tri_index) {
  return check_triangle_vs_all_triangles_for_intersection(
      m_mesh.get_triangle(tri_index));
}
...
bool DynamicSurface::check_triangle_vs_all_triangles_for_intersection(
  ...
  m_broad_phase->get_potential_triangle_collisions(low, high, true, true,
                                                   overlapping_triangles);

  for (size_t i = 0; i < overlapping_triangles.size(); ++i) {
    const Vec3st& curr_tri = m_mesh.get_triangle(overlapping_triangles[i]);
    bool result = check_edge_triangle_intersection_by_index(
        tri[0], tri[1], curr_tri[0], curr_tri[1], curr_tri[2], get_positions(),
        false);

    if (result) {
      check_edge_triangle_intersection_by_index(tri[0], tri[1], curr_tri[0],
  ...
  for (size_t i = 0; i < overlapping_triangles.size(); ++i) {
    ...
    bool result = check_edge_triangle_intersection_by_index(
        tri[1], tri[2], curr_tri[0], curr_tri[1], curr_tri[2], get_positions(),
        false);

    if (result) {
      check_edge_triangle_intersection_by_index(tri[1], tri[2], curr_tri[0],
                                                curr_tri[1], curr_tri[2],
                                                get_positions(), true);
  ...
  for (size_t i = 0; i < overlapping_triangles.size(); ++i) {
    ...
    bool result = check_edge_triangle_intersection_by_index(
        tri[2], tri[0], curr_tri[0], curr_tri[1], curr_tri[2], get_positions(),
        false);

    if (result) {
      check_edge_triangle_intersection_by_index(tri[2], tri[0], curr_tri[0],
                                                curr_tri[1], curr_tri[2],
                                                get_positions(), true);
  ...
  minmax(get_position(tri[0]), get_position(tri[1]), get_position(tri[2]), low,
         high);
  low -= Vec3d(m_aabb_padding);
  high += Vec3d(m_aabb_padding);
  ...
  m_broad_phase->get_potential_edge_collisions(low, high, true, true,
                                               overlapping_edges);

  for (size_t i = 0; i < overlapping_edges.size(); ++i) {
    bool result = check_edge_triangle_intersection_by_index(
        m_mesh.m_edges[overlapping_edges[i]][0],
        m_mesh.m_edges[overlapping_edges[i]][1], tri[0], tri[1], tri[2],
        get_positions(), false);

    if (result) {
      check_edge_triangle_intersection_by_index(
          m_mesh.m_edges[overlapping_edges[i]][0],
          m_mesh.m_edges[overlapping_edges[i]][1], tri[0], tri[1], tri[2],
          get_positions(), true);
...
void DynamicSurface::get_intersections(
    bool degeneracy_counts_as_intersection, bool use_new_positions,
    std::vector<Intersection>& intersections) {
  ...
  for (int i = 0; i < (int)m_mesh.num_triangles(); ++i) {
    ...
    m_broad_phase->get_potential_edge_collisions(low, high, get_solid_edges,
                                                 true, edge_candidates);
    ...
    for (size_t j = 0; j < edge_candidates.size(); ++j) {
      ...
      if (edge[0] == triangle[0] || edge[0] == triangle[1] ||
          edge[0] == triangle[2] || edge[1] == triangle[0] ||
          edge[1] == triangle[1] || edge[1] == triangle[2]) {
        continue;
      }

      assert(!triangle_is_all_solid(i) ||
             !edge_is_all_solid(edge_candidates[j]));

      const Vec3d& e0 =
          use_new_positions ? get_newposition(edge[0]) : get_position(edge[0]);
      const Vec3d& e1 =
          use_new_positions ? get_newposition(edge[1]) : get_position(edge[1]);
      const Vec3d& t0 = use_new_positions ? get_newposition(triangle[0])
                                          : get_position(triangle[0]);
      const Vec3d& t1 = use_new_positions ? get_newposition(triangle[1])
                                          : get_position(triangle[1]);
      const Vec3d& t2 = use_new_positions ? get_newposition(triangle[2])
                                          : get_position(triangle[2]);

      if (segment_triangle_intersection(
              e0, edge[0], e1, edge[1], t0, triangle[0], t1, triangle[1], t2,
              triangle[2], degeneracy_counts_as_intersection, m_verbose)) {
        std::cout << "intersection: " << edge << " vs " << triangle
                  << std::endl;
...
void DynamicSurface::assert_mesh_is_intersection_free(
    bool degeneracy_counts_as_intersection) {
  std::vector<Intersection> intersections;
  get_intersections(degeneracy_counts_as_intersection, false, intersections);

  for (size_t i = 0; i < intersections.size(); ++i) {
    const Vec3st& triangle =
        m_mesh.get_triangle(intersections[i].m_triangle_index);
    const Vec2st& edge = m_mesh.m_edges[intersections[i].m_edge_index];

    std::cout << "Intersection!  Triangle " << triangle << " vs edge " << edge
              << std::endl;

    segment_triangle_intersection(
        get_position(edge[0]), edge[0], get_position(edge[1]), edge[1],
        get_position(triangle[0]), triangle[0], get_position(triangle[1]),
        triangle[1], get_position(triangle[2]), triangle[2], true, true);
...
// ---------------------------------------------------------
///
/// Using m_newpositions as the geometry, fire an assert if any edge is
/// intersecting any triangles. This is a useful debugging tool, as it will
/// detect any missed collisions before the mesh is advected into an
/// intersecting state.
///
// ---------------------------------------------------------
...
void DynamicSurface::assert_predicted_mesh_is_intersection_free(
  ...
  for (size_t i = 0; i < intersections.size(); ++i) {
    ...
    std::cout << "number of collisions detected: " << check_collisions.size()
              << std::endl;
    ...
    bool result = segment_triangle_intersection(
        get_position(edge[0]), edge[0], get_position(edge[1]), edge[1],
        get_position(triangle[0]), triangle[0], get_position(triangle[1]),
        triangle[1], get_position(triangle[2]), triangle[2],
        degeneracy_counts_as_intersection, m_verbose);

    std::cout << "result: " << result << std::endl;

    std::cout << "-----\n edge-triangle check using new m_positions"
              << std::endl;

    result = segment_triangle_intersection(
        get_newposition(edge[0]), edge[0], get_newposition(edge[1]), edge[1],
        get_newposition(triangle[0]), triangle[0], get_newposition(triangle[1]),
        triangle[1], get_newposition(triangle[2]), triangle[2],
        degeneracy_counts_as_intersection, m_verbose);
    ...
    std::cout << "Vec3d ea( " << ea[0] << ", " << ea[1] << ", " << ea[2] << ");"
              << std::endl;
    std::cout << "Vec3d eb( " << eb[0] << ", " << eb[1] << ", " << eb[2] << ");"
              << std::endl;
    std::cout << "Vec3d ta( " << ta[0] << ", " << ta[1] << ", " << ta[2] << ");"
              << std::endl;
    std::cout << "Vec3d tb( " << tb[0] << ", " << tb[1] << ", " << tb[2] << ");"
              << std::endl;
    std::cout << "Vec3d tc( " << tc[0] << ", " << tc[1] << ", " << tc[2] << ");"
              << std::endl;

    std::cout << "Vec3d ea_new( " << ea_new[0] << ", " << ea_new[1] << ", "
              << ea_new[2] << ");" << std::endl;
    std::cout << "Vec3d eb_new( " << eb_new[0] << ", " << eb_new[1] << ", "
              << eb_new[2] << ");" << std::endl;
    std::cout << "Vec3d ta_new( " << ta_new[0] << ", " << ta_new[1] << ", "
              << ta_new[2] << ");" << std::endl;
    std::cout << "Vec3d tb_new( " << tb_new[0] << ", " << tb_new[1] << ", "
              << tb_new[2] << ");" << std::endl;
    std::cout << "Vec3d tc_new( " << tc_new[0] << ", " << tc_new[1] << ", "
              << tc_new[2] << ");" << std::endl;
    ...
    assert(!segment_segment_collision(ea, ea_new, edge[0], eb, eb_new, edge[1],
                                      ta, ta_new, triangle[0], tb, tb_new,
                                      triangle[1]));

    std::cout << "-----" << std::endl;

    assert(!segment_segment_collision(ea, ea_new, edge[0], eb, eb_new, edge[1],
                                      tb, tb_new, triangle[1], tc, tc_new,
                                      triangle[2]));

    std::cout << "-----" << std::endl;

    assert(!segment_segment_collision(ea, ea_new, edge[0], eb, eb_new, edge[1],
                                      ta, ta_new, triangle[0], tc, tc_new,
                                      triangle[2]));

    std::cout << "-----" << std::endl;

    assert(!point_triangle_collision(ea, ea_new, edge[0], ta, ta_new,
                                     triangle[0], tb, tb_new, triangle[1], tc,
                                     tc_new, triangle[2]));

    std::cout << "-----" << std::endl;

    assert(!point_triangle_collision(eb, eb_new, edge[1], ta, ta_new,
                                     triangle[0], tb, tb_new, triangle[1], tc,
                                     tc_new, triangle[2]));
