...
// ---------------------------------------------------------
///
/// Compute the maximum timestep that will not invert any triangle normals,
/// using a quadratic solve as in [Jiao 2007].
///
// ---------------------------------------------------------

namespace LosTopos {

double MeshSmoother::compute_max_timestep_quadratic_solve(
    const std::vector<Vec3st>& tris, const std::vector<Vec3d>& positions,
    const std::vector<Vec3d>& displacements, bool verbose) {
  double max_beta = 1.0;
  ...
  for (size_t i = 0; i < tris.size(); ++i) {
    ...
    if (c < 1e-14) {
      if (verbose) {
        std::cout << "super small triangle " << i << " (" << tris[i] << ")"
                  << std::endl;
      }
    }
    ...
    if (std::fabs(a) == 0) {
      ...
      if (descriminant < 0.0) {
        ...
        if (verbose) {
          std::cout << "triangle " << i << ": discriminant == " << descriminant
                    << std::endl;
        }
        ...
        if (beta_1 < 0.0) {
          if (beta_2 < 0.0) {
            assert(dot(triangle_normal(x1, x2, x3),
                       triangle_normal(new_x1, new_x2, new_x3)) > 0.0);
          } else {
            beta = beta_2;
          }
        } else {
    ...
    if (dot(old_normal, new_normal) < 0.0) {
      ...
      std::cout << "dot product: "
                << dot(triangle_normal(x1, x2, x3),
                       triangle_normal(new_x1, new_x2, new_x3))
                << std::endl;
...
void MeshSmoother::null_space_smooth_vertex(
    size_t v, const std::vector<double>& triangle_areas,
    const std::vector<Vec3d>& triangle_normals,
    const std::vector<Vec3d>& triangle_centroids, Vec3d& displacement) const {
  ...
  for (size_t j = 0; j < edges.size(); ++j) {
    if (mesh.m_edge_to_triangle_map[edges[j]].size() ==
        1)  // boundary edge //TODO Handle boundary edges more wisely. (Treat as
            // ridge).
    {
      displacement = Vec3d(0, 0, 0);
      return;
  ...
  const std::vector<size_t>& incident_triangles =
      mesh.m_vertex_to_triangle_map[v];

  // if we're being aggressive, instead do naive Laplacian smoothing of the
  // vertex and then return.

  if (m_surf.m_aggressive_mode) {
    displacement =
        get_smoothing_displacement_naive(v, incident_triangles, triangle_areas,
                                         triangle_normals, triangle_centroids);
    return;
  }

  // identify vertices that are folded to be near-coplanar. (i.e. fail to be
  // identified by Jiao's quadric)
  ...
  for (size_t i = 0; i < mesh.m_vertex_to_edge_map[v].size(); ++i) {
    ...
    if (M_PI - angle <
        m_sharp_fold_regularization_threshold) {  // dihedral angle 170 degrees
                                                  // or more, i.e. two planes
                                                  // intersect at 10 degrees or
                                                  // less. consider it a "fold"
      regularize_folded_feature = true;
    }
  }

  if (regularize_folded_feature) {
    // Regularize these very sharp features. These situations typically indicate
    // merging or "fold-overs" so we try to encourage nicer merging by smoothing
    // them in such a way that the sharp angle becomes less sharp.

    // Figure out which volumetric region is the sharp one.

    // Collect all the regions
    std::set<int> incident_regions;
    for (size_t i = 0; i < m_surf.m_mesh.m_vertex_to_triangle_map[v].size();
         ++i) {
    ...
    for (size_t i = 0; i < m_surf.m_mesh.m_vertex_to_edge_map[v].size(); ++i) {
      ...
      // let's only consider 3-way junctions, since 4-ways are more complex and
      // unstable anyway
      if (m_surf.m_mesh.m_edge_to_triangle_map[edge].size() > 3) continue;

      // for each region...
      for (std::set<int>::iterator it = incident_regions.begin();
           it != incident_regions.end(); ++it) {
        int region = *it;
        Vec3d normal_pair[2];
        ...
        // there should only be two triangles on this edge bordering the same
        // region, given that we consider edges with 3 or fewer tris.
        int next_tri_ind = 0;
        for (size_t j = 0;
             j < m_surf.m_mesh.m_edge_to_triangle_map[edge].size(); ++j) {
    ...
    if (sharpest_region != -1 &&
        M_PI - sharp_angle < m_sharp_fold_regularization_threshold) {
      // choose only that specific region/surface to smooth in the non-manifold
      // case.
      ...
      // This is useful in this case because quadric-based null-space smoothing
      // identifies the tangent plane of the highly folded-triangles, and smooths
      // only in that plane. The result is that the vertices inside the sharp
      // fold are pulled outward, slightly opens up the angle and regularizes the
      // merge curve.
      displacement = get_smoothing_displacement(
          v, tri_set, triangle_areas, triangle_normals, triangle_centroids);
    } else {
      displacement = get_smoothing_displacement_dihedral(
          v, incident_triangles, triangle_areas, triangle_normals,
          triangle_centroids);
    }
...
Vec3d MeshSmoother::get_smoothing_displacement(
    size_t v, const std::vector<size_t>& triangles,
...
Vec3d MeshSmoother::get_smoothing_displacement_dihedral(
    size_t v, const std::vector<size_t>& triangles,
    const std::vector<double>& triangle_areas,
    const std::vector<Vec3d>& triangle_normals,
    const std::vector<Vec3d>& triangle_centroids) const {
  int feature_edge_count =
      m_surf.vertex_feature_edge_count(v, triangle_normals);

  // Corner, don't smooth it at all.
  if (feature_edge_count >= 3) return Vec3d(0, 0, 0);
  ...
  if (feature_edge_count == 0) {
    ...
    // vertical (normal) component

    // Determine the normal per Jiao's approach
    //->using the medial quadric (see Identification of C1 and C2
    //Discontinuities for Surface Meshes in CAD, equation 2)

    // TODO Try other, less expensive normals to see if we can bring down the
    // cost without sacrificing quality.
    //     Alternately, is there an equivalent normal to Jiao's that doesn't
    //     require an eigendecomposition?

    // TODO Apply smoothing selectively only if the geometry is worse than some
    // threshold? Likewise
    //     can we adjust the collision detection/response used here to be
    //     localized?
    ...
    // use only the eigenvector associated with the smallest eigenvalue, since
    // we're assuming we are on a ridge, so there is one degree of freedom

    // Check that the ridge direction is well-conditioned before using it, per
    // Jiao "Identification of C1 and C2 Discontinuities for Surface Meshes in
    //CAD" Eigenvalues are sorted in ascending order. (Jiao numbers them in the
    // opposite order from us.)
    const double epsilon_thresh = sqr(tan(5 * M_PI / 180));
    if (eigenvalues[0] / eigenvalues[1] <= 0.7 &&
        eigenvalues[1] / eigenvalues[2] >= 0.00765) {
      T.push_back(Vec3d(A(0, 0), A(1, 0), A(2, 0)));
    } else {
      // Try to concoct a reasonable alternative ridge/edge vector, when the
      // quadric-based vector is ill-conditioned (e.g. surface seems flat, or
      // actually sharply folded)
      if (feature_edge_count == 1) {
        // One feature edge; use its vector as the edge direction.
        for (size_t i = 0; i < m_surf.m_mesh.m_vertex_to_edge_map[v].size();
             ++i) {
          size_t edge = m_surf.m_mesh.m_vertex_to_edge_map[v][i];
          if (m_surf.edge_is_feature(edge)) {
            Vec3d edgeVec =
                m_surf.get_position(m_surf.m_mesh.m_edges[edge][0]) -
                m_surf.get_position(m_surf.m_mesh.m_edges[edge][1]);
            normalize(edgeVec);
            T.push_back(edgeVec);
          }
        }
        ...
        for (size_t i = 0; i < m_surf.m_mesh.m_vertex_to_edge_map[v].size();
          ...
          if (m_surf.edge_is_feature(edge)) {
            Vec3d midpoint =
                0.5 * (m_surf.get_position(m_surf.m_mesh.m_edges[edge][0]) +
                       m_surf.get_position(m_surf.m_mesh.m_edges[edge][1]));
...
Vec3d MeshSmoother::get_smoothing_displacement_naive(
    size_t v, const std::vector<size_t>& triangles,
    const std::vector<double>& triangle_areas,
    const std::vector<Vec3d>& triangle_normals,
    const std::vector<Vec3d>& triangle_centroids) const {
...
bool MeshSmoother::null_space_smoothing_pass(double dt) {
  ...
  if (m_surf.m_verbose) {
    std::cout << "---------------------- Los Topos: vertex redistribution "
                 "----------------------"
              << std::endl;
  }
  ...
  for (size_t i = 0; i < m_surf.m_mesh.num_triangles(); ++i) {
    ...
    if (tri[0] == tri[1]) {
      ...
      triangle_centroids.push_back((m_surf.get_position(tri[0]) +
  ...
  if (!m_surf.m_aggressive_mode) {
    ...
    for (int i = 0; i < (int)m_surf.get_num_vertices(); ++i) {
      if (!m_surf.vertex_is_all_solid(i)) {
        null_space_smooth_vertex(i, triangle_areas, triangle_normals,
                                 triangle_centroids, displacements[i]);
        max_displacement = max(max_displacement, mag(displacements[i]));
      }
    }
  } else {
    // in aggressive mode, identify only the triangles with bad angles, and
    // smooth all of their vertices (with naive Laplacian smoothing)
    ...
    for (size_t i = 0; i < m_surf.m_mesh.num_triangles(); ++i) {
      ...
      if (angles[0] < m_surf.m_min_triangle_angle ||
          angles[0] > m_surf.m_max_triangle_angle ||
          angles[1] < m_surf.m_min_triangle_angle ||
          angles[1] > m_surf.m_max_triangle_angle ||
          angles[2] < m_surf.m_min_triangle_angle ||
          angles[2] > m_surf.m_max_triangle_angle) {
        // std::cout << "Bad triangle angles: " << angles << std::endl;
        for (int j = 0; j < 3; ++j) {
          size_t v = tri[j];
          if (!m_surf.vertex_is_all_solid(v) && !smoothed_already[v]) {
            null_space_smooth_vertex(v, triangle_areas, triangle_normals,
                                     triangle_centroids, displacements[v]);
  ...
  // compute maximum dt
  double max_beta = 1.0;  // compute_max_timestep_quadratic_solve(
                          // m_surf.m_mesh.get_triangles(), m_surf.m_positions,
                          // displacements, m_surf.m_verbose );
  ...
  for (size_t i = 0; i < m_surf.get_num_vertices(); ++i) {
    ...
    m_surf.m_velocities[i] =
        (m_surf.get_newposition(i) - m_surf.get_position(i)) / dt;
  }

  // repositioned locations stored in m_newpositions, but needs to be collision
  // safe
  if (m_surf.m_collision_safety) {
    bool all_collisions_handled =
        m_surf.m_collision_pipeline->handle_collisions(dt);
    ...
    if (!all_collisions_handled) {
      ...
      if (!result) {
        ...
        std::cerr
            << "WARNING: Aborting mesh null-space smoothing due to CCD problem"
            << std::endl;
        return true;
      }
    }
  ...
  for (size_t i = 0; i < m_surf.get_num_vertices(); i++) {
    max_position_change =
        max(max_position_change,
            mag(m_surf.get_newposition(i) - m_surf.get_position(i)));
  }

  m_surf.set_positions_to_newpositions();

  if (m_surf.m_verbose) {
    std::cout << "max_position_change: " << max_position_change << std::endl;
  }

  // We will test convergence by checking whether the largest change in
  // position has magnitude less than: CONVERGENCE_TOL_SCALAR *
  // average_edge_length
  const static double CONVERGENCE_TOL_SCALAR = 1.0;
  bool converged = false;
  if (max_position_change <
      CONVERGENCE_TOL_SCALAR * m_surf.get_average_edge_length()) {
    converged = true;
  }
