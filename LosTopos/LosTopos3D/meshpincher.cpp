...
//  triangle neighbourhood, and splits the mesh surface at these vertices. This
//  also works for multiphase too, subject to label copying.
//
// ---------------------------------------------------------
...
void MeshPincher::partition_vertex_neighbourhood(
    size_t vertex_index, std::vector<TriangleSet>& connected_components) {
  // triangles incident to vertex
  TriangleSet triangles_incident_to_vertex =
      m_surf.m_mesh.m_vertex_to_triangle_map[vertex_index];
  ...
  while (triangles_incident_to_vertex.size() > 0) {
    ...
    while (unvisited_triangles.size() > 0) {
      ...
      triangles_incident_to_vertex.erase(
          find(triangles_incident_to_vertex.begin(),
               triangles_incident_to_vertex.end(), curr_tri));

      // put it on closed
      visited_triangles.push_back(curr_tri);

      // get find a triangle which is incident to vertex and adjacent to
      // curr_tri
...
bool MeshPincher::pull_apart_vertex(
    size_t vertex_index, const std::vector<TriangleSet>& connected_components) {
  ...
  for (int i = 0; i < (int)connected_components.size() - 1; ++i) {
    ...
    size_t duplicate_vertex_index = m_surf.add_vertex(
        m_surf.get_position(vertex_index), m_surf.m_masses[vertex_index]);
    ...
    for (size_t t = 0; t < connected_components[i].size(); ++t) {
      // create a new triangle with 2 vertices the same, and one vertex set to
      ...
      triangle_labels_to_add.push_back(
          m_surf.m_mesh.get_triangle_label(old_tri_ind));
    ...
    Vec3d added_vertex_position =
        (1.0 - dx) * m_surf.get_position(duplicate_vertex_index) +
        dx * centroid;
  ...
  // all new triangles check out okay for collision safety.  Add them to the
  // data structure.

  for (size_t i = 0; i < triangles_to_add.size(); ++i) {
    size_t new_tri_ind =
        m_surf.add_triangle(triangles_to_add[i], triangle_labels_to_add[i]);
  }
