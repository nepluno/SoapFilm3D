...
//
//  The SurfTrack class: a dynamic mesh with topological changes and mesh
...
// ---------------------------------------------------------
///
/// Structure for setting up a SurfTrack object with some initial parameters.
/// This is passed to the SurfTrack constructor.
///
// ---------------------------------------------------------

struct SurfTrackInitializationParameters {
  ///  Constructor. Sets default values for parameters which are not likely to
  ///  be specified.
  ///
  ...
  /// Collision epsilon to use during mesh improvement operations (i.e. if any
  /// mesh elements are closer than this, the operation is aborted).  NOTE: This
  /// should be greater than collision_epsilon, to prevent improvement
  /// operations from moving elements into a collision configuration.
  ///
  double m_improve_collision_epsilon;

  /// Whether to set the min and max edge lengths as fractions of the initial
  /// average edge length
  ///
  bool m_use_fraction;

  // If use_fraction is true, the following three values are taken to be
  // fractions of the average edge length of the new surface. If use_fraction is
  // false, these are absolute.
  ...
  /// Largest interior angle at a triangle vertex allowed before large-angle
  /// split pass kicks in
  ///
  ...
  /// Whether to scale by curvature when computing edge lengths, in order to
  /// refine high-curvature regions
  ///
  bool m_use_curvature_when_splitting;

  /// Whether to scale by curvature when computing edge lengths, in order to
  /// coarsen low-curvature regions
  ///
  ...
  /// Type of subdivision to use when collapsing or splitting (butterfly,
  /// quadric error minimization, etc.)
  ///
  SubdivisionScheme* m_subdivision_scheme;

  /// Whether to enforce collision-free surfaces (including during mesh
  /// maintenance operations)
  ///
  ...
  /// Whether to perform remeshing on mesh boundary edges (in the case of open
  /// surfaces, e.g. sheets)
  ///
...
struct SortableEdge {
  ...
  SortableEdge(size_t i, double edge_len)
      : m_edge_index(i), m_length(edge_len) {}
...
struct SortablePair {
  ...
  SortablePair(size_t i, size_t j, double sep_dist)
      : m_vertex0(i), m_vertex1(j), m_length(sep_dist) {}
...
struct SortableProximity {
  /// Constructor
  ///
  SortableProximity(size_t face, size_t vertex, double sep_dist,
                    bool isFaceVert)
      : m_index0(face),
        m_index1(vertex),
...
// ---------------------------------------------------------
///
/// Keeps track of a vertex removal or addition.  If it's an addition, it also
/// points to the edge that was split to create it.
///
// ---------------------------------------------------------
...
struct VertexUpdateEvent {
  /// Constructor
  ///
  VertexUpdateEvent(bool is_remove = false, size_t vertex_index = (size_t)~0,
  ...
  /// If this is a vertex addition due to edge splitting, the edge that was
  /// split
  ///
  Vec2st m_split_edge;
};
...
// ---------------------------------------------------------
///
/// Keeps track of a triangle removal or addition. If addition, contains the
/// three vertices that form the new triangle.
///
// ---------------------------------------------------------
...
struct MeshUpdateEvent {
  ...
  /// Constructors
  ///
  MeshUpdateEvent(EventType eType)
      : m_type(eType),
        m_deleted_tris(0),
        m_created_tris(0),
        m_created_tri_data(0) {}
  ...
  /// The location of the final vertex (for a split, collapse, or possibly
  /// smooth)
  ///
...
class SurfTrack : public DynamicSurface {
 public:
  ...
  ///
  SurfTrack(const std::vector<Vec3d>& vs, const std::vector<Vec3st>& ts,
 ...
 public:
  ...
  /// Efficiently renumber a triangle (replace its verts) for defragging
  /// purposes.
  ///
  ...
  ///
  size_t add_vertex(const Vec3d& new_vertex_position,
  ...
  void defrag_mesh_from_scratch_manual(
      std::vector<size_t>& vertices_to_be_mapped);
  void defrag_mesh_from_scratch_copy(
      std::vector<size_t>& vertices_to_be_mapped);
  ...
  /// Check for and delete flaps and zero-area triangles among the given
  /// triangle indices, then separate singular vertices.
  ///
  void trim_degeneracies(std::vector<size_t>& triangle_indices);

  /// Check for and delete flaps and zero-area triangles among *all* triangles,
  /// then separate singular vertices.
  ///
  ...
  /// Detect any bad angle, i.e. out of range [m_min_triangle_angle,
  ...
  /// Don't create triangles with angles less than this.  If angles less than
  /// this do exist, try to remove them.
  ///
  double m_min_triangle_angle;

  /// Don't create triangles with angles greater than this.  If angles greater
  /// than this do exist, try to remove them.
  ///
  double m_max_triangle_angle;
  ...
  /// If we allocate our own SubdivisionScheme object, we must delete it in this
  /// object's deconstructor.
  ///
  bool should_delete_subdivision_scheme_object;

  /// Triangles which are involved in connectivity changes which may introduce
  /// degeneracies
  ///
  ...
  /// Flag that dictates whether to aggressively pursue good angles for only the
  /// worst offenders.
  ///
  bool m_aggressive_mode;

  /// boolean, whether to allow vertices to move during collapses (i.e. use
  /// points other than the endpoints)
  int m_allow_vertex_movement_during_collapse;

  /// boolean, whether to do null space smoothing on vertex positions
  int m_perform_smoothing;

  // Return whether the given edge is a feature as determined by dihedral
  // angles.
  bool edge_is_feature(size_t edge) const;
  bool edge_is_feature(size_t edge,
  ...
  int vertex_feature_edge_count(size_t vertex,
                                const std::vector<Vec3d>& cached_normals) const;

  // Return whether the incident feature curves on a vertex form a smooth ridge
  // (implying # of feature curves = 2)
  ...
  class MeshEventCallback {
   public:
    ...
    virtual void post_collapse(const SurfTrack& st, size_t e,
                               size_t merged_vertex, void* data) {}

    virtual void pre_split(const SurfTrack& st, size_t e, void** data) {}
    virtual void post_split(const SurfTrack& st, size_t e, size_t new_vertex,
                            void* data) {}
    ...
    virtual void post_t1(const SurfTrack& st, size_t v, size_t a, size_t b,
                         void* data) {}

    virtual void pre_facesplit(const SurfTrack& st, size_t f, void** data) {}
    virtual void post_facesplit(const SurfTrack& st, size_t f,
                                size_t new_vertex, void* data) {}

    virtual void pre_snap(const SurfTrack& st, size_t v0, size_t v1,
                          void** data) {}
    virtual void post_snap(const SurfTrack& st, size_t v_kept, size_t v_deleted,
                           void* data) {}
  ...
  class SolidVerticesCallback {
   public:
    virtual bool generate_collapsed_position(SurfTrack& st, size_t v0,
                                             size_t v1, Vec3d& pos) = 0;

    virtual bool generate_split_position(SurfTrack& st, size_t v0, size_t v1,
                                         Vec3d& pos) = 0;

    virtual Vec3c generate_collapsed_solid_label(SurfTrack& st, size_t v0,
                                                 size_t v1, const Vec3c& label0,
                                                 const Vec3c& label1) = 0;

    virtual Vec3c generate_split_solid_label(SurfTrack& st, size_t v0,
                                             size_t v1, const Vec3c& label0,
                                             const Vec3c& label1) = 0;

    virtual bool generate_edge_popped_positions(SurfTrack& st, size_t oldv,
                                                const Vec2i& cut,
                                                Vec3d& pos_upper,
                                                Vec3d& pos_lower) = 0;

    virtual bool generate_vertex_popped_positions(SurfTrack& st, size_t oldv,
                                                  int A, int B, Vec3d& pos_a,
                                                  Vec3d& pos_b) = 0;
  ...
  /// Map of triangle indices, mapping pre-defrag triangle indices to
  /// post-defrag indices (deprecated; see defrag_mesh())
  ///
  std::vector<Vec2st> m_defragged_triangle_map;

  /// Map of vertex indices, mapping pre-defrag vertex indices to post-defrag
  /// indices (deprecated; see defrag_mesh())
  ///
