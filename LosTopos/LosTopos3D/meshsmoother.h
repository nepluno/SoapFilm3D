...
// ---------------------------------------------------------
///
/// Mesh smoother object.  Performs NULL-space constrained Laplacian smoothing
/// of mesh vertices.
///
// ---------------------------------------------------------
...
class MeshSmoother {
 public:
  ...
  MeshSmoother(SurfTrack& surf)
      : m_surf(surf), m_sharp_fold_regularization_threshold(15 * M_PI / 180) {}
  ...
  ///
  static double compute_max_timestep_quadratic_solve(
 ...
 private:
  ...
  Vec3d get_smoothing_displacement(
  ...
  Vec3d get_smoothing_displacement_dihedral(
      size_t v, const std::vector<size_t>& triangles,
      const std::vector<double>& triangle_areas,
      const std::vector<Vec3d>& triangle_normals,
      const std::vector<Vec3d>& triangle_centroids) const;

  // this version does vanilla area-weighted smoothing, rather than
  // tangential/null-space smoothing
  Vec3d get_smoothing_displacement_naive(
      size_t v, const std::vector<size_t>& triangles,
      const std::vector<double>& triangle_areas,
      const std::vector<Vec3d>& triangle_normals,
      const std::vector<Vec3d>& triangle_centroids) const;
