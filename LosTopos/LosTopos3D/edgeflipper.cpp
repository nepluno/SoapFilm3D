...
bool EdgeFlipper::flip_introduces_collision(size_t edge_index,
  ...
  size_t tet_vertex_indices[4] = {old_edge[0], old_edge[1], new_edge[0],
                                  new_edge[1]};

  const Vec3d tet_vertex_positions[4] = {
      xs[tet_vertex_indices[0]], xs[tet_vertex_indices[1]],
      xs[tet_vertex_indices[2]], xs[tet_vertex_indices[3]]};

  Vec3d low, high;
  minmax(tet_vertex_positions[0], tet_vertex_positions[1],
         tet_vertex_positions[2], tet_vertex_positions[3], low, high);

  std::vector<size_t> overlapping_vertices;
  m_surf.m_broad_phase->get_potential_vertex_collisions(low, high, true, true,
                                                        overlapping_vertices);

  // do point-in-tet tests
  for (size_t i = 0; i < overlapping_vertices.size(); ++i) {
    if ((overlapping_vertices[i] == old_edge[0]) ||
        (overlapping_vertices[i] == old_edge[1]) ||
        (overlapping_vertices[i] == new_edge[0]) ||
        (overlapping_vertices[i] == new_edge[1])) {
      continue;
    }

    if (point_tetrahedron_intersection(
            xs[overlapping_vertices[i]], overlapping_vertices[i],
  ...
  minmax(xs[new_triangle_a[0]], xs[new_triangle_a[1]], xs[new_triangle_a[2]],
         low, high);
  ...
  for (size_t i = 0; i < overlapping_edges.size(); ++i) {
    ...
    if (check_edge_triangle_intersection_by_index(
            edge[0], edge[1], new_triangle_a[0], new_triangle_a[1],
            new_triangle_a[2], xs, m_surf.m_verbose)) {
      return true;
    }
  }
  ...
  minmax(xs[new_triangle_b[0]], xs[new_triangle_b[1]], xs[new_triangle_b[2]],
         low, high);
  ...
  for (size_t i = 0; i < overlapping_edges.size(); ++i) {
    ...
    if (check_edge_triangle_intersection_by_index(
            edge[0], edge[1], new_triangle_b[0], new_triangle_b[1],
            new_triangle_b[2], xs, m_surf.m_verbose)) {
      return true;
    }
  }
...
bool EdgeFlipper::flip_edge(size_t edge, size_t tri0, size_t tri1,
  ...
  double vol =
      std::fabs(signed_volume(xs[edge_vertices[0]], xs[edge_vertices[1]],
                              xs[new_edge[0]], xs[new_edge[1]]));

  if (vol > m_surf.m_max_volume_change) {
    g_stats.add_to_int("EdgeFlipper:edge_flip_volume_change", 1);
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: volume change = " << vol << std::endl;
    }
    return false;
  }
  ...
  if (false == m_surf.m_allow_non_manifold) {
    for (size_t i = 0; i < m_mesh.m_vertex_to_edge_map[third_vertex_0].size();
         ++i) {
      if ((m_mesh.m_edges[m_mesh.m_vertex_to_edge_map[third_vertex_0][i]][0] ==
           third_vertex_1) ||
          (m_mesh.m_edges[m_mesh.m_vertex_to_edge_map[third_vertex_0][i]][1] ==
           third_vertex_1)) {
        // edge already exists
        if (m_surf.m_verbose) {
          std::cout << "edge flip rejected: edge exists" << std::endl;
        }
  ...
  if (third_vertex_0 == third_vertex_1) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: degenerate tet" << std::endl;
    }
  ...
  // new edge winding order == winding order of old triangle0 == winding order
  // of new triangle0 then we make the second new triangle match that, and set
  // labels appropriately.

  size_t new_triangle_third_vertex_0, new_triangle_third_vertex_1;
  if (m_mesh.oriented(m_mesh.m_edges[edge][0], m_mesh.m_edges[edge][1],
                      m_mesh.get_triangle(tri0))) {
  ...
  // if both triangle normals agree before flipping, make sure they agree after
  // flipping
  if (!m_surf.m_aggressive_mode) {
    if (dot(m_surf.get_triangle_normal(tri0),
            m_surf.get_triangle_normal(tri1)) > 0.0 &&
        m_mesh.oriented(m_mesh.m_edges[edge][0], m_mesh.m_edges[edge][1],
                        m_mesh.get_triangle(tri0)) ==
            m_mesh.oriented(m_mesh.m_edges[edge][1], m_mesh.m_edges[edge][0],
                            m_mesh.get_triangle(tri1))) {
      if (dot(m_surf.get_triangle_normal(new_triangle0),
              m_surf.get_triangle_normal(new_triangle1)) < 0.0) {
        if (m_surf.m_verbose) {
          std::cout << "edge flip rejected: normal inversion" << std::endl;
        }
      ...
      if (dot(m_surf.get_triangle_normal(new_triangle0),
              m_surf.get_triangle_normal(tri0)) < 0.0) {
        if (m_surf.m_verbose) {
          std::cout << "edge flip rejected: normal inversion" << std::endl;
        }
        g_stats.add_to_int("EdgeFlipper:edge_flip_normal_inversion", 1);
      ...
      if (dot(m_surf.get_triangle_normal(new_triangle1),
              m_surf.get_triangle_normal(tri1)) < 0.0) {
        if (m_surf.m_verbose) {
          std::cout << "edge flip rejected: normal inversion" << std::endl;
        }
    ...
    } else if (dot(m_surf.get_triangle_normal(tri0),
                   m_surf.get_triangle_normal(tri1)) < 0.0 &&
               m_mesh.oriented(m_mesh.m_edges[edge][0], m_mesh.m_edges[edge][1],
                               m_mesh.get_triangle(tri0)) !=
                   m_mesh.oriented(m_mesh.m_edges[edge][1],
                                   m_mesh.m_edges[edge][0],
                                   m_mesh.get_triangle(tri1))) {
      if (dot(m_surf.get_triangle_normal(new_triangle0),
              m_surf.get_triangle_normal(new_triangle1)) < 0.0) {
        if (m_surf.m_verbose) {
  ...
  if (m_surf.m_collision_safety &&
      flip_introduces_collision(edge, new_edge, new_triangle0, new_triangle1)) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: intersection" << std::endl;
    }
  ...
  Vec3d old_normal0 = cross(xs[old_tri0[1]] - xs[old_tri0[0]],
                            xs[old_tri0[2]] - xs[old_tri0[0]]);
  Vec3d old_normal1 = cross(xs[old_tri1[1]] - xs[old_tri1[0]],
                            xs[old_tri1[2]] - xs[old_tri1[0]]);
  double old_area0 = mag(old_normal0) / 2;
  double old_area1 = mag(old_normal1) / 2;
  normalize(old_normal0);
  normalize(old_normal1);
  Vec3d new_normal0 = cross(xs[new_triangle0[1]] - xs[new_triangle0[0]],
                            xs[new_triangle0[2]] - xs[new_triangle0[0]]);
  Vec3d new_normal1 = cross(xs[new_triangle1[1]] - xs[new_triangle1[0]],
                            xs[new_triangle1[2]] - xs[new_triangle1[0]]);
  double new_area0 = mag(new_normal0) / 2;
  double new_area1 = mag(new_normal1) / 2;
  normalize(new_normal0);
  normalize(new_normal1);

  if (new_area0 < std::min(m_surf.m_min_triangle_area,
                           std::min(old_area0, old_area0) * 0.5)) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: area0 too small" << std::endl;
    }
    g_stats.add_to_int("EdgeFlipper:edge_flip_new_area_too_small", 1);
    return false;
  }

  if (new_area1 < std::min(m_surf.m_min_triangle_area,
                           std::min(old_area0, old_area0) * 0.5)) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: area1 too small" << std::endl;
    }
    g_stats.add_to_int("EdgeFlipper:edge_flip_new_area_too_small", 1);
    return false;
  }
  ...
  if (std::fabs(old_area0 + old_area1 - new_area0 - new_area1) >
      0.1 * (old_area0 + old_area1)) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: area change too great" << std::endl;
    }
  ...
  if (m_surf.edge_is_feature(edge)) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: edge is a feature" << std::endl;
    }
    g_stats.add_to_int("EdgeFlipper:edge_flip_not_smooth", 1);
    return false;
  }

  // Don't flip if it produces triangles with bad aspect ratio (regardless of
  // area)
  double old_min_edge_0 =
      std::min(std::min(mag(xs[old_tri0[1]] - xs[old_tri0[0]]),
                        mag(xs[old_tri0[2]] - xs[old_tri0[1]])),
               mag(xs[old_tri0[0]] - xs[old_tri0[2]]));
  double old_min_edge_1 =
      std::min(std::min(mag(xs[old_tri1[1]] - xs[old_tri1[0]]),
                        mag(xs[old_tri1[2]] - xs[old_tri1[1]])),
               mag(xs[old_tri1[0]] - xs[old_tri1[2]]));
  double old_max_edge_0 =
      std::max(std::max(mag(xs[old_tri0[1]] - xs[old_tri0[0]]),
                        mag(xs[old_tri0[2]] - xs[old_tri0[1]])),
               mag(xs[old_tri0[0]] - xs[old_tri0[2]]));
  double old_max_edge_1 =
      std::max(std::max(mag(xs[old_tri1[1]] - xs[old_tri1[0]]),
                        mag(xs[old_tri1[2]] - xs[old_tri1[1]])),
               mag(xs[old_tri1[0]] - xs[old_tri1[2]]));
  double new_min_edge_0 =
      std::min(std::min(mag(xs[new_triangle0[1]] - xs[new_triangle0[0]]),
                        mag(xs[new_triangle0[2]] - xs[new_triangle0[1]])),
               mag(xs[new_triangle0[0]] - xs[new_triangle0[2]]));
  double new_min_edge_1 =
      std::min(std::min(mag(xs[new_triangle1[1]] - xs[new_triangle1[0]]),
                        mag(xs[new_triangle1[2]] - xs[new_triangle1[1]])),
               mag(xs[new_triangle1[0]] - xs[new_triangle1[2]]));
  double new_max_edge_0 =
      std::max(std::max(mag(xs[new_triangle0[1]] - xs[new_triangle0[0]]),
                        mag(xs[new_triangle0[2]] - xs[new_triangle0[1]])),
               mag(xs[new_triangle0[0]] - xs[new_triangle0[2]]));
  double new_max_edge_1 =
      std::max(std::max(mag(xs[new_triangle1[1]] - xs[new_triangle1[0]]),
                        mag(xs[new_triangle1[2]] - xs[new_triangle1[1]])),
               mag(xs[new_triangle1[0]] - xs[new_triangle1[2]]));
  double AR_THRESHOLD = 10.0;
  double arthreshold = AR_THRESHOLD;
  arthreshold = std::max(
      arthreshold, 1 / (old_area0 * 2 / (old_max_edge_0 * old_max_edge_0)));
  arthreshold = std::max(
      arthreshold, 1 / (old_area1 * 2 / (old_max_edge_1 * old_max_edge_1)));
  arthreshold =
      std::max(arthreshold, old_area0 * 2 / (old_min_edge_0 * old_min_edge_0));
  arthreshold =
      std::max(arthreshold, old_area1 * 2 / (old_min_edge_1 * old_min_edge_1));
  if ((new_area0 * 2 / (new_max_edge_0 * new_max_edge_0) < 1 / arthreshold ||
       new_area0 * 2 / (new_min_edge_0 * new_min_edge_0) > arthreshold) ||
      (new_area1 * 2 / (new_max_edge_1 * new_max_edge_1) < 1 / arthreshold ||
       new_area1 * 2 / (new_min_edge_1 * new_min_edge_1) > arthreshold)) {
    if (m_surf.m_verbose) {
      std::cout << "edge flip rejected: flip will produce triangles with bad "
                   "aspect ratio"
                << std::endl;
    }
  ...
  double min_angle = min_triangle_angle(
      xs[new_triangle0[0]], xs[new_triangle0[1]], xs[new_triangle0[2]]);
  min_angle = min(min_angle,
                  min_triangle_angle(xs[new_triangle1[0]], xs[new_triangle1[1]],
                                     xs[new_triangle1[2]]));
  ...
  double max_angle = max_triangle_angle(
      xs[new_triangle0[0]], xs[new_triangle0[1]], xs[new_triangle0[2]]);
  max_angle = max(max_angle,
                  max_triangle_angle(xs[new_triangle1[0]], xs[new_triangle1[1]],
                                     xs[new_triangle1[2]]));
  ...
  if (m_surf.m_collision_safety) {
    if (m_surf.check_triangle_vs_all_triangles_for_intersection(
            new_triangle_index_0)) {
      std::cout << "missed an intersection.  New triangles: " << new_triangle0
                << ", " << new_triangle1 << std::endl;
      std::cout << "old triangles: " << old_tri0 << ", " << old_tri1
                << std::endl;
      assert(0);
    }

    if (m_surf.check_triangle_vs_all_triangles_for_intersection(
            new_triangle_index_1)) {
      std::cout << "missed an intersection.  New triangles: " << new_triangle0
                << ", " << new_triangle1 << std::endl;
      std::cout << "old triangles: " << old_tri0 << ", " << old_tri1
                << std::endl;
  ...
  if (m_surf.m_verbose) {
    std::cout << "edge flip: ok" << std::endl;
  }
...
void EdgeFlipper::getQuadric(size_t vertex, Mat33d& A) {
  ...
  const std::vector<size_t>& incident_triangles =
      mesh.m_vertex_to_triangle_map[vertex];
...
bool EdgeFlipper::is_Delaunay_anisotropic(size_t edge, size_t tri0, size_t tri1,
  ...
  Mat22d M(eigenvalues[1] / eigenvalues[2], 0, 0,
  ...
  // convert the relevant vertices from Cartesian coords into the coordinate
  // system of the local frame (defined by the eigenvectors)
  Mat33d conversion_matrix;
  conversion_matrix = A;  // note that A is now holding the eigenvectors, not
                          // the original quadric tensor.
  ...
  ////warp the vertices into the normalized space to account for anisotropy, via
  ///M
  n0_2d = M * n0_2d;
  n1_2d = M * n1_2d;
  n2_2d = M * n2_2d;
  ...
  // check the Delaunay criterion (sum of opposite angles < 180) in the modified
  // space
...
bool EdgeFlipper::flip_pass() {
  ...
  //
  // Each "pass" is once over the entire set of edges (ignoring edges created
  ...
  while (flip_occurred && num_flip_passes++ < MAX_NUM_FLIP_PASSES) {
    ...
    size_t number_of_edges =
        m_mesh.m_edges.size();  // don't work on newly created edges

    for (size_t i = 0; i < number_of_edges; i++) {
      if (m_mesh.m_edges[i][0] == m_mesh.m_edges[i][1]) {
        continue;
      }
      if (m_mesh.m_edge_to_triangle_map[i].size() > 4 ||
          m_mesh.m_edge_to_triangle_map[i].size() < 2) {
        continue;
      }

      if (m_mesh.m_edge_to_triangle_map[i].size() == 3)
        continue;  // don't try flipping Y-junction non-manifold edges.

      // if ( m_mesh.m_is_boundary_vertex[ m_mesh.m_edges[i][0] ] ||
      // m_mesh.m_is_boundary_vertex[ m_mesh.m_edges[i][1] ] )  { continue; }  //
      // skip boundary vertices NOTE: This check disables flipping on edges where
      // either endpoint is on the boundary. Future work: extend to the cases
      // where meshes have open boundaries
      ...
      if (m_mesh.m_edge_to_triangle_map[i].size() == 2) {
        ...
        // Find first triangle with orientation opposite triangle_a's
        // orientation
        unsigned int j = 1;
        for (; j < 4; ++j) {
          triangle_b = m_mesh.m_edge_to_triangle_map[i][j];
          if (m_mesh.oriented(m_mesh.m_edges[i][0], m_mesh.m_edges[i][1],
                              m_mesh.get_triangle(triangle_a)) !=
              m_mesh.oriented(m_mesh.m_edges[i][0], m_mesh.m_edges[i][1],
                              m_mesh.get_triangle(triangle_b))) {
            break;
          }
        }
        assert(j < 4);
      } else {
        std::cout << m_mesh.m_edge_to_triangle_map[i].size()
                  << " triangles incident to an edge" << std::endl;
        assert(0);
      }

      // don't flip if one of the faces is all solid
      if (m_surf.triangle_is_all_solid(triangle_a) ||
          m_surf.triangle_is_all_solid(triangle_b)) {
        continue;
      }

      // Don't flip edge on a degenerate/deleted triangles
      if (m_mesh.triangle_is_deleted(triangle_a) ||
          m_mesh.triangle_is_deleted(triangle_b))
        continue;

      const Vec3st& tri_a = m_mesh.get_triangle(triangle_a);
      const Vec3st& tri_b = m_mesh.get_triangle(triangle_b);

      size_t third_vertex_0 = m_mesh.get_third_vertex(
          m_mesh.m_edges[i][0], m_mesh.m_edges[i][1], tri_a);
      size_t third_vertex_1 = m_mesh.get_third_vertex(
          m_mesh.m_edges[i][0], m_mesh.m_edges[i][1], tri_b);
      ...
      if (m_use_Delaunay_criterion) {
        ...
        if (m_surf.m_aggressive_mode) {
          ...
          double min_angle =
              min_triangle_angle(pos_vert_0, pos_vert_1, pos_3rd_0);
          min_angle = min(
              min_angle, min_triangle_angle(pos_vert_0, pos_vert_1, pos_3rd_1));
          if (min_angle > m_surf.m_min_triangle_angle) continue;

          double max_angle =
              max_triangle_angle(pos_vert_0, pos_vert_1, pos_3rd_0);
          max_angle = min(
              max_angle, max_triangle_angle(pos_vert_0, pos_vert_1, pos_3rd_1));
          if (max_angle < m_surf.m_max_triangle_angle) continue;
        }

        // if the sum of the opposing angles exceeds 180, then we should flip
        // (according to the Delaunay criterion) Delaunay apparently maximizes
        ...
        // Here we treat non-manifold vertices as being on boundaries, and
        // boundaries as boundaries. so their optimal valence is 4 instead of 6.
        // See e.g.
        // https://code.google.com/p/stacker/source/browse/trunk/GraphicsLibrary/Remeshing/LaplacianRemesher.h

        int opt_val_a =
                m_mesh.is_vertex_nonmanifold(third_vertex_0)
                    ? 4
                    : (m_mesh.m_is_boundary_vertex[third_vertex_0] ? 4 : 6),
            opt_val_b =
                m_mesh.is_vertex_nonmanifold(third_vertex_1)
                    ? 4
                    : (m_mesh.m_is_boundary_vertex[third_vertex_1] ? 4 : 6),
            opt_val_0 = m_mesh.is_vertex_nonmanifold(vert_0)
                            ? 4
                            : (m_mesh.m_is_boundary_vertex[vert_0] ? 4 : 6),
            opt_val_1 = m_mesh.is_vertex_nonmanifold(vert_1)
                            ? 4
                            : (m_mesh.m_is_boundary_vertex[vert_1] ? 4 : 6);

        int val_a, val_b, val_0, val_1;
        Vec2i region_pair = m_mesh.get_triangle_label(
            triangle_a);  // doesn't matter which triangle we consider.
        ...
        int score_before = sqr(val_a - opt_val_a) + sqr(val_b - opt_val_b) +
                           sqr(val_0 - opt_val_0) + sqr(val_1 - opt_val_1);
        ...
        int score_after = sqr(val_a - opt_val_a) + sqr(val_b - opt_val_b) +
                          sqr(val_0 - opt_val_0) + sqr(val_1 - opt_val_1);

        flip_required = score_before > score_after;

        double current_length =
            mag(xs[m_mesh.m_edges[i][1]] - xs[m_mesh.m_edges[i][0]]);
        double potential_length = mag(xs[third_vertex_1] - xs[third_vertex_0]);
      }

      if (flip_required) {
        flipped = flip_edge(i, triangle_a, triangle_b, third_vertex_0,
                            third_vertex_1);
      }
...
int EdgeFlipper::edge_count_bordering_region_pair(size_t vertex,
                                                  Vec2i region_pair) {
  int count = 0;
  ...
  for (size_t i = 0; i < m_surf.m_mesh.m_vertex_to_edge_map[vertex].size();
       ++i) {
    ...
    for (size_t j = 0; j < m_surf.m_mesh.m_edge_to_triangle_map[edge].size();
      ...
      // if one of the triangles matches the requested manifold region (label
      // pair), we're done.
