...
bool write_binary_file_with_newpositions(
    const NonDestructiveTriMesh &mesh, const std::vector<Vec3d> &x,
    const std::vector<double> &masses, const std::vector<Vec3d> &new_positions,
...
bool read_binary_file(NonDestructiveTriMesh &mesh, std::vector<Vec3d> &x,
...
bool read_binary_file_with_velocities(NonDestructiveTriMesh &mesh,
                                      ...
                                      std::vector<Vec3d> &v, double &curr_t,
                                      const char *filename_format, ...) {
  va_list ap;
  va_start(ap, filename_format);
  bifstream infile(filename_format, ap);
...
void generate_normals(const std::vector<Vec3d> &vertices,
  ...
  for (size_t i = 0; i < triangles.size(); i++) {
    ...
    e = std::pair<size_t, size_t>(t[0], t[1]);
    if (e.first > e.second) std::swap(e.first, e.second);
    ef_map[e].push_back(i);
    e = std::pair<size_t, size_t>(t[1], t[2]);
    if (e.first > e.second) std::swap(e.first, e.second);
    ef_map[e].push_back(i);
    e = std::pair<size_t, size_t>(t[2], t[0]);
    if (e.first > e.second) std::swap(e.first, e.second);
    ef_map[e].push_back(i);
  }
  ...
  for (size_t i = 0; i < triangles.size(); i++) {
    if (manifold_patch_labels[i] < 0) {
      ...
      while (open.size() > 0) {
        ...
        for (int j = 0; j < 3; j++) {
          std::pair<size_t, size_t> e;
          e = std::pair<size_t, size_t>(t[(j + 0) % 3], t[(j + 1) % 3]);
          if (e.first > e.second) std::swap(e.first, e.second);
  ...
  for (size_t i = 0; i < vertices.size(); i++) {
    ...
    int k = 0;
    for (std::set<int>::iterator j = manifold_patches.begin();
         j != manifold_patches.end(); j++, k++) {
      index_to_label[k] = *j;
    ...
    for (size_t j = 0; j < vf_map[i].size(); j++) {
      ...
      Vec3d facenormal = cross(vertices[t[1]] - vertices[t[0]],
...
bool write_objfile(const NonDestructiveTriMesh &mesh,
                   const std::vector<Vec3d> &x, const char *filename_format,
                   ...) {
  va_list ap;
  va_start(ap, filename_format);
#ifdef _MSC_VER
  int len = _vscprintf(filename_format, ap) +
  ...
  for (unsigned int t = 0; t < mesh.m_tris.size(); ++t)
    output << "f " << mesh.m_tris[t][0] + 1 << "//" << normal_indices[t][0] + 1
           << " " << mesh.m_tris[t][1] + 1 << "//" << normal_indices[t][1] + 1
           << " " << mesh.m_tris[t][2] + 1 << "//" << normal_indices[t][2] + 1
           << std::endl;  // correct for 1-based indexing in OBJ files
  return output.good();
}
...
bool write_objfile_per_region(const NonDestructiveTriMesh &mesh,
                              const std::vector<Vec3d> &x, int label,
  ...
  for (size_t i = 0; i < mesh.m_tris.size(); ++i) {
    Vec2i cur_label = mesh.get_triangle_label(i);
    if ((cur_label[0] == label || cur_label[1] == label) &&
        (excluding_regions.find(cur_label[0]) == excluding_regions.end() &&
         excluding_regions.find(cur_label[1]) == excluding_regions.end())) {
      Vec3st old_tri = mesh.m_tris[i];
      Vec3st new_tri(new_indices[old_tri[0]], new_indices[old_tri[1]],
                     new_indices[old_tri[2]]);
      // swap the orientation depending on which label is on the "front", for
      // good measure.
      if (cur_label[1] == label) std::swap(new_tri[1], new_tri[2]);
  ...
  output << "# generated by VoronoiFluid3D - meeting all your wacky fluid sim "
            "needs since 2010"
         << std::endl;
  ...
  for (unsigned int t = 0; t < new_tris.size(); ++t)
    output << "f " << new_tris[t][0] + 1 << "//" << normal_indices[t][0] + 1
           << " " << new_tris[t][1] + 1 << "//" << normal_indices[t][1] + 1
           << " " << new_tris[t][2] + 1 << "//" << normal_indices[t][2] + 1
           << std::endl;  // correct for 1-based indexing in OBJ files
  output.close();
...
bool write_objfile_per_region_pair(const NonDestructiveTriMesh &mesh,
                                   const std::vector<Vec3d> &x, Vec2i label,
                                   const char *filename_format, ...) {
  va_list ap;
  va_start(ap, filename_format);
#ifdef _MSC_VER
  ...
  for (size_t i = 0; i < mesh.m_tris.size(); ++i) {
    ...
    if (cur_label == label || cur_label == Vec2i(label[1], label[0])) {
      ...
      // swap the orientation depending on which label is on the "front", for
      // good measure.
      if (cur_label[1] == label[0]) std::swap(new_tri[1], new_tri[2]);
      new_tris.push_back(new_tri);
    }
  }
  ...
  //    output<<"f "<<new_tris[t][0]+1<<"//"<<normal_indices[t][0]+1<<"
  //    "<<new_tris[t][1]+1<<"//"<<normal_indices[t][1]+1<<"
  //    "<<new_tris[t][2]+1<<"//"<<normal_indices[t][2]+1<<std::endl; // correct
  //    for 1-based indexing in OBJ files
  for (unsigned int t = 0; t < new_tris.size(); ++t)
    output << "f " << new_tris[t][0] + 1 << " " << new_tris[t][1] + 1 << " "
           << new_tris[t][2] + 1
           << std::endl;  // correct for 1-based indexing in OBJ files
  output.close();
...
bool write_objfile_excluding_regions(const NonDestructiveTriMesh &mesh,
                                     const std::vector<Vec3d> &x,
  ...
  for (size_t i = 0; i < mesh.m_tris.size(); ++i) {
    Vec2i cur_label = mesh.get_triangle_label(i);
    if (labels.find(cur_label[0]) == labels.end() &&
        labels.find(cur_label[1]) == labels.end()) {
      Vec3st tri = mesh.m_tris[i];
      for (int j = 0; j < 3; ++j) {
        vert_needed[tri[j]] = true;
      }
...
static bool read_int(const char *s, int &value, bool &leading_slash,
                     int &position) {
  leading_slash = false;
...
bool read_objfile(NonDestructiveTriMesh &mesh, std::vector<Vec3d> &x,
                  const char *filename_format, ...) {
  va_list ap;
  va_start(ap, filename_format);
  ...
  while (input.good()) {
    ...
    switch (line[0]) {
      case 'v':  // vertex data
        if (line[1] == ' ') {
          Vec3d new_vertex;
          std::sscanf(line + 2, "%lf %lf %lf", &new_vertex[0], &new_vertex[1],
                      &new_vertex[2]);
      ...
      case 'f':  // face data
        if (line[1] == ' ') {
          ...
          for (int j = 0; j < (int)vertex_list.size() - 2; ++j)
            mesh.m_tris.push_back(
                Vec3st(vertex_list[0], vertex_list[j + 1], vertex_list[j + 2]));
        }
        break;
    }
  }
...
bool read_objfile(std::vector<Vec3st> &tris, std::vector<Vec3d> &x,
                  const char *filename_format, ...) {
  va_list ap;
  va_start(ap, filename_format);
...
bool write_ribfile(const NonDestructiveTriMesh &mesh,
...
bool write_ribfile(const NonDestructiveTriMesh &mesh,
  ...
  fprintf(output, " [ ");
  for (unsigned int i = 0; i < mesh.m_tris.size(); ++i) {
    fprintf(output, " %ld %ld %ld ", mesh.m_tris[i][0], mesh.m_tris[i][1],
            mesh.m_tris[i][2]);
...
#ifdef _MSC_VER
  len = _vscprintf("track%04d_shadow.tiff", ap) +
        1;  // _vscprintf doesn't count // terminating '\0'
  delete[] filename;
...
bool output_rib(const std::vector<Vec3d> &positions,
                const NonDestructiveTriMesh &mesh, const char *filename_format,
                ...) {
  va_list ap;
  va_start(ap, filename_format);
...
#ifdef _MSC_VER
  len = _vscprintf("track%04d.tiff", ap) +
        1;  // _vscprintf doesn't count // terminating '\0'
  delete[] filename;
  filename = new char[len];
  ...
  for (unsigned int i = 0; i < positions.size(); ++i) {
    ...
    for (unsigned int j = 0; j < mesh.m_vertex_to_triangle_map[i].size(); ++j) {
      Vec3d u = positions[mesh.m_tris[mesh.m_vertex_to_triangle_map[i][j]][1]] -
                positions[mesh.m_tris[mesh.m_vertex_to_triangle_map[i][j]][0]];
      Vec3d v = positions[mesh.m_tris[mesh.m_vertex_to_triangle_map[i][j]][2]] -
                positions[mesh.m_tris[mesh.m_vertex_to_triangle_map[i][j]][0]];
  ...
  // next line: image format (width and height in pixels, pixel aspect ratio)
  out << "Format " << Gluvi::winwidth << " " << Gluvi::winheight << " 1"
      << std::endl;
  ...
  out << "LightSource \"ambientlight\" 1 \"intensity\" .3 \"lightcolor\" [1 1 "
         "1]\n";
  ...
  /*
   ...
   light_pos += Vec3f( lights[i].target[0], lights[i].target[1],
   lights[i].target[2] );

   std::cout << "light position: " << light_pos << std::endl;

   out << "LightSource \"singleshadowpoint\" 2 \"intensity\" 30 \"from\" [" <<
   light_pos << "] \"shadowmap\" \"" << shadow_filename << "\"\n";
   }
   */

  // out << "LightSource \"distantlight\" 3 \"intensity\" 0.3 \"from\" [-5 -10
  // 20] \"to\" [0 0 0]\n";
  ...
  sprintf(buf,
          "  Polygon \"P\" [-%f -%f -%f %f -%f -%f  %f %f -%f  -%f %f -%f]\n",
          plane_limit, plane_limit, plane_distance, plane_limit, plane_limit,
          plane_distance, plane_limit, plane_limit, plane_distance, plane_limit,
          plane_limit, plane_distance);
  out << buf;
...
bool write_pbrtfile(const NonDestructiveTriMesh &mesh,
                    const std::vector<float> &x, const char *filename_format,
                    ...) {
  va_list ap;
  va_start(ap, filename_format);
