...
// --------------------------------------------------------
///
/// Construct one grid from the given set of AABBs, using the given length scale
/// as the cell size, with the given padding
///
// --------------------------------------------------------
...
void BroadPhaseGrid::build_acceleration_grid(AccelerationGrid& grid,
  ...
  // going backwards from n to 0, so hopefully the grid only has to allocate
  // once
  for (int i = (int)n - 1; i >= 0; i--) {
    // don't add inside-out AABBs
    if (xmins[i][0] > xmaxs[i][0]) {
      continue;
    }
...
void BroadPhaseGrid::update_broad_phase(const DynamicSurface& surface,
                                        bool continuous) {
  ...
  //
  {
    ...
    build_acceleration_grid(m_solid_vertex_grid, solid_vertex_xmins,
                            solid_vertex_xmaxs, solid_vertex_indices,
                            grid_scale, surface.m_aabb_padding);

    build_acceleration_grid(m_dynamic_vertex_grid, dynamic_vertex_xmins,
                            dynamic_vertex_xmaxs, dynamic_vertex_indices,
                            grid_scale, surface.m_aabb_padding);
  }
  ...
  //
  {
    ...
    build_acceleration_grid(m_solid_edge_grid, solid_edge_xmins,
                            solid_edge_xmaxs, solid_edge_indices, grid_scale,
                            surface.m_aabb_padding);

    build_acceleration_grid(m_dynamic_edge_grid, dynamic_edge_xmins,
                            dynamic_edge_xmaxs, dynamic_edge_indices,
                            grid_scale, surface.m_aabb_padding);
  }
  ...
  //
  {
    ...
    build_acceleration_grid(m_solid_triangle_grid, solid_tri_xmins,
                            solid_tri_xmaxs, solid_tri_indices, grid_scale,
                            surface.m_aabb_padding);

    build_acceleration_grid(m_dynamic_triangle_grid, dynamic_tri_xmins,
                            dynamic_tri_xmaxs, dynamic_tri_indices, grid_scale,
                            surface.m_aabb_padding);
  }
}
