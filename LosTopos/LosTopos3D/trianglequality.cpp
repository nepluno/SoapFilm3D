...
double mixed_area(size_t vertex_index, size_t triangle_index,
  ...
  bool obtuse_triangle =
      ((dot(b - a, c - a) < 0.0) || (dot(a - b, c - b) < 0.0) ||
       (dot(a - c, b - c) < 0.0));

  if (obtuse_triangle) {
    // std::cout << "obtuse_triangle " << triangle_index << ": " << tri <<
    // std::endl;
...
void vertex_mean_curvature_normal(size_t vertex_index, const SurfTrack& surf,
  ...
  for (size_t i = 0; i < surf.m_mesh.m_vertex_to_edge_map[vertex_index].size();
       ++i) {
    ...
    if (curr_edge[0] == vertex_index) {
      edge_vector =
          surf.get_position(curr_edge[1]) - surf.get_position(vertex_index);
    } else {
      assert(curr_edge[1] == vertex_index);
      edge_vector =
          surf.get_position(curr_edge[0]) - surf.get_position(vertex_index);
    }
    ...
    size_t third_vertex_0 = surf.m_mesh.get_third_vertex(
        curr_edge[0], curr_edge[1], surf.m_mesh.get_triangle(tri0));
    size_t third_vertex_1 = surf.m_mesh.get_third_vertex(
        curr_edge[0], curr_edge[1], surf.m_mesh.get_triangle(tri1));

    Vec3d v00 =
        surf.get_position(curr_edge[0]) - surf.get_position(third_vertex_0);
    Vec3d v10 =
        surf.get_position(curr_edge[1]) - surf.get_position(third_vertex_0);
    ...
    Vec3d v01 =
        surf.get_position(curr_edge[0]) - surf.get_position(third_vertex_1);
    Vec3d v11 =
        surf.get_position(curr_edge[1]) - surf.get_position(third_vertex_1);
  ...
  for (size_t i = 0;
       i < surf.m_mesh.m_vertex_to_triangle_map[vertex_index].size(); ++i) {
    vertex_area +=
        mixed_area(vertex_index,
                   surf.m_mesh.m_vertex_to_triangle_map[vertex_index][i], surf);
  }
...
// ---------------------------------------------------------
///
/// Return an estimate for curvature by computing the minimum radius of a sphere
/// defined by the edge neighbourhood around a vertex, and taking the
/// reciprocal.
///
// ---------------------------------------------------------
...
double estimated_max_curvature(const SurfTrack& surf, size_t vertex) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (std::set<size_t>::iterator it = twoneighbors.begin();
         it != twoneighbors.end(); ++it) {
      size_t curVert = *it;
      const std::vector<size_t>& nbrEdges =
          surf.m_mesh.m_vertex_to_edge_map[curVert];
  ...
  for (std::set<size_t>::iterator it = twoneighbors.begin();
    ...
    Vec3d diff;
    for (int i = 0; i < 3; i++) diff[i] = adjpt[i] - centpt[i];
  ...
  // LosTopos stores things in column major rather than row major, I believe,
  // hence the transposing
  Mat33d U(u[0], v[0], normal[0], u[1], v[1], normal[1], u[2], v[2], normal[2]);
  U = U.transpose();

  Mat32d J(1, 0, coeffs[3], 0, 1, coeffs[4]);
...
double get_curvature_scaled_length(const SurfTrack& surf, size_t vertex_a,
  ...
  double length =
      dist(surf.get_position(vertex_a), surf.get_position(vertex_b));
...
double min_triangle_area(const SurfTrack& surf) {
  ...
  for (size_t i = 0; i < surf.m_mesh.num_triangles(); ++i) {
    if (surf.m_mesh.triangle_is_deleted(i)) {
      continue;
    }
    if (surf.triangle_is_all_solid(i)) {
      continue;
    }
...
size_t num_angles_below_threshold(const SurfTrack& surf, double low_threshold) {
  ...
  for (size_t i = 0; i < surf.m_mesh.num_triangles(); ++i) {
    ...
    if (angle_a < low_threshold) {
      ++num_small_angles;
    }
    if (angle_b < low_threshold) {
      ++num_small_angles;
    }
    if (angle_c < low_threshold) {
      ++num_small_angles;
    }
  }
...
size_t num_angles_above_threshold(const SurfTrack& surf,
  ...
  for (size_t i = 0; i < surf.m_mesh.num_triangles(); ++i) {
    ...
    if (angle_a > high_threshold) {
      ++num_large_angles;
    }
    if (angle_b > high_threshold) {
      ++num_large_angles;
    }
    if (angle_c > high_threshold) {
      ++num_large_angles;
    }
  }
...
double triangle_aspect_ratio(const SurfTrack& surf, size_t triangle_index) {
  const Vec3st& tri = surf.m_mesh.get_triangle(triangle_index);
  assert(tri[0] != tri[1]);
  return triangle_aspect_ratio(surf.get_position(tri[0]),
                               surf.get_position(tri[1]),
                               surf.get_position(tri[2]));
}
