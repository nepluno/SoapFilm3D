...
//
//  Search for mesh edges which are near to each other, zipper their
//  neighbouring triangles together.
//
// ---------------------------------------------------------
...
void MeshMerger::twist_vertices(size_t* zipper_vertices) {
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 4; j < 8; ++j) {
      dist = mag(m_surf.get_position(zipper_vertices[i]) -
                 m_surf.get_position(zipper_vertices[j]));
      if (dist < min_dist) {
        min_dist = dist;
        min_pair[0] = i;
        min_pair[1] = j;
...
bool MeshMerger::get_zipper_triangles(size_t edge_index_a, size_t edge_index_b,
                                      std::vector<Vec3st>& output_triangles,
                                      int& shared_label) {
  ...
  const std::vector<size_t>& incident_triangles_a =
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index_a];
  const std::vector<size_t>& incident_triangles_b =
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index_b];

  assert(incident_triangles_b.size() ==
         2);  // should be checked before calling this function
  assert(incident_triangles_a.size() == 2);

  // get wings of the first edge
  const Vec3st& inc_tri_a0 =
      m_surf.m_mesh.get_triangle(incident_triangles_a[0]);
  const Vec3st& inc_tri_a1 =
      m_surf.m_mesh.get_triangle(incident_triangles_a[1]);

  // get wings of the second edge
  const Vec3st& inc_tri_b0 =
      m_surf.m_mesh.get_triangle(incident_triangles_b[0]);
  const Vec3st& inc_tri_b1 =
      m_surf.m_mesh.get_triangle(incident_triangles_b[1]);

  // work out which will be the "middle" or shared region, i.e. the one that the
  // tunnel will penetrate. it must be one where there is a duplicate region
  // label. (There may be *two* duplicate regions in the re-merging / 2-phase
  //case. Doesn't matter as long as we pick one shared region and be
  // consistent.)

  // get two labels that we'll use to do the ordering decisions.
  const Vec2i& inc_tri_a0_label =
      m_surf.m_mesh.get_triangle_label(incident_triangles_a[0]);
  const Vec2i& inc_tri_b0_label =
      m_surf.m_mesh.get_triangle_label(incident_triangles_b[0]);

  // work out which of the labels is shared between the two triangles
  shared_label = -1;
  if (inc_tri_a0_label[0] == inc_tri_b0_label[0] ||
      inc_tri_a0_label[0] == inc_tri_b0_label[1]) {
    shared_label = inc_tri_a0_label[0];
  } else if (inc_tri_a0_label[1] == inc_tri_b0_label[0] ||
             inc_tri_a0_label[1] == inc_tri_b0_label[1]) {
    shared_label = inc_tri_a0_label[1];
  } else {
    // Note: this case can happen if a triangle comes close to another triangle,
    // when the two are also separated by yet ANOTHER interface. Collision
    // detection would cull it, but lets kill it here instead.
    return false;
  }

  // get the other two vertices comprising the first patch
  size_t third_vertices[2];
  third_vertices[0] = m_surf.m_mesh.get_third_vertex(
      zipper_vertices[0], zipper_vertices[2], inc_tri_a0);
  third_vertices[1] = m_surf.m_mesh.get_third_vertex(
      zipper_vertices[0], zipper_vertices[2], inc_tri_a1);

  // assign the vertex ordering so that the loop is consistently oriented with
  // *triangle a0*.
  if (m_surf.m_mesh.oriented(zipper_vertices[0], zipper_vertices[2],
                             inc_tri_a0)) {
  ...
  third_vertices[0] = m_surf.m_mesh.get_third_vertex(
      zipper_vertices[4], zipper_vertices[6], inc_tri_b0);
  third_vertices[1] = m_surf.m_mesh.get_third_vertex(
      zipper_vertices[4], zipper_vertices[6], inc_tri_b1);

  // assign the vertex ordering to be consistent with *triangle b0*.
  if (m_surf.m_mesh.oriented(zipper_vertices[4], zipper_vertices[6],
                             inc_tri_b0)) {
  ...
  for (unsigned int i = 0; i < 8; ++i) {
    for (unsigned int j = i + 1; j < 8; ++j) {
      ...
      // Check if an edge already exists between two vertices in opposite edge
      // neighbourhoods (i.e. look for an edge which would be created by
      // zippering)

      if ((i < 4) && (j > 3)) {
        for (size_t ii = 0;
             ii < m_surf.m_mesh.m_vertex_to_edge_map[zipper_vertices[i]].size();
             ++ii) {
          for (size_t jj = 0;
               jj <
               m_surf.m_mesh.m_vertex_to_edge_map[zipper_vertices[j]].size();
               ++jj) {
            if (m_surf.m_mesh.m_vertex_to_edge_map[zipper_vertices[i]][ii] ==
                m_surf.m_mesh.m_vertex_to_edge_map[zipper_vertices[j]][jj]) {
              return false;
            }
          }
        }
  ...
  if (shared_label_pos_a == shared_label_pos_b) {
    // normal orientation
    output_triangles[0] =
        Vec3st(zipper_vertices[0], zipper_vertices[1], zipper_vertices[7]);
    output_triangles[1] =
        Vec3st(zipper_vertices[0], zipper_vertices[7], zipper_vertices[4]);
    output_triangles[2] =
        Vec3st(zipper_vertices[1], zipper_vertices[2], zipper_vertices[6]);
    output_triangles[3] =
        Vec3st(zipper_vertices[1], zipper_vertices[6], zipper_vertices[7]);
    output_triangles[4] =
        Vec3st(zipper_vertices[2], zipper_vertices[3], zipper_vertices[5]);
    output_triangles[5] =
        Vec3st(zipper_vertices[2], zipper_vertices[5], zipper_vertices[6]);
    output_triangles[6] =
        Vec3st(zipper_vertices[3], zipper_vertices[0], zipper_vertices[4]);
    output_triangles[7] =
        Vec3st(zipper_vertices[3], zipper_vertices[4], zipper_vertices[5]);
  } else {
    // switched orientation
    output_triangles[0] =
        Vec3st(zipper_vertices[0], zipper_vertices[1], zipper_vertices[5]);
    output_triangles[1] =
        Vec3st(zipper_vertices[0], zipper_vertices[5], zipper_vertices[4]);
    output_triangles[2] =
        Vec3st(zipper_vertices[1], zipper_vertices[2], zipper_vertices[6]);
    output_triangles[3] =
        Vec3st(zipper_vertices[1], zipper_vertices[6], zipper_vertices[5]);
    output_triangles[4] =
        Vec3st(zipper_vertices[2], zipper_vertices[3], zipper_vertices[7]);
    output_triangles[5] =
        Vec3st(zipper_vertices[2], zipper_vertices[7], zipper_vertices[6]);
    output_triangles[6] =
        Vec3st(zipper_vertices[3], zipper_vertices[0], zipper_vertices[4]);
    output_triangles[7] =
        Vec3st(zipper_vertices[3], zipper_vertices[4], zipper_vertices[7]);
  }

  // now we have a tunnel, whose triangles match the orientation of triangle a0.
  // we can simply copy the label over.
  // both triangle b labels needs to be adjusted so that the shared_region is
  // replaced with a's interior (region 0, the non-shared region).
...
// --------------------------------------------------------
///
/// Check whether the introduction of the new zippering triangles causes a
/// collision
///
// --------------------------------------------------------

bool MeshMerger::zippering_introduces_collision(
    const std::vector<Vec3st>& new_triangles,
    const std::vector<size_t>& deleted_triangles) {
  for (size_t i = 0; i < new_triangles.size(); ++i) {
    // Check all existing edges vs new triangles
    Vec3d low, high;
    minmax(m_surf.get_position(new_triangles[i][0]),
           m_surf.get_position(new_triangles[i][1]),
           m_surf.get_position(new_triangles[i][2]), low, high);
    ...
    // Check to make sure there doesn't already exist triangles with the same
    // vertices
    ...
    for (size_t t = 0; t < overlapping_triangles.size(); ++t) {
      ...
      if (check_triangle_triangle_intersection(
              new_triangles[i],
              m_surf.m_mesh.get_triangle(overlapping_triangles[t]),
    ...
    for (size_t j = 0; j < new_triangles.size(); ++j) {
      ...
      if (check_triangle_triangle_intersection(
              new_triangles[i], new_triangles[j], m_surf.get_positions())) {
        return true;
      }
    }
  }

  // For real collision safety, we need to check for vertices inside the new,
  // joined volume. Checking edges vs triangles is technically not enough.
...
bool MeshMerger::zipper_edges(size_t edge_index_a, size_t edge_index_b) {
  ...
  if (m_surf.m_mesh.m_edge_to_triangle_map[edge_index_a].size() != 2 ||
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index_b].size() != 2) {
  ...
  // each patch should have labels consistent within themselves and their
  // orientations.
  if (m_surf.m_mesh.oriented(v0, v1,
                             m_surf.m_mesh.get_triangle(triangle_a_0)) ==
      m_surf.m_mesh.oriented(v1, v0,
                             m_surf.m_mesh.get_triangle(triangle_a_1))) {
    assert(label_a_0 == label_a_1);
  } else {
    assert(label_a_0[0] == label_a_1[1] && label_a_0[1] == label_a_1[0]);
  }
  if (m_surf.m_mesh.oriented(v2, v3,
                             m_surf.m_mesh.get_triangle(triangle_b_0)) ==
      m_surf.m_mesh.oriented(v3, v2,
                             m_surf.m_mesh.get_triangle(triangle_b_1))) {
  ...
  deleted_triangles.push_back(
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index_a][0]);
  deleted_triangles.push_back(
      m_surf.m_mesh.m_edge_to_triangle_map[edge_index_a][1]);
  // if the labels don't match, keep the next two triangles as the separating
  // film
  if (region_0 == region_1) {
    ...
    deleted_triangles.push_back(
        m_surf.m_mesh.m_edge_to_triangle_map[edge_index_b][0]);
    deleted_triangles.push_back(
        m_surf.m_mesh.m_edge_to_triangle_map[edge_index_b][1]);
  }

  //
  // Check the new triangles for collision safety, ignoring the triangles which
  // will be deleted
  //
  ...
  if (m_surf.m_collision_safety &&
      zippering_introduces_collision(new_triangles, deleted_triangles)) {
  ...
  // on the original b triangles, just replace the shared region label (where it
  // occurs) with a's other region which now comprises the interior of the
  // tunnel.
  ...
  m_surf.remove_triangle(m_surf.m_mesh.m_edge_to_triangle_map[edge_index_a][0]);
  if (region_0 == region_1) {
    // the original labels matched, so this is two of the same region combining
    // together. Delete the wall/film.
    m_surf.remove_triangle(
        m_surf.m_mesh.m_edge_to_triangle_map[edge_index_b][0]);
    m_surf.remove_triangle(
        m_surf.m_mesh.m_edge_to_triangle_map[edge_index_b][0]);
  } else {
  ...
  if (region_0 != region_1) {  // account for relabeling of retained film
    update.m_dirty_tris.push_back(
        std::pair<size_t, Vec2i>(triangle_b_0, label_new_b_0));
    update.m_dirty_tris.push_back(
        std::pair<size_t, Vec2i>(triangle_b_1, label_new_b_1));
  }
...
// ---------------------------------------------------------
///
/// Look for pairs of edges close to each other, attempting to merge when close
/// edges are found.
///
// ---------------------------------------------------------
...
bool MeshMerger::merge_pass() {
  ...
  for (size_t i = 0; i < m_surf.m_mesh.m_edges.size(); i++) {
    ...
    if (m_surf.edge_is_any_solid(i)) {
      continue;
    }

    if (m_surf.m_mesh.m_is_boundary_vertex[e0[0]] ||
        m_surf.m_mesh.m_is_boundary_vertex[e0[1]]) {
      continue;
    ...
    m_surf.m_broad_phase->get_potential_edge_collisions(emin, emax, false, true,
                                                        edge_candidates);
    ...
    for (size_t j = 0; j < edge_candidates.size(); j++) {
      ...
      if (m_surf.m_mesh.m_is_boundary_vertex[e1[0]] ||
          m_surf.m_mesh.m_is_boundary_vertex[e1[1]]) {
        continue;
      }  // skip boundary vertices
      ...
      if (e0[0] != e1[0] && e0[0] != e1[1] && e0[1] != e1[0] &&
        ...
        check_edge_edge_proximity(
            m_surf.get_position(e0[0]), m_surf.get_position(e0[1]),
            m_surf.get_position(e1[0]), m_surf.get_position(e1[1]), distance,
            s0, s2, normal);

        if (distance < m_surf.m_merge_proximity_epsilon) {
          if (m_surf.m_verbose) {
            std::cout << "proximity: " << distance << " / "
                      << m_surf.m_merge_proximity_epsilon
                      << std::endl;  // proximities[i].distance << std::endl;
          }
