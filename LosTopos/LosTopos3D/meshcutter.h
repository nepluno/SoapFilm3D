...
// ---------------------------------------------------------
///
/// Mesh cutter object.  Splits the mesh along a desired set of edges, if
/// possible.
///
// ---------------------------------------------------------
...
class MeshCutter {
 ...
 private:
  ...
  /// separated by the specified edge (which must have at least one vertex on
  /// the boundary).
  ///
  void partition_edge_neighbourhood(
      size_t edge_index, std::vector<TriangleSet>& connected_components);

  /// Partition the triangles incident to an internal vertex into connected
  /// components, separated by two connected internal edges.
  ///
  void partition_edge_neighbourhood_internal(
      size_t edge0, size_t edge1,
      std::vector<TriangleSet>& connected_components);

  /// Helper function to partition a set of triangles into connected components
  /// separated by a given list of edges
  ///
  void perform_partitioning(const std::vector<size_t>& incident_tris,
                            const std::vector<size_t>& separating_edges,
                            std::vector<TriangleSet>& connected_components);

  /// Duplicate one or two boundary vertices of an edge, and move the copies
  /// away from each other slightly
  ///
  bool pull_apart_edge(size_t edge_index,
                       const std::vector<TriangleSet>& connected_components);
  bool pull_apart_edge_internal(
      size_t edge0, size_t edge1,
  ...
  ///
  bool perform_pull_apart(const std::vector<size_t>& separating_verts,
                          const std::vector<TriangleSet>& connected_components,
