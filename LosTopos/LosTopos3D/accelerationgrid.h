...
class AccelerationGrid {
 public:
  ...
  /// Define the grid given, the extents of the domain and the number of voxels
  ...
  ///
  void boundstoindices(const Vec3d& xmin, const Vec3d& xmax, Vec3i& xmini,
                       Vec3i& xmaxi);
  ...
  ///
  void find_overlapping_elements(const Vec3d& xmin, const Vec3d& xmax,
                                 std::vector<size_t>& results);

  /// Each cell contains an array of indices specifying the elements whose AABBs
  /// overlap the cell
  ///
  Array3<std::vector<size_t>*> m_cells;

  /// For each element, a list of triples, each triple specifying a cell which
  ...
  /// For each element, the timestamp of the last query that examined the
  /// element
  ///
