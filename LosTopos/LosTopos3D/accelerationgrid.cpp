...
AccelerationGrid::~AccelerationGrid() { clear(); }

// --------------------------------------------------------
///
/// Define the grid, given the extents of the domain and the number of desired
/// voxels along each dimension
///
// --------------------------------------------------------

void AccelerationGrid::set(const Vec3st& dims, const Vec3d& xmin,
                           const Vec3d& xmax) {
  m_gridxmin = xmin;
  m_gridxmax = xmax;
...
void AccelerationGrid::boundstoindices(const Vec3d& xmin, const Vec3d& xmax,
                                       Vec3i& xmini, Vec3i& xmaxi) {
...
void AccelerationGrid::add_element(size_t idx, const Vec3d& xmin,
                                   const Vec3d& xmax) {
  if (m_elementcount <= (int)idx) {
    m_elementidxs.resize(
        idx + 1);  // only ever grow m_elementidxs. but we won't clear it, since
                   // we don't want to have to reallocate its contained vectors.
    ...
    for (cur_index[1] = xmini[1]; cur_index[1] <= xmaxi[1]; cur_index[1]++) {
      for (cur_index[0] = xmini[0]; cur_index[0] <= xmaxi[0]; cur_index[0]++) {
        std::vector<size_t>*& cell =
            m_cells(cur_index[0], cur_index[1], cur_index[2]);
        if (!cell) {
...
void AccelerationGrid::remove_element(size_t idx) {
  ...
  for (size_t c = 0; c < m_elementidxs[idx].size(); c++) {
    ...
    std::vector<size_t>* cell =
        m_cells((int)cellcoords[0], (int)cellcoords[1], (int)cellcoords[2]);
...
void AccelerationGrid::update_element(size_t idx, const Vec3d& xmin,
                                      const Vec3d& xmax) {
  // FYI: formerly we had a full remove-element and then add-element, which is
  // less efficient.

  if (idx >= m_elementcount) {
    // I don't see why this case should ever occur, but for now let's handle it
    // in the same manner as a remove-then-add did in the past.
  ...
  // if the list of cells it formerly filled is zero, it's basically a new
  // element, since there can be no overlap
  ...
  if (!is_new && boxes_overlap(xmini_old, xmaxi_old, xmini_new, xmaxi_new)) {
    ...
    Vec3i cur_index;
    for (cur_index[2] = total_min[2]; cur_index[2] <= total_max[2];
         cur_index[2]++) {
      bool in_new_z =
          cur_index[2] >= xmini_new[2] && cur_index[2] <= xmaxi_new[2];
      bool in_old_z =
          cur_index[2] >= xmini_old[2] && cur_index[2] <= xmaxi_old[2];
      if (!in_new_z && !in_old_z) continue;

      for (cur_index[1] = total_min[1]; cur_index[1] <= total_max[1];
           cur_index[1]++) {
        bool in_new_y =
            cur_index[1] >= xmini_new[1] && cur_index[1] <= xmaxi_new[1];
        bool in_old_y =
            cur_index[1] >= xmini_old[1] && cur_index[1] <= xmaxi_old[1];
        if (!in_new_y && !in_old_y) continue;

        for (cur_index[0] = total_min[0]; cur_index[0] <= total_max[0];
             cur_index[0]++) {
          bool in_new_x =
              cur_index[0] >= xmini_new[0] && cur_index[0] <= xmaxi_new[0];
          bool in_old_x =
              cur_index[0] >= xmini_old[0] && cur_index[0] <= xmaxi_old[0];
          ...
          if (in_new)  // in new set
          {
          ...
          } else if (in_old) {  // not in new set, but it is in the old set;
                                // must delete it
            std::vector<size_t>*& cell =
                m_cells(cur_index[0], cur_index[1], cur_index[2]);
            ...
            while (*it2 != Vec3st(cur_index)) it2++;
    ...
    // the old and new regions don't overlap, so do it the easy way - remove and
    // then add. but still slightly smarter than actually calling remove_elt and
    // add_elt
    ...
    for (cur_index[2] = xmini[2]; cur_index[2] <= xmaxi[2]; cur_index[2]++) {
      for (cur_index[1] = xmini[1]; cur_index[1] <= xmaxi[1]; cur_index[1]++) {
        for (cur_index[0] = xmini[0]; cur_index[0] <= xmaxi[0];
             cur_index[0]++) {
          std::vector<size_t>*& cell =
              m_cells(cur_index[0], cur_index[1], cur_index[2]);
          if (!cell) {
...
void AccelerationGrid::clear() {
  for (size_t i = 0; i < m_cells.a.size(); i++) {
    ...
    if (cell) {
      delete cell;
      cell = 0;
      // cell->clear(); //don't clear the memory, since we likely want to reuse
      // it.
    }
  }

  // clear the entries for each element, but don't clear the overall vector
  // itself, since we don't want to reallocate memory for the individual vectors.
  // m_elementidxs.clear();
...
void AccelerationGrid::find_overlapping_elements(const Vec3d& xmin,
                                                 const Vec3d& xmax,
        ...
        if (cell) {
          for (std::vector<size_t>::const_iterator citer = cell->begin();
            ...
            if (m_elementquery[oidx] < m_lastquery) {
              // Object has not been found.  Set m_elementquery so that it will
              // not be tested again during this query.
