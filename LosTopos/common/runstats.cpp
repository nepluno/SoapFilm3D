...
bool RunStats::get_per_frame_ints(std::string name,
                                  std::vector<PerFrameInt>& sequence) {
  std::map<std::string, std::vector<PerFrameInt> >::iterator iter =
      per_frame_int_stats.find(name);
...
bool RunStats::get_per_frame_doubles(std::string name,
                                     std::vector<PerFrameDouble>& sequence) {
  std::map<std::string, std::vector<PerFrameDouble> >::iterator iter =
      per_frame_double_stats.find(name);
...
void RunStats::write_to_file(const char* filename) {
  ...
  if (!double_stats.empty()) {
    ...
    std::map<std::string, double>::iterator double_iterator =
        double_stats.begin();
    for (; double_iterator != double_stats.end(); ++double_iterator) {
      file << double_iterator->first << ": " << double_iterator->second
           << std::endl;
    }
    file << std::endl;
  }
  ...
  if (!per_frame_int_stats.empty()) {
    ...
    std::map<std::string, std::vector<PerFrameInt> >::iterator pfi_iter =
        per_frame_int_stats.begin();
  ...
  if (!per_frame_double_stats.empty()) {
    file << "per_frame_double_stats: " << std::endl
         << "----------" << std::endl;
    std::map<std::string, std::vector<PerFrameDouble> >::iterator pfd_iter =
        per_frame_double_stats.begin();
