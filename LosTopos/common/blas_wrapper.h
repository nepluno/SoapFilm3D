...
// Useful dense kernels from BLAS, with readable, overloaded, cross-platform
// names and some simplified calling
...
//  gemv   (multiplying a matrix times a vector, scaling, and adding result to
//  another vector)) gemm   (multiplying two matrices, scaling, and adding
//  result to another matrix)
// In addition:
//  set_zero  (zero out all entries in a vector)
//  abs_max   (return the infinity norm of a vector, i.e. the magnitude of its
//  largest element)
...
//    USE_FORTRAN_BLAS (if your BLAS calls should look like dgemm_ with FORTRAN
//    calling conventions, as in GOTO BLAS) USE_AMD_BLAS     (if using the AMD
//    Math Library) USE_CBLAS        (if instead you have calls like
//    cblas_dgemm, and have a file "cblas.h" available)
// or, if you're on the Mac, it will default to the vecLib CBLAS if none of
// these are specified.
...
extern "C" {
double dsdot_(const int *, const float *, const int *, const float *,
              const int *);
double sdot_(const int *, const float *, const int *, const float *,
             const int *);
...
inline double dot(int n, const float *x, int incx, const float *y,
                  int incy = 1) {
...
inline double dot(int n, const float *x, const float *y, int incy = 1) {
  const int one = 1;
  return dsdot_(&n, x, &one, y, &incy);
}

inline float dotf(int n, const float *x, int incx, const float *y,
                  int incy = 1) {
...
inline float dotf(int n, const float *x, const float *y, int incy = 1) {
  const int one = 1;
  return (float)sdot_(&n, x, &one, y, &incy);
}
...
inline void copy(int n, const double *x, double *y, int incy = 1) {
  const int one = 1;
  dcopy_(&n, x, &one, y, &incy);
}

// saxpy (y=alpha*x+y)

inline void add_scaled(int n, float alpha, const float *x, int incx, float *y,
                       int incy = 1) {
...
inline void add_scaled(int n, float alpha, const float *x, float *y,
                       int incy = 1) {
  const int one = 1;
  saxpy_(&n, &alpha, x, &one, y, &incy);
}

inline void add_scaled(int n, double alpha, const double *x, int incx,
                       double *y, int incy = 1) {
...
inline void multiply_matrix_vector(Transpose transpose, int m, int n,
                                   float alpha, const float *A, int lda,
                                   const float *x, int incx, float beta,
                                   float *y, int incy = 1) {
  sgemv_((const char *)&transpose, &m, &n, &alpha, A, &lda, x, &incx, &beta, y,
         &incy);
}

inline void multiply_matrix_vector(int m, int n, const float *A, const float *x,
                                   float *y, int incy = 1)  // y=A*x
{
  const int onei = 1;
  const float zero = 0, onef = 1;
  sgemv_("N", &m, &n, &onef, A, &m, x, &onei, &zero, y, &incy);
}
...
inline void multiply_matrix_vector(Transpose transpose, int m, int n,
                                   ...
                                   double *y, int incy = 1) {
  dgemv_((const char *)&transpose, &m, &n, &alpha, A, &lda, x, &incx, &beta, y,
         &incy);
}

inline void multiply_matrix_vector(int m, int n, const double *A,
                                   const double *x, double *y,
                                   int incy = 1)  // y=A*x
{
  const int onei = 1;
  const double zero = 0, onef = 1;
  dgemv_("N", &m, &n, &onef, A, &m, x, &onei, &zero, y, &incy);
}

// gemm (C=alpha*A*B+beta*C)

inline void multiply_matrix_matrix(Transpose transA, Transpose transB, int m,
                                   int n, int k, float alpha, const float *A,
                                   int lda, const float *B, int ldb, float beta,
                                   float *C, int ldc) {
  sgemm_((const char *)&transA, (const char *)&transB, &m, &n, &k, &alpha, A,
         &lda, B, &ldb, &beta, C, &ldc);
}

inline void multiply_matrix_matrix(int m, int n, int k, const float *A,
...
inline void multiply_matrix_matrix(Transpose transA, Transpose transB, int m,
  ...
  dgemm_((const char *)&transA, (const char *)&transB, &m, &n, &k, &alpha, A,
         &lda, B, &ldb, &beta, C, &ldc);
}

inline void multiply_matrix_matrix(int m, int n, int k, const double *A,
...
inline void multiply_matrix_vector(Transpose transpose, int m, int n,
                                   ...
                                   const float *x, int incx, float beta,
                                   float *y, int incy = 1) {
  sgemv(transpose, m, n, alpha, (float *)A, lda, (float *)x, incx, beta, y,
        incy);
}
...
inline void multiply_matrix_vector(Transpose transpose, int m, int n,
                                   ...
                                   const double *x, int incx, double beta,
                                   double *y, int incy = 1) {
  dgemv(transpose, m, n, alpha, (double *)A, lda, (double *)x, incx, beta, y,
        incy);
}
...
inline void multiply_matrix_matrix(Transpose transA, Transpose transB, int m,
                                   ...
                                   int lda, const float *B, int ldb, float beta,
                                   float *C, int ldc) {
  sgemm(transA, transB, m, n, k, alpha, (float *)A, lda, (float *)B, ldb, beta,
        C, ldc);
}

inline void multiply_matrix_matrix(int m, int n, int k, const float *A,
                                   const float *B, float *C) {
  sgemm(NoTrans, NoTrans, m, n, k, 1.f, (float *)A, m, (float *)B, k, 0.f, C,
        m);
}  // C=A*B
...
inline void multiply_matrix_matrix(Transpose transA, Transpose transB, int m,
                                   ...
                                   int lda, const double *B, int ldb,
                                   double beta, double *C, int ldc) {
  dgemm(transA, transB, m, n, k, alpha, (double *)A, lda, (double *)B, ldb,
        beta, C, ldc);
}

inline void multiply_matrix_matrix(int m, int n, int k, const double *A,
                                   const double *B, double *C) {
  dgemm(NoTrans, NoTrans, m, n, k, 1., (double *)A, m, (double *)B, k, 0., C,
        m);
}  // C=A*B
...
inline void multiply_matrix_vector(Transpose transpose, int m, int n,
                                   ...
                                   const float *x, int incx, float beta,
                                   float *y, int incy = 1) {
  cblas_sgemv(CblasColMajor, (CBLAS_TRANSPOSE)transpose, m, n, alpha, A, lda, x,
              incx, beta, y, incy);
}
...
inline void multiply_matrix_vector(Transpose transpose, int m, int n,
                                   ...
                                   const double *x, int incx, double beta,
                                   double *y, int incy = 1) {
  cblas_dgemv(CblasColMajor, (CBLAS_TRANSPOSE)transpose, m, n, alpha, A, lda, x,
              incx, beta, y, incy);
}
...
inline void multiply_matrix_matrix(int m, int n, int k, const float *A,
                                   const float *B, float *C) {
  cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m, n, k, 1.f, A, m, B,
              k, 0.f, C, m);
}  // C=A*B
...
inline void multiply_matrix_matrix(int m, int n, int k, const double *A,
                                   const double *B, double *C) {
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m, n, k, 1., A, m, B,
              k, 0., C, m);
}  // C=A*B

inline void rank_one_update(int m, int n, double alpha, const double *x,
...
inline void add_scaled(float alpha, const std::vector<float> &x,
                       std::vector<float> &y) {
  assert(x.size() == y.size());
  add_scaled((int)x.size(), alpha, &x[0], &y[0]);
}
...
// I'm not sure if it makes sense to include level 2 or level 3 std::vector
// versions, since there isn't an STL matrix type...
