...
Target3D::Target3D(float target_[3], float dist_, float heading_, float pitch_,
                   float fovy_, float near_clip_factor_, float far_clip_factor_)
    : dist(dist_),
      heading(heading_),
      pitch(pitch_),
      default_dist(dist_),
      default_heading(heading_),
      default_pitch(pitch_),
      fovy(fovy_),
...
void Target3D::transform_mouse(int x, int y, float ray_origin[3],
  ...
  float camx = (x - 0.5f * winwidth) * scale,
        camy = (0.5f * winheight - y) * scale,
        camz = -1.0f;  // in camera coordinates, this is ray_direction (but not
                       // normalized)
...
void Target3D::get_viewing_direction(float direction[3]) {
  float ch = cos(heading), sh = sin(heading);
  float cp = cos(pitch), sp = sin(pitch);
  direction[0] = -sh * cp;
  direction[1] = sp;
...
void Target3D::gl_transform(void) {
  ...
  glRotatef(-180.0f / (float)M_PI * pitch, 1.0f, 0.0f,
            0.0f);  // rotate pitch in the yz plane
  glRotatef(-180.0f / (float)M_PI * heading, 0.0f, 1.0f,
            0.0f);  // rotate heading in the xz plane
...
TargetOrtho3D::TargetOrtho3D(float target_[3], float dist_, float heading_,
                             float pitch_, float height_factor_,
                             float near_clip_factor_, float far_clip_factor_)
    : dist(dist_),
      heading(heading_),
      pitch(pitch_),
      default_dist(dist_),
  ...
  switch (action_mode) {
    ...
    case ROTATE:
      ...
      if (heading < -M_PI)
        ...
        heading -= 2.0f * (float)M_PI;
      pitch += 0.007f * (oldmousey - y);
      if (pitch < -0.5f * (float)M_PI)
        pitch = -0.5f * (float)M_PI;
      else if (pitch > 0.5f * (float)M_PI)
        pitch = 0.5f * (float)M_PI;
      break;
    case TRUCK:
      target[0] += (0.002f * dist) * cos(heading) * (oldmousex - x);
...
void TargetOrtho3D::transform_mouse(int, int, float * /*ray_origin[3]*/,
                                    float * /*ray_direction[3]*/) {
  // @@@ unimplemented
}

void TargetOrtho3D::get_viewing_direction(float direction[3]) {
  float ch = cos(heading), sh = sin(heading);
  float cp = cos(pitch), sp = sin(pitch);
  direction[0] = -sh * cp;
  direction[1] = sp;
...
void TargetOrtho3D::gl_transform(void) {
  ...
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  float halfheight = 0.5f * height_factor * dist,
        halfwidth = halfheight * winwidth / (float)winheight;
...
PanZoom2D::PanZoom2D(float bottom_, float left_, float height_)
    : bottom(bottom_),
      left(left_),
      height(height_),
      default_bottom(bottom_),
      default_left(left_),
    ...
    switch (action_mode) {
      ...
      case ZOOM_IN:
        if (moved_since_mouse_down) {
          ...
          left +=
              0.5f * (x + clickx) * height / winheight - 0.5f * desired_width;
          bottom += (winheight - 0.5f * (y + clicky)) * height / winheight -
                    0.5f * desired_height;
...
void PanZoom2D::transform_mouse(int x, int y, float coords[2]) {
  float r = height / winheight;
  coords[0] = x * r + left;
...
void PanZoom2D::export_rib(ostream &output) {
  ...
  if (winwidth > winheight)
    scalefactor = 2.0f / height;
  else
    scalefactor = 2.0f / (winwidth * height / winheight);
...
DynamicText::DynamicText(const std::string &text_) : text(text_) {
  color[0] = 0.0f;
  color[1] = 0.0f;
  color[2] = 0.0f;
}
...
void DynamicText::display(int x, int y) {
  dispx = x;
  dispy = y;
  width = glutBitmapLength(GLUT_BITMAP_HELVETICA_12,
...
void draw_3d_arrow(const float base[3], const float point[3],
  ...
  if (mag(u) == 0) u = cross(w, Vec3f(1, 0, 0));

  // get a vector perp to w and u
  Vec3f v = cross(w, u / mag(u));
  v = v / mag(v);

  if (!arrow_head_length) arrow_head_length = 0.1f * len;
...
void draw_text(const float * /*point[3]*/, const char * /*text*/,
               int /*fontsize*/) {
  // please implement me!
}
