...
//
//  Tunicate-based implementation of collision and intersection queries.  (See
//  Robert Bridson's "Tunicate" library.)
//
// ---------------------------------------------------------
...
bool tunicate_point_segment_collision(const Vec2d& x0, const Vec2d& xnew0,
                                      size_t index0, const Vec2d& x1,
                                      const Vec2d& xnew1, size_t index1,
...
bool tunicate_point_segment_collision(const Vec2d& x0, const Vec2d& xnew0,
                                      ...
                                      const Vec2d& xnew1, size_t index1,
                                      const Vec2d& x2, const Vec2d& xnew2,
                                      size_t index2, double& edge_alpha,
                                      Vec2d& normal, double& time,
                                      double& relative_normal_displacement);

bool tunicate_point_triangle_collision(
    ...
    double& bary1, double& bary2, double& bary3, Vec3d& normal, double& t,
    double& relative_normal_displacement, bool verbose);

bool tunicate_point_triangle_collision(const Vec3d& x0, const Vec3d& xnew0,
...
bool tunicate_segment_segment_collision(
    const Vec3d& x0, const Vec3d& xnew0, size_t index0, const Vec3d& x1,
    const Vec3d& xnew1, size_t index1, const Vec3d& x2, const Vec3d& xnew2,
    size_t index2, const Vec3d& x3, const Vec3d& xnew3, size_t index3,
    double& bary0, double& bary2, Vec3d& normal, double& t,
    double& relative_normal_displacement, bool verbose);

bool tunicate_segment_segment_collision(const Vec3d& x0, const Vec3d& xnew0,
                                        size_t index0, const Vec3d& x1,
                                        const Vec3d& xnew1, size_t index1,
                                        const Vec3d& x2, const Vec3d& xnew2,
                                        size_t index2, const Vec3d& x3,
...
bool tunicate_point_segment_collision(const Vec2d& x0, const Vec2d& xnew0,
  ...
  if (simplex_intersection3d(segment_triangle_test, p0, pnew0, p1, p2, pnew2,
                             &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4])) {
    intersections[0] = true;
  }

  if (simplex_intersection3d(segment_triangle_test, p0, pnew0, p1, pnew1, pnew2,
                             &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4])) {
...
bool tunicate_point_triangle_collision(const Vec3d& x0, const Vec3d& xnew0,
  ...
  if (simplex_intersection4d(segment_tetrahedron_test, p0, pnew0, p1, p2, p3,
                             pnew3, &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4], &bary[5])) {
    ++num_intersections;
  }

  if (simplex_intersection4d(segment_tetrahedron_test, p0, pnew0, p1, p2, pnew2,
                             pnew3, &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4], &bary[5])) {
    ++num_intersections;
  }

  if (simplex_intersection4d(segment_tetrahedron_test, p0, pnew0, p1, pnew1,
                             pnew2, pnew3, &bary[0], &bary[1], &bary[2],
...
bool tunicate_point_triangle_collision(
    const Vec3d& x0, const Vec3d& xnew0, size_t, const Vec3d& x1,
    const Vec3d& xnew1, size_t index1, const Vec3d& x2, const Vec3d& xnew2,
    size_t index2, const Vec3d& x3, const Vec3d& xnew3, size_t index3,
    double& bary1, double& bary2, double& bary3, Vec3d& normal, double& t,
    double& relative_normal_displacement, bool /*verbose*/) {
  ...
  if (simplex_intersection4d(segment_tetrahedron_test, p0, pnew0, p1, p2, pnew2,
    ...
    if (bary[1] < t) {
      bary1 = 0;
      bary2 = (bary[4] + 1e-300) /
              (bary[4] + bary[5] + 2e-300);  // guard against zero/zero
      bary3 = 1 - bary2;
      t = bary[1];
  ...
  if (simplex_intersection4d(segment_tetrahedron_test, p0, pnew0, p1, pnew1,
    ...
    if (bary[1] < t) {
      bary1 = (bary[3] + 1e-300) / (bary[3] + bary[4] + bary[5] +
                                    3e-300);  // guard against zero/zero
      bary2 = (bary[4] + 1e-300) / (bary[3] + bary[4] + bary[5] +
                                    3e-300);  // guard against zero/zero
      bary3 = 1 - bary1 - bary2;
      t = bary[1];
  ...
  if (tunicate_verbose) {
    std::cout << "point-triangle, num_intersections: " << num_intersections
              << std::endl;
  }
...
bool tunicate_segment_segment_collision(const Vec3d& x0, const Vec3d& xnew0,
  ...
  if (simplex_intersection4d(triangle_triangle_test, p0, p1, pnew1, p2, p3,
                             pnew3, &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4], &bary[5])) {
    ++num_intersections;
  }

  if (simplex_intersection4d(triangle_triangle_test, p0, pnew0, pnew1, p2, p3,
                             pnew3, &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4], &bary[5])) {
    ++num_intersections;
  }

  if (simplex_intersection4d(triangle_triangle_test, p0, p1, pnew1, p2, pnew2,
                             pnew3, &bary[0], &bary[1], &bary[2], &bary[3],
                             &bary[4], &bary[5])) {
    ++num_intersections;
  }

  if (simplex_intersection4d(triangle_triangle_test, p0, pnew0, pnew1, p2,
                             pnew2, pnew3, &bary[0], &bary[1], &bary[2],
                             &bary[3], &bary[4], &bary[5])) {
    ++num_intersections;
  }
...
bool tunicate_segment_segment_collision(
  ...
  if (simplex_intersection4d(triangle_triangle_test, p0, p1, pnew1, p2, p3,
    ...
    if (tunicate_verbose) {
      ...
      std::cout << bary[0] << " " << bary[1] << " " << bary[2] << " " << bary[3]
                << " " << bary[4] << " " << bary[5] << std::endl;
    }
  ...
  if (simplex_intersection4d(triangle_triangle_test, p0, pnew0, pnew1, p2, p3,
    ...
    if (bary[5] < t) {
      bary0 = (bary[1] + 1e-300) /
              (bary[1] + bary[2] + 2e-300);  // guard against zero/zero
      bary2 = 0;
      t = bary[5];
...
bool point_segment_collision(const Vec2d& x0, const Vec2d& xnew0, size_t index0,
                             ...
                             const Vec2d& x2, const Vec2d& xnew2,
                             size_t index2) {
  return tunicate_point_segment_collision(x0, xnew0, index0, x1, xnew1, index1,
                                          x2, xnew2, index2);
}
...
bool point_segment_collision(const Vec2d& x0, const Vec2d& xnew0, size_t index0,
                             ...
                             double& edge_alpha, Vec2d& normal, double& time,
                             double& relative_normal_displacement) {
  bool tunicate_result = tunicate_point_segment_collision(
      x0, xnew0, index0, x1, xnew1, index1, x2, xnew2, index2, edge_alpha,
      normal, time, relative_normal_displacement);
...
bool segment_segment_intersection(const Vec2d& x0, size_t /*index0*/,
  ...
  return simplex_intersection2d(2, x0.v, x1.v, x2.v, x3.v, &bary[0], &bary[1],
                                &bary[2], &bary[3]);
}
...
bool point_triangle_collision(const Vec3d& x0, const Vec3d& xnew0,
                              ...
                              size_t index2, const Vec3d& x3,
                              const Vec3d& xnew3, size_t index3) {
  bool tunicate_result =
      tunicate_point_triangle_collision(x0, xnew0, index0, x1, xnew1, index1,
                                        x2, xnew2, index2, x3, xnew3, index3);
...
bool point_triangle_collision(const Vec3d& x0, const Vec3d& xnew0,
                              ...
                              const Vec3d& x2, const Vec3d& xnew2,
                              size_t index2, const Vec3d& x3,
                              const Vec3d& xnew3, size_t index3, double& bary1,
                              double& bary2, double& bary3, Vec3d& normal,
                              double& relative_normal_displacement) {
  double time;
  bool verbose = false;
  bool tunicate_result = tunicate_point_triangle_collision(
      x0, xnew0, index0, x1, xnew1, index1, x2, xnew2, index2, x3, xnew3,
      index3, bary1, bary2, bary3, normal, time, relative_normal_displacement,
      verbose);
...
bool segment_segment_collision(const Vec3d& x0, const Vec3d& xnew0,
                               ...
                               const Vec3d& xnew3, size_t index3) {
  bool tunicate_result =
      tunicate_segment_segment_collision(x0, xnew0, index0, x1, xnew1, index1,
                                         x2, xnew2, index2, x3, xnew3, index3);
...
bool segment_segment_collision(const Vec3d& x0, const Vec3d& xnew0,
                               ...
                               const Vec3d& x2, const Vec3d& xnew2,
                               size_t index2, const Vec3d& x3,
                               const Vec3d& xnew3, size_t index3, double& bary0,
                               double& bary2, Vec3d& normal,
                               double& relative_normal_displacement) {
  double time;
  bool verbose = false;
  bool tunicate_result = tunicate_segment_segment_collision(
      x0, xnew0, index0, x1, xnew1, index1, x2, xnew2, index2, x3, xnew3,
      index3, bary0, bary2, normal, time, relative_normal_displacement,
      verbose);
