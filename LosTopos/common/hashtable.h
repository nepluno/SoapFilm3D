#ifndef LOSTOPOS_HASHTABLE_H
#define LOSTOPOS_HASHTABLE_H
...
inline unsigned int hash(unsigned int k) { return k * 2654435769u; }
...
template <typename Key, typename Data, class HashFunction = DefaultHashFunction,
          class KeyEqual = my_equal>
struct HashTable {
  ...
  explicit HashTable(unsigned int expected_size = 64)
      : table_rank(static_cast<unsigned int>(~0)),
        table_bits(static_cast<unsigned int>(~0)),
        table(0),
        num_entries(static_cast<unsigned int>(~0)),
        pool(),
        free_list(static_cast<unsigned int>(~0)),
  ...
  void add(const Key &k, const Data &d) {
    ...
    pool[i].data = d;
    pool[i].next =
        table[t];  // put the new entry at the start of table[t]'s list
    table[t] = i;
    ++num_entries;
  }
  ...
  void delete_entry(
      const Key &k,
      const Data &d)  // delete first entry that matches both key and data
  {
  ...
  unsigned int size() const { return num_entries; }
  ...
  bool has_entry(const Key &k) const {
    ...
    int i = table[t];
    while (i != -1) {
      if (key_equal(k, pool[i].key)) return true;
      i = pool[i].next;
    }
    return false;
  }
  ...
  Data &operator()(const Key &k, const Data &missing_data) {
    ...
    int i = table[t];
    while (i != -1) {
      if (key_equal(k, pool[i].key)) return pool[i].data;
      i = pool[i].next;
    }
    add(k, missing_data);  // note - this could cause the table to be resized,
                           // and t made out-of-date
    return pool[table[hash_function(k) & table_bits]]
        .data;  // we know that add() puts it here!
  }
  ...
  void output_statistics() const {
    ...
    for (t = 0; t < lengthcount.size() && t < 10; ++t) {
      ...
      if (lengthcount[t] > 0) {
        std::cout << "length " << t << ": " << lengthcount[t] << "   ("
                  << lengthcount[t] / (float)total * 100.0 << "%)" << std::endl;
        maxlength = t;
      }
    }
    std::cout << "rest: " << total - subtotal << "   ("
              << 100.0 * (1.0 - subtotal / (float)total) << "%)" << std::endl;
    for (; t < lengthcount.size(); ++t)
      if (lengthcount[t] > 0) maxlength = t;
