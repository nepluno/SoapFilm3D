...
//==============================================================================
bool is_zero(const expansion& a) { return (a.v.size() == 0); }
...
int sign(const expansion& a) {
  ...
  // REVIEW: I'm assuming we can get the sign of the expansion by the sign of
  // its leading term (i.e. the sum of all other terms < leading term ) This
  // true if the expansion if increasing and nonoverlapping
  if (a.v.back() > 0) {
    return 1;
  }
  return -1;
...
void negative(const expansion& input, expansion& output) {
  ...
  for (unsigned int i = 0; i < input.v.size(); ++i) output.v[i] = -input.v[i];
}
...
double estimate(const expansion& a) {
  double x = 0;
  for (unsigned int i = 0; i < a.v.size(); ++i) x += a.v[i];
  return x;
}
