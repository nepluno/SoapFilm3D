...
/// Tolerance for static distance query at coplanarity time to be considered a
/// collision
const double g_collision_epsilon = 1e-6;

/// Check if segment x0-x1 and segment x2-x3 are intersecting and return
/// barycentric coordinates of intersection if so
///
bool check_edge_edge_intersection(const Vec2d &x0, const Vec2d &x1,
                                  const Vec2d &x2, const Vec2d &x3, double &s01,
                                  double &s23, double tolerance);

/// Find the roots in [0,1] of the specified quadratic (append to possible_t).
///
void find_possible_quadratic_roots_in_01(double A, double B, double C,
                                         std::vector<double> &possible_t,
                                         double tol);

/// Check if point x0 collides with segment x1-x2 during the motion from old to
/// new positions.
///
bool check_point_edge_collision(const Vec2d &x0old, const Vec2d &x1old,
                                const Vec2d &x2old, const Vec2d &x0new,
                                const Vec2d &x1new, const Vec2d &x2new,
                                double collision_epsilon);

/// Check if point x0 collides with segment x1-x2 during the motion from old to
/// new positions. Return the barycentric coordinates, collision normal, and
/// time if so.
///
bool check_point_edge_collision(const Vec2d &x0old, const Vec2d &x1old,
                                const Vec2d &x2old, const Vec2d &x0new,
                                const Vec2d &x1new, const Vec2d &x2new,
                                double &s12, Vec2d &normal,
                                double &collision_time,
                                double collision_epsilon);

/// Find the possible coplanarity times of the four vertices with trajectories
/// specified by x and xnew in the range [0,1].
///
void find_coplanarity_times(const Vec3d &x0, const Vec3d &x1, const Vec3d &x2,
                            const Vec3d &x3, const Vec3d &xnew0,
                            const Vec3d &xnew1, const Vec3d &xnew2,
                            const Vec3d &xnew3,
                            std::vector<double> &possible_times);

/// Check if segment x0-x1 collides with segment x2-x3 during the motion from
/// old to new positions.
///
bool check_edge_edge_collision(const Vec3d &x0, const Vec3d &x1,
                               const Vec3d &x2, const Vec3d &x3,
                               const Vec3d &xnew0, const Vec3d &xnew1,
                               const Vec3d &xnew2, const Vec3d &xnew3,
                               double collision_epsilon);

/// If the collision normal found during CCD testing is degenerate, this
/// function will attempt to pick a suitable normal.
///
...
/// Check if segment x0-x1 collides with segment x2-x3 during the motion from
/// old to new positions. Return the barycentric coordinates, collision normal,
/// and time if so.
///
bool check_edge_edge_collision(const Vec3d &x0, const Vec3d &x1,
                               const Vec3d &x2, const Vec3d &x3,
                               const Vec3d &xnew0, const Vec3d &xnew1,
                               const Vec3d &xnew2, const Vec3d &xnew3,
                               double &s0, double &s2, Vec3d &normal, double &t,
                               double collision_epsilon);

/// Check if point x0 collides with triangle x1-x2-x3 during the motion from old
/// to new positions.
///
bool check_point_triangle_collision(const Vec3d &x0, const Vec3d &x1,
                                    const Vec3d &x2, const Vec3d &x3,
                                    const Vec3d &xnew0, const Vec3d &xnew1,
                                    const Vec3d &xnew2, const Vec3d &xnew3,
                                    double collision_epsilon);
...
/// Check if point x0 collides with triangle x1-x2-x3 during the motion from old
/// to new positions.  Return the barycentric coordinates, collision normal, and
/// time if so.
///
bool check_point_triangle_collision(const Vec3d &x0, const Vec3d &x1,
                                    const Vec3d &x2, const Vec3d &x3,
...
void find_possible_quadratic_roots_in_01(double A, double B, double C,
  ...
  if (A != 0) {
    ...
    if (discriminant > 0) {
      double numer;
      if (B > 0)
        numer = 0.5 * (-B - sqrt(discriminant));
      else
        numer = 0.5 * (-B + sqrt(discriminant));
      double t0 = numer / A, t1 = C / numer;
      if (t0 < t1) {
        if (t0 >= -tol && t0 < 1) possible_t.push_back(max(0., t0));
        if (t1 >= -tol && t1 < 1) possible_t.push_back(max(0., t1));
      } else {
        if (t1 >= -tol && t1 < 1) possible_t.push_back(max(0., t1));
        if (t0 >= -tol && t0 < 1) possible_t.push_back(max(0., t0));
      }
      ...
      if (t >= -tol && t < 1) possible_t.push_back(max(0., t));
    }
  } else if (B != 0) {
    double t = -C / B;
    if (t >= -tol && t < 1) possible_t.push_back(max(0., t));
...
bool check_point_edge_collision(const Vec2d &x0old, const Vec2d &x1old,
  ...
  std::vector<double> possible_t;
  double A = cross(d10, d20), B = cross(d10, x20) + cross(x10, d20),
         C = cross(x10, x20);
  ...
  double proximity_tol =
      collision_epsilon * std::sqrt(mag2(x0old) + mag2(x0new) + mag2(x1old) +
                                    mag2(x1new) + mag2(x2new) + mag2(x2old));
  for (size_t i = 0; i < possible_t.size(); ++i) {
    double collision_time = possible_t[i];
    double u = 1 - collision_time;
    double distance;
    check_point_edge_proximity(false, u * x0old + collision_time * x0new,
                               u * x1old + collision_time * x1new,
                               u * x2old + collision_time * x2new, distance);
...
bool check_point_edge_collision(const Vec2d &x0old, const Vec2d &x1old,
                                ...
                                const Vec2d &x1new, const Vec2d &x2new,
                                double &s12, Vec2d &normal,
                                double &collision_time, double tol) {
  Vec2d x10 = x1old - x0old, x20 = x2old - x0old;
  Vec2d d10 = (x1new - x0new) - x10, d20 = (x2new - x0new) - x20;
  // figure out possible collision times to check
  std::vector<double> possible_t;
  ...
  // check proximities at possible collision times
  double proximity_tol =
      tol * std::sqrt(mag2(x0old) + mag2(x0new) + mag2(x1old) + mag2(x1new) +
                      mag2(x2new) + mag2(x2old));
  for (size_t i = 0; i < possible_t.size(); ++i) {
...
void find_coplanarity_times(const Vec3d &x0, const Vec3d &x1, const Vec3d &x2,
  ...
  Vec3d v03 = (xnew0 - xnew3) - x03, v13 = (xnew1 - xnew3) - x13,
        v23 = (xnew2 - xnew3) - x23;
  double A = triple(v03, v13, v23),
         B = triple(x03, v13, v23) + triple(v03, x13, v23) +
             triple(v03, v13, x23),
         C = triple(x03, x13, v23) + triple(x03, v13, x23) +
             triple(v03, x13, x23),
  ...
  if (discriminant <= 0) {  // monotone cubic: only one root in [0,1] possible
    ...
    if (simplex_verbose) {
      std::cout << "monotone cubic" << std::endl;
    }
    ...
    if (A == 0) {  // the cubic is just a quadratic, B*t^2+C*t+D
      ...
      if (discriminant <= 0) {
        ...
        if (t >= -g_cubic_solver_tol && t <= 1 + g_cubic_solver_tol) {
          t = clamp(t, 0., 1.);
          if (std::fabs(LosTopos::signed_volume(
        ...
        t1 = D / (B * t0);
        if (t1 < t0) swap(t0, t1);
        if (t0 >= -g_cubic_solver_tol && t0 <= 1 + g_cubic_solver_tol)
          possible_times.push_back(clamp(t0, 0., 1.));
        if (t1 >= -g_cubic_solver_tol && t1 <= 1 + g_cubic_solver_tol)
          add_unique(possible_times, clamp(t1, 0., 1.));
      }
      ...
      if (simplex_verbose) {
        std::cout << "interval times: " << t0 << ", " << t1 << std::endl;
      }

      interval_times.push_back(0);
      if (t0 > 0 && t0 < 1) interval_times.push_back(t0);
      if (t1 > 0 && t1 < 1) interval_times.push_back(t1);
      interval_times.push_back(1);
    }
  }
  ...
  if (simplex_verbose) {
    ...
    for (unsigned int i = 0; i < n_samples; ++i) {
      ...
      double sample_val =
          LosTopos::signed_volume((1 - sample_t) * x0 + sample_t * xnew0,
  ...
  for (size_t i = 0; i < interval_times.size(); ++i) {
    ...
    interval_values[i] = LosTopos::signed_volume(
        (1 - t) * x0 + t * xnew0, (1 - t) * x1 + t * xnew1,
        (1 - t) * x2 + t * xnew2, (1 - t) * x3 + t * xnew3);
    if (simplex_verbose) {
      std::cout << "interval time: " << t << ", value: " << interval_values[i]
                << std::endl;
    }
  }
  ...
  for (size_t i = 1; i < interval_times.size(); ++i) {
    ...
    if ((vlo < 0 && vhi > 0) || (vlo > 0 && vhi < 0)) {
      ...
      if (simplex_verbose) {
        std::cout << "cubic solver tol: " << 1e-2 * convergence_tol
                  << std::endl;
      }

      for (; iteration < 50; ++iteration) {
        vmid = LosTopos::signed_volume(
...
bool check_edge_edge_collision(const Vec3d &x0, const Vec3d &x1,
  ...
  for (size_t a = 0; a < possible_times.size(); ++a) {
    ...
    Vec3d xt0 = (1 - t) * x0 + t * xnew0, xt1 = (1 - t) * x1 + t * xnew1,
          xt2 = (1 - t) * x2 + t * xnew2, xt3 = (1 - t) * x3 + t * xnew3;
    double distance;
...
bool check_edge_edge_collision(const Vec3d &x0, const Vec3d &x1,
  ...
  for (size_t a = 0; a < possible_times.size(); ++a) {
    ...
    Vec3d xt0 = (1 - t) * x0 + t * xnew0, xt1 = (1 - t) * x1 + t * xnew1,
          xt2 = (1 - t) * x2 + t * xnew2, xt3 = (1 - t) * x3 + t * xnew3;
    double distance;
    LosTopos::check_edge_edge_proximity(xt0, xt1, xt2, xt3, distance, s0, s2,
                                        normal);
    if (distance < collision_epsilon) {
      // now figure out a decent normal
      if (distance <
          1e-2 * g_degen_normal_epsilon) {  // if we don't trust the normal...
        ...
        if (m > sqr(g_degen_normal_epsilon)) {
          ...
          degenerate_get_edge_edge_collision_normal(x0, x1, x2, x3, s0, s2,
                                                    normal);
        }
      }
      return true;
...
bool check_point_triangle_collision(const Vec3d &x0, const Vec3d &x1,
  ...
  for (size_t a = 0; a < possible_times.size(); ++a) {
    ...
    Vec3d xt0 = (1 - t) * x0 + t * xnew0, xt1 = (1 - t) * x1 + t * xnew1,
          xt2 = (1 - t) * x2 + t * xnew2, xt3 = (1 - t) * x3 + t * xnew3;
    double distance;
    LosTopos::check_point_triangle_proximity(xt0, xt1, xt2, xt3, distance, s1,
                                             s2, s3, normal);
    if (distance < collision_epsilon) {
      // now figure out a decent normal
      if (distance <
        ...
        if (m > sqr(g_degen_normal_epsilon)) {
          ...
          degenerate_get_point_triangle_collision_normal(x0, x1, x2, x3, s1, s2,
                                                         s3, normal);
        }
      }
      return true;
...
bool triangle_intersects_segment(const Vec3d &x1, const Vec3d &x2,
                                 const Vec3d &x3, const Vec3d &x4,
                                 const Vec3d &x5, double a, double b, double c,
                                 double d, double e, double /*tolerance*/,
                                 bool verbose, bool &degenerate) {
  ...
  // note: using the triangle edges in the first two spots guarantees the same
  // floating point result (up to sign) if we transpose the triangle vertices --
  // e.g. testing an adjacent triangle -- so this triangle-line test is
  // watertight.
  ...
  if (std::fabs(sum_abc) < machine_epsilon) {
    if (verbose) {
      std::cout << "sum_abc degenerate" << std::endl;
    }
    degenerate = true;
  ...
  if (std::fabs(sum_de) < machine_epsilon) {
    if (verbose) {
      std::cout << "sum_de degenerate" << std::endl;
    }
    degenerate = true;
    return false;  // degenerate situation
  }

  if ((std::fabs(a) < machine_epsilon) || (std::fabs(b) < machine_epsilon) ||
      (std::fabs(c) < machine_epsilon)) {
    if (verbose) {
      std::cout << "degenerate: a = " << a << ", b = " << b << ", c = " << c
                << std::endl;
    }
    degenerate = true;
  }
  ...
  if (verbose) {
    std::cout << "normalized coords: " << a << " " << b << " " << c << " " << d
              << " " << e << std::endl;
  }
...
// ---------------------------------------------------------
///
/// Return true if triangle (xtri0,xtri1,xtri2) intersects segment (xedge0,
/// xedge1), within the specified tolerance. If
/// degenerate_counts_as_intersection is true, this function will return true in
/// a degenerate situation.
///
// ---------------------------------------------------------

bool check_edge_triangle_intersection(
    const Vec3d &xedge0, const Vec3d &xedge1, const Vec3d &xtri0,
    const Vec3d &xtri1, const Vec3d &xtri2, double bary_e0, double bary_e1,
    double bary_t0, double bary_t1, double bary_t2, double tolerance,
    bool degenerate_counts_as_intersection, bool verbose) {
  bool is_degenerate;
  if (triangle_intersects_segment(xtri0, xtri1, xtri2, xedge0, xedge1, bary_t0,
                                  bary_t1, bary_t2, bary_e0, bary_e1, tolerance,
                                  verbose, is_degenerate)) {
    if (is_degenerate) {
...
/// Returns true if vertex proximity to any of the tet's faces is less than
/// epsilon.
///
// ---------------------------------------------------------

bool vertex_is_in_tetrahedron(const Vec3d &p, const Vec3d &x1, const Vec3d &x2,
                              const Vec3d &x3, const Vec3d &x4,
                              double epsilon) {
  double distance;
  ...
  // if there was a degenerate case, but the point was not in any triangle, the
  // point must be outside the tet
  if ((std::fabs(a) < epsilon) || (std::fabs(b) < epsilon) ||
      (std::fabs(c) < epsilon) || (std::fabs(d) < epsilon)) {
    return false;
  }
...
bool point_segment_collision(const Vec2d &x0, const Vec2d &xnew0, size_t,
                             ...
                             double &edge_alpha, Vec2d &normal,
                             double &rel_disp) {
  double t;
  bool result =
      check_point_edge_collision(x0, x1, x2, xnew0, xnew1, xnew2, edge_alpha,
...
bool point_segment_collision(const Vec2d &x0, const Vec2d &xnew0, size_t,
                             const Vec2d &x1, const Vec2d &xnew1, size_t,
                             const Vec2d &x2, const Vec2d &xnew2, size_t) {
  bool result = check_point_edge_collision(x0, x1, x2, xnew0, xnew1, xnew2,
                                           g_collision_epsilon);
  return result;
}
...
bool segment_segment_intersection(const Vec2d &x0, size_t, const Vec2d &x1,
                                  size_t, const Vec2d &x2, size_t,
                                  const Vec2d &x3, size_t) {
  double s0, s2;
  return check_edge_edge_intersection(x0, x1, x2, x3, s0, s2,
                                      g_collision_epsilon);
}

bool segment_segment_intersection(const Vec2d &x0, size_t, const Vec2d &x1,
                                  size_t, const Vec2d &x2, size_t,
                                  const Vec2d &x3, size_t, double &s0,
                                  double &s2) {
  return check_edge_edge_intersection(x0, x1, x2, x3, s0, s2,
                                      g_collision_epsilon);
}
...
bool point_triangle_collision(const Vec3d &x0, const Vec3d &xnew0, size_t,
  ...
  bool cubic_result = check_point_triangle_collision(
      x0, x1, x2, x3, xnew0, xnew1, xnew2, xnew3, g_collision_epsilon);
  return cubic_result;
}
...
bool point_triangle_collision(const Vec3d &x0, const Vec3d &xnew0, size_t,
  ...
  double t;
  bool cubic_result = check_point_triangle_collision(
      x0, x1, x2, x3, xnew0, xnew1, xnew2, xnew3, bary1, bary2, bary3, normal,
      t, g_collision_epsilon);
...
bool segment_segment_collision(const Vec3d &x0, const Vec3d &xnew0, size_t,
                               ...
                               const Vec3d &x2, const Vec3d &xnew2, size_t,
                               const Vec3d &x3, const Vec3d &xnew3, size_t) {
  bool cubic_result = check_edge_edge_collision(
      x0, x1, x2, x3, xnew0, xnew1, xnew2, xnew3, g_collision_epsilon);
...
bool segment_segment_collision(const Vec3d &x0, const Vec3d &xnew0, size_t,
  ...
  double t;
  bool cubic_result =
      check_edge_edge_collision(x0, x1, x2, x3, xnew0, xnew1, xnew2, xnew3,
                                bary0, bary2, normal, t, g_collision_epsilon);
...
bool segment_triangle_intersection(const Vec3d &x0, size_t, const Vec3d &x1,
                                   size_t, const Vec3d &x2, size_t,
                                   const Vec3d &x3, size_t, const Vec3d &x4,
                                   size_t,
                                   bool degenerate_counts_as_intersection,
                                   bool verbose) {
  double bary[5] = {0, 0, 0, 0, 0};
  return check_edge_triangle_intersection(
      x0, x1, x2, x3, x4, bary[0], bary[1], bary[2], bary[3], bary[4],
...
bool segment_triangle_intersection(const Vec3d &x0, size_t, const Vec3d &x1,
                                   size_t, const Vec3d &x2, size_t,
                                   const Vec3d &x3, size_t, const Vec3d &x4,
                                   size_t, double &bary0, double &bary1,
                                   double &bary2, double &bary3, double &bary4,
                                   bool degenerate_counts_as_intersection,
                                   bool verbose) {
  return check_edge_triangle_intersection(
      x0, x1, x2, x3, x4, bary0, bary1, bary2, bary3, bary4,
      g_collision_epsilon, degenerate_counts_as_intersection, verbose);
}

/// x0 is the point and x1-x2-x3-x4 is the tetrahedron. Order is irrelevant.
bool point_tetrahedron_intersection(const Vec3d &x0, size_t, const Vec3d &x1,
                                    size_t, const Vec3d &x2, size_t,
                                    const Vec3d &x3, size_t, const Vec3d &x4,
                                    size_t) {
