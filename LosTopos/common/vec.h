...
template <unsigned int N, class T>
void zero(Vec<N, T> &a) {
  for (unsigned int i = 0; i < N; ++i) a.v[i] = 0;
}

template <unsigned int N, class T>
std::ostream &operator<<(std::ostream &out, const Vec<N, T> &v) {
  out << v.v[0];
  for (unsigned int i = 1; i < N; ++i) out << ' ' << v.v[i];
  return out;
}
...
std::istream &operator>>(std::istream &in, Vec<N, T> &v) {
  in >> v.v[0];
  for (unsigned int i = 1; i < N; ++i) in >> v.v[i];
  return in;
}
...
template <class T>
inline Vec<3, T> rotate(const Vec<3, T> &x, const T &angle,
                        const Vec<3, T> &u) {
  T ux = u[0] * x[0];
  T uy = u[0] * x[1];
  T uz = u[0] * x[2];
  ...
  return Vec<3, T>(
      u[0] * (ux + vy + wz) +
          (x[0] * (u[1] * u[1] + u[2] * u[2]) - u[0] * (vy + wz)) * ca +
          (-wy + vz) * sa,
      u[1] * (ux + vy + wz) +
          (x[1] * (u[0] * u[0] + u[2] * u[2]) - u[1] * (ux + wz)) * ca +
          (wx - uz) * sa,
      u[2] * (ux + vy + wz) +
          (x[2] * (u[0] * u[0] + u[1] * u[1]) - u[2] * (ux + vy)) * ca +
          (-vx + uy) * sa);
}
...
inline unsigned int hash(const Vec<N, T> &a) {
  ...
  for (unsigned int i = 1; i < N; ++i) h = hash(h ^ a.v[i]);
  return h;
}
...
inline Vec<N, int> round(const Vec<N, T> &a) {
  Vec<N, int> rounded;
  for (unsigned int i = 0; i < N; ++i) rounded.v[i] = lround(a.v[i]);
  return rounded;
}
...
inline Vec<N, int> floor(const Vec<N, T> &a) {
  Vec<N, int> rounded;
  for (unsigned int i = 0; i < N; ++i) rounded.v[i] = (int)floor(a.v[i]);
  return rounded;
}
...
inline Vec<N, int> ceil(const Vec<N, T> &a) {
  Vec<N, int> rounded;
  for (unsigned int i = 0; i < N; ++i) rounded.v[i] = (int)ceil(a.v[i]);
  return rounded;
}
...
inline Vec<N, T> fabs(const Vec<N, T> &a) {
  Vec<N, T> result;
  for (unsigned int i = 0; i < N; ++i) result.v[i] = (T)std::fabs(a.v[i]);
  return result;
}
