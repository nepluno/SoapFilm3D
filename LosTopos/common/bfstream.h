...
inline void swap_endianity(T &x) {
  assert(sizeof(T) <= 8);  // should not be called on composite types: instead
                           // specialize swap_endianity if needed.
  T old = x;
...
struct bifstream {
  ...
  void assert_correct_endianity(void) {
    ...
    assert(
        *(char *)&test ==
        0);  // if this fails, you should have defined __LITTLE_ENDIAN__ instead
#else
    assert(*(char *)&test ==
           1);  // if this fails, you should have defined __BIG_ENDIAN__ instead
#endif
  }
  ...
  bool good(void) { return input.good(); }

  bool fail(void) { return input.fail(); }

  void close(void) { input.close(); }

  void set_big_endian(void) { big_endian = true; }

  void set_little_endian(void) { big_endian = false; }

  void read_endianity(void) { (*this) >> big_endian; }

  void skip(long numbytes) { input.seekg(numbytes, std::ios_base::cur); }

  void seek(long position) { input.seekg(position); }

  int get(void) { return input.get(); }
...
struct bofstream {
  ...
  bool good(void) { return output.good(); }

  bool fail(void) { return output.fail(); }

  void close(void) { output.close(); }

  void set_big_endian(void) { big_endian = true; }

  void set_little_endian(void) { big_endian = false; }

  void write_endianity(void) { (*this) << big_endian; }

  void write_zero(unsigned int numbytes) {
    for (unsigned int i = 0; i < numbytes; ++i) output.put(0);
  }

  void put(char byte) { output.put(byte); }
