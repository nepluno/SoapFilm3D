...
struct LinearOperator {
  ...
  virtual void apply_and_subtract(const std::vector<double> &x,
  ...
  virtual void apply_transpose(const std::vector<double> &x,
                               std::vector<double> &y) const {
  ...
  virtual void apply_transpose_and_subtract(const std::vector<double> &x,
                                            const std::vector<double> &y,
                                            std::vector<double> &z) const {
  ...
  virtual void write_matlab(std::ostream &output,
    ...
    output << variable_name << "='unimplemented LinearOperator output';"
           << std::endl;
  }
};

// useful extra: compositions of linear operators (e.g. for factored
// preconditioners)
struct FactoredLinearOperator : public LinearOperator {
  std::vector<const LinearOperator *> factors;
  std::vector<bool> transpose;

  FactoredLinearOperator(int m_ = 0)
      : LinearOperator(m_), factors(0), transpose(0), temp(0) {}
  FactoredLinearOperator(const LinearOperator *A, bool Atranspose = false);
  FactoredLinearOperator(const LinearOperator *A, bool Atranspose,
                         const LinearOperator *B, bool Btranspose);
  FactoredLinearOperator(const LinearOperator *A, bool Atranspose,
                         const LinearOperator *B, bool Btranspose,
                         const LinearOperator *C, bool Ctranspose);
 ...
 private:
  std::vector<double>
      temp;  // intermediate vector for use in the apply functions if needed
};
