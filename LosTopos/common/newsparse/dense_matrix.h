...
struct DenseMatrix : public LinearOperator {
  ...
  DenseMatrix(int m_ = 0) : LinearOperator(m_, m_), value(m_ * m_, 0) {}

  DenseMatrix(int m_, int n_) : LinearOperator(m_, n_), value(m_ * n_, 0) {}
  ...
  virtual void apply_and_subtract(const double *x, const double *y,
                                  double *z) const;
  virtual void apply_transpose(const double *input_vector,
                               double *output_vector) const;
