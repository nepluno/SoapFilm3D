...
// and if the system is inconsistent convergence will not be automatically
// detected.
struct CG_Solver {
  ...
  CG_Solver(void)
      : tolerance_factor(1e-9),
        max_iterations(100),
        residual_norm(0),
        iteration(0),
        status(KRYLOV_CONVERGED),
        r(0),
        z(0),
        s(0) {}
  ...
  // If "use_given_initial_guess" is false (default), take initial guess to be
  // all zeros;
  //  if true, instead use whatever is in result at the moment.
  KrylovSolverStatus solve(const LinearOperator &A, const double *rhs,
                           double *result,
...
struct CGNR_Solver {
  double tolerance_factor;
  unsigned int max_iterations;
  double residual_norm;  // we use the infinity norm of the residual in the
                         // normal equations, A^T*(b-A*x)
  ...
  // Attempt to solve min ||rhs-A*result|| (or A*result=rhs for nonsingular A)
  // for result. Sets residual_norm, iteration, and status (and also returns
  // status) If optional preconditioner given, use it; it should precondition A,
  // so that A*M is close to orthogonal. If "use_given_initial_guess" is false
  // (default), take initial guess to be all zeros;
  //  if true, instead use whatever is in result at the moment.
  KrylovSolverStatus solve(const LinearOperator &A, const double *rhs,
                           double *result,
                           const LinearOperator *preconditioner = 0,
