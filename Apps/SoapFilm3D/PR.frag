...
void main() {
  ...
  // phong shading
  vec4 shading_diffuse =
      u_light_diffuse * clamp(dot(normal, -u_light_direction), 0.0, 1.0);
  ...
  vec4 shading_specular =
      u_light_specular *
      pow(clamp(dot(halfway, normal), 0.0, 1.0), u_light_specularity);
  ...
  //    gl_FragColor = (shading_diffuse * 0.6 + shading_specular * 0.5 +
  //    shading_ambient) * 0.3 + reflection * 0.7;;
