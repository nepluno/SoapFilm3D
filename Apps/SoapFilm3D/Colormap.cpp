...
int Colormap::getNumSamples() const
{
  return m_num_samples;
}

void Colormap::getColorByDensity( const double& rho, double& r, double& g, double& b ) const
{
  assert( rho >= 0.0 ); assert( rho <= 1.0 );
  int idx = floor(((double)(m_num_samples-1))*rho + 0.5);
  getColorByIndex( idx, r, g, b );
}

void Colormap::getColorByIndex( const int& i, double& r, double& g, double& b ) const
{
  assert( i >= 0 ); assert( i < m_num_samples );
...
void Colormap::generateMatlabJet()
{
	...
	int o = 0;
	int s = 0;
	int b_len = 0;	for( int i = 0; i < 3*n-1; ++i )
	{
		if( !( b[i] < 1 ) )
		{
			++b_len;
...
std::ostream& operator<<( std::ostream& os, Colormap& rhs )
{
	...
	//{
	//	os << rhs.m_r[i] << "\t\t" << rhs.m_g[i] << "\t\t" << rhs.m_b[i] << std::endl;
	//}
