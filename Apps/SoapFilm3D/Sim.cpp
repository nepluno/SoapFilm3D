...
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Sim::init(const std::string & option_file, bool save_outputs, bool headless)
{
    ...
    if (m_scene == "load")
    {
        ...
        VS3D * vs = new VS3D(std::vector<LosTopos::Vec3d>(), std::vector<LosTopos::Vec3st>(), std::vector<LosTopos::Vec2i>());
    ...
    } else
    {
        ...
        if (m_scene == "sphere")                    m_vs = Scenes::sceneSphere              (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "tet")                  m_vs = Scenes::sceneTet                 (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "cube")                 m_vs = Scenes::sceneCube                (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "sheet")                m_vs = Scenes::sceneSheet               (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "barrel")               m_vs = Scenes::sceneBarrel              (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "doublebubble")         m_vs = Scenes::sceneDoubleBubble        (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "twobubbles")           m_vs = Scenes::sceneTwoBubbles          (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "triplejunction")       m_vs = Scenes::sceneTripleJunction      (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "foaminit")             m_vs = Scenes::sceneFoamInit            (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "foam")                 m_vs = Scenes::sceneFoam                (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "quadjunction")         m_vs = Scenes::sceneQuadJunction        (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "constrainedsphere")    m_vs = Scenes::sceneConstrainedSphere   (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "bubblewand")           m_vs = Scenes::sceneBubbleWand          (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "tworingspinching")     m_vs = Scenes::sceneTwoRingsPinching    (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "pullingfoam")          m_vs = Scenes::scenePullingFoam         (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "peanutbubble")         m_vs = Scenes::scenePeanutBubble        (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "straw")                m_vs = Scenes::sceneStraw               (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "carousel")             m_vs = Scenes::sceneCarousel            (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "octahedron")           m_vs = Scenes::sceneOctahedron          (this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
...
void Sim::step()
{
    ...
    if (m_scene == "sphere")                    Scenes::stepSphere              (m_dt, this, m_vs);
    else if (m_scene == "tet")                  Scenes::stepTet                 (m_dt, this, m_vs);
    else if (m_scene == "cube")                 Scenes::stepCube                (m_dt, this, m_vs);
    else if (m_scene == "sheet")                Scenes::stepSheet               (m_dt, this, m_vs);
    else if (m_scene == "barrel")               Scenes::stepBarrel              (m_dt, this, m_vs);
    else if (m_scene == "doublebubble")         Scenes::stepDoubleBubble        (m_dt, this, m_vs);
    else if (m_scene == "twobubbles")           Scenes::stepTwoBubbles          (m_dt, this, m_vs);
    else if (m_scene == "triplejunction")       Scenes::stepTripleJunction      (m_dt, this, m_vs);
    else if (m_scene == "foaminit")             Scenes::stepFoamInit            (m_dt, this, m_vs);
    else if (m_scene == "foam")                 Scenes::stepFoam                (m_dt, this, m_vs);
    else if (m_scene == "quadjunction")         Scenes::stepQuadJunction        (m_dt, this, m_vs);
    else if (m_scene == "constrainedsphere")    Scenes::stepConstrainedSphere   (m_dt, this, m_vs);
    else if (m_scene == "bubblewand")           Scenes::stepBubbleWand          (m_dt, this, m_vs);
    else if (m_scene == "tworingspinching")     Scenes::stepTwoRingsPinching    (m_dt, this, m_vs);
    else if (m_scene == "pullingfoam")          Scenes::stepPullingFoam         (m_dt, this, m_vs);
    else if (m_scene == "peanutbubble")         Scenes::stepPeanutBubble        (m_dt, this, m_vs);
    else if (m_scene == "straw")                Scenes::stepStraw               (m_dt, this, m_vs);
    else if (m_scene == "carousel")             Scenes::stepCarousel            (m_dt, this, m_vs);
    else if (m_scene == "octahedron")           Scenes::stepOctahedron          (m_dt, this, m_vs);
...
void Sim::stepOutput(bool headless)
{
    if (m_output_directory != "")
    {
        ...
        if ((pngfd == 0 || frameid % pngfd == 0) && Options::boolValue("output-png") && !headless)
        {
            ...
            png_ss << m_output_directory << "/frame" << std::setfill('0') << std::setw(6) << (frameid / std::max(pngfd, 1)) << ".png";
...
//    gamma_min = -0.05;
    ...
    for (size_t i = 0; i < m_vs->mesh().nt(); i++)
    {
        ...
        if (m_vs->surfTrack()->m_mesh.get_triangle_label(i)[0] < m_vs->surfTrack()->m_mesh.get_triangle_label(i)[1])
            nt = -nt;
    ...
    for (size_t i = 0; i < m_vs->surfTrack()->m_mesh.nt(); i++)
    {
        ...
        if (m_vs->m_st->triangle_is_all_solid(i))
            continue;
...
//                if (!is_edge_next_to_nonmanifold_vertices(*m_vs->surfTrack(), i))
                if (!is_edge_nonmanifold(*m_vs->surfTrack(), i))
                    continue;
            }
        ...
        for (size_t i = 0; i < m_vs->mesh().ne(); i++)
        {
            if(m_vs->isVertexConstrained(m_vs->mesh().m_edges[i][0]) && m_vs->isVertexConstrained(m_vs->mesh().m_edges[i][1])) {
                if (rm == RM_NONMANIFOLD)
                {
                    //                if (!is_edge_next_to_nonmanifold_vertices(*m_vs->surfTrack(), i))
                    if (!is_edge_nonmanifold(*m_vs->surfTrack(), i))
...
//                if (!is_vertex_next_to_nonmanifold_vertices(*m_vs->surfTrack(), i))
                if (!is_vertex_nonmanifold(*m_vs->surfTrack(), i))
                    continue;
            }
    ...
    if (rm != RM_OPAQUE_SMOOTH_SHADED)
    {
        ...
        {
            Vec3d x = m_vs->pos(i);
            if (m_vs->m_st->vertex_is_any_solid(i))
                glVertex3d(x[0], x[1], x[2]);
        }
        glEnd();
        glPointSize(1);
    ...
    if (rm != RM_OPAQUE_SMOOTH_SHADED)
    {
        ...
        for (size_t i = 0; i < m_vs->mesh().nt(); i++)
        {
            ...
            LosTopos::Vec3st t = m_vs->mesh().get_triangle(i);
            if (m_vs->m_st->vertex_is_any_solid(t[0]) && m_vs->m_st->vertex_is_any_solid(t[1]) && m_vs->m_st->vertex_is_any_solid(t[2]))
...
void Sim::showPrimitiveInfo()
{
    if (m_nearest_vertex >= 0)
    {
        std::cout << "Vertex of Interest: " << m_nearest_vertex << " (" << m_vs->pos(m_nearest_vertex).transpose() << ")" << std::endl;
        std::cout << "  circulation = " << std::endl << m_vs->Gamma(m_nearest_vertex).values << std::endl;
        if (m_vs->m_dbg_v1.size() == m_vs->mesh().nv())
        {
            std::cout << "  dbg v1 = "; for (size_t i = 0; i < m_vs->m_dbg_v1[m_nearest_vertex].size(); i++) std::cout << m_vs->m_dbg_v1[m_nearest_vertex][i] << " "; std:: cout << std::endl;
        }
        std::cout << "  incident edges:"; for (size_t i = 0; i < m_vs->mesh().m_vertex_to_edge_map    [m_nearest_vertex].size(); i++) std::cout << " " << m_vs->mesh().m_vertex_to_edge_map    [m_nearest_vertex][i]; std::cout << std::endl;
        std::cout << "  incident faces:"; for (size_t i = 0; i < m_vs->mesh().m_vertex_to_triangle_map[m_nearest_vertex].size(); i++) std::cout << " " << m_vs->mesh().m_vertex_to_triangle_map[m_nearest_vertex][i]; std::cout << std::endl;
//        std::cout << "  ====== " << (*m_vs->m_Gamma)[m_nearest_vertex][0] - (*m_vs->m_Gamma)[m_nearest_vertex][1] << std::endl;
//        if (m_vs->m_dbg_v1.size() == m_vs->mesh().nv())
//            std::cout << "  ====== " << m_vs->m_dbg_v1[m_nearest_vertex][0] << std::endl;
    }
    
    if (m_nearest_edge >= 0)
    {
        std::cout << "Edge of Interest: " << m_nearest_edge << ": " << m_vs->mesh().m_edges[m_nearest_edge][0] << " (" << m_vs->pos(m_vs->mesh().m_edges[m_nearest_edge][0]).transpose() << ") - " << m_vs->mesh().m_edges[m_nearest_edge][1] << " (" << m_vs->pos(m_vs->mesh().m_edges[m_nearest_edge][1]).transpose() << ") length = " << (m_vs->pos(m_vs->mesh().m_edges[m_nearest_edge][1]) - m_vs->pos(m_vs->mesh().m_edges[m_nearest_edge][0])).norm() << std::endl;
        // std::cout << "  dbg e1 = " << m_vs->m_dbg_e1[m_nearest_edge][0] << " " << m_vs->m_dbg_e1[m_nearest_edge][1] << " " << m_vs->m_dbg_e1[m_nearest_edge][2] << std::endl;
        std::cout << "  incident faces:"; for (size_t i = 0; i < m_vs->mesh().m_edge_to_triangle_map[m_nearest_edge].size(); i++) std::cout << " " << m_vs->mesh().m_edge_to_triangle_map[m_nearest_edge][i]; std::cout << std::endl;
    }
    
    if (m_nearest_face >= 0)
    {
        std::cout << "Face of Interest: " << m_nearest_face << ": " << m_vs->mesh().m_tris[m_nearest_face][0] << " (" << m_vs->pos(m_vs->mesh().m_tris[m_nearest_face][0]).transpose() << "), " << m_vs->mesh().m_tris[m_nearest_face][1] << " (" << m_vs->pos(m_vs->mesh().m_tris[m_nearest_face][1]).transpose() << "), " << m_vs->mesh().m_tris[m_nearest_face][2] << " (" << m_vs->pos(m_vs->mesh().m_tris[m_nearest_face][2]).transpose() << ")" << std::endl;
        std::cout << "  labels: " << m_vs->mesh().get_triangle_label(m_nearest_face) << std::endl;
        std::cout << "  incident edges:"; for (size_t i = 0; i < 3; i++) std::cout << " " << m_vs->mesh().m_triangle_to_edge_map[m_nearest_face][i]; std::cout << std::endl;
    }
}
