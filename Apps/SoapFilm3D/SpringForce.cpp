...
SpringForce::SpringForce(const std::pair<int, int>& endpoints, const scalar& k,
                         const scalar& l0, const scalar& b)
    : Force(), m_endpoints(endpoints), m_k(k), m_l0(l0), m_b(b) {
...
void SpringForce::addEnergyToTotal(const VectorXs& x, const VectorXs& v,
  ...
  assert(x.size() % 3 == 0);
  assert(m_endpoints.first >= 0);
  assert(m_endpoints.first < x.size() / 3);
  assert(m_endpoints.second >= 0);
  assert(m_endpoints.second < x.size() / 3);

  scalar l = (x.segment<3>(3 * m_endpoints.second) -
              x.segment<3>(3 * m_endpoints.first))
                 .norm();
...
void SpringForce::addGradEToTotal(const VectorXs& x, const VectorXs& v,
  ...
  Vector3s nhat = x.segment<3>(3 * m_endpoints.second) -
                  x.segment<3>(3 * m_endpoints.first);
  scalar l = nhat.norm();
  assert(l != 0.0);
  nhat /= l;
  ...
  fdamp *= m_b * fdamp.dot(v.segment<3>(3 * m_endpoints.second) -
                           v.segment<3>(3 * m_endpoints.first));
...
void SpringForce::addHessXToTotal(const VectorXs& x, const VectorXs& v,
  ...
  Vector3s dv = v.segment<3>(3 * m_endpoints.second) -
                v.segment<3>(3 * m_endpoints.first);
  ...
  for (int r = 0; r < 3; ++r)
    for (int s = 0; s < 3; ++s)
      hessE.push_back(Triplets(3 * m_endpoints.first + r,
                               3 * m_endpoints.first + s, hess(r, s)));
  for (int r = 0; r < 3; ++r)
    for (int s = 0; s < 3; ++s)
      hessE.push_back(Triplets(3 * m_endpoints.second + r,
                               3 * m_endpoints.second + s, hess(r, s)));
  for (int r = 0; r < 3; ++r)
    for (int s = 0; s < 3; ++s)
      hessE.push_back(Triplets(3 * m_endpoints.first + r,
                               3 * m_endpoints.second + s, -hess(r, s)));
...
void SpringForce::addHessVToTotal(const VectorXs& x, const VectorXs& v,
                                  const VectorXs& m, TripletXs& hessE) {
  assert(x.size() == v.size());
  assert(x.size() == m.size());
  assert(x.size() % 3 == 0);
