...
bool MeshIO::save(VS3D &vs, const std::string &filename, bool binary) {
  ...
  if (binary) {
    ...
    for (size_t i = 0; i < vs.constrainedVertices().size(); i++)
      os << vs.constrainedVertices()[i] << " "
         << vs.constrainedPositions()[i][0] << " "
         << vs.constrainedPositions()[i][1] << " "
         << vs.constrainedPositions()[i][2] << std::endl;
...
bool MeshIO::load(VS3D &vs, const std::string &filename, bool binary) {
  ...
  if (!test.is_open()) {
    std::cout << "[MeshIO::load] Error: file " << filename << " not found."
              << std::endl;
    return false;
  }

  for (size_t i = 0; i < st.m_mesh.nt(); i++) {
    if (st.m_mesh.get_triangle(i)[0] == st.m_mesh.get_triangle(i)[1]) continue;
    st.remove_triangle(i);
  }

  for (size_t i = 0; i < st.m_mesh.nv(); i++) st.remove_vertex(i);
  ...
  if (binary) {
    ...
    for (size_t i = 0; i < n; i++) st.m_masses[i] = LosTopos::Vec3d(1, 1, 1);

    st.pm_positions = pos;
    st.pm_newpositions = pos;
    st.set_all_remesh_velocities(
        std::vector<LosTopos::Vec3d>(n, LosTopos::Vec3d(0)));
    ...
    for (size_t i = 0; i < vs.mesh().nv(); i++) vs.Gamma(i) = Gammas[i];

    bool good = is.good();

    n = 0;
    is.read((char *)&n, sizeof(size_t));
    if (!is.eof())  // if eof, this means the rec file is older version (not
                    // containing constraints info)
    {
      ...
      for (size_t i = 0; i < vs.constrainedVertices().size(); i++)
        st.m_masses[vs.constrainedVertices()[i]] =
...
bool MeshIO::loadIntoRaw(std::vector<LosTopos::Vec3d> &vs,
...
struct VertexPatchPair {
  ...
  bool operator()(const VertexPatchPair &vpp1,
                  const VertexPatchPair &vpp2) const {
    return (vpp1.v < vpp2.v ||
            (vpp1.v == vpp2.v &&
             (vpp1.rp[0] < vpp2.rp[0] ||
              (vpp1.rp[0] == vpp2.rp[0] && vpp1.rp[1] < vpp2.rp[1]))));
  }
};
...
bool MeshIO::saveOBJ(VS3D &vs, const std::string &filename) {
  ...
  // define all the normals (for manifold vertices, average the incident face
  // normals weighted by area; for nonmanifold vertex, one normal instance is
  // created for each manifold patch.)
  ...
  for (size_t i = 0; i < vs.mesh().nv(); i++) {
    ...
    for (std::set<Vec2i, Vec2iComp>::iterator j = incident_regions.begin();
         j != incident_regions.end(); j++) {
      VertexPatchPair vpp;
      vpp.v = i;
      vpp.rp = *j;
      vertex_normal_index[vpp] = (int)vertex_normals.size();
      Vec3d n(0, 0, 0);
      for (size_t k = 0; k < vs.mesh().m_vertex_to_triangle_map[i].size();
           k++) {
        LosTopos::Vec2i l = vs.mesh().get_triangle_label(
            vs.mesh().m_vertex_to_triangle_map[i][k]);
        if ((l[0] == (*j)[0] && l[1] == (*j)[1]) ||
            (l[1] == (*j)[0] && l[0] == (*j)[1])) {
          LosTopos::Vec3st t =
              vs.mesh().get_triangle(vs.mesh().m_vertex_to_triangle_map[i][k]);
          Vec3d fn =
              (vs.pos(t[1]) - vs.pos(t[0])).cross(vs.pos(t[2]) - vs.pos(t[0])) *
              (l[0] == (*j)[0] ? 1 : -1);
          n += fn;
        }
      }
  ...
  std::ofstream os(filename.c_str());
  for (size_t i = 0; i < vs.mesh().nv(); i++)
    os << "v " << vs.pos(i).x() << " " << vs.pos(i).y() << " " << vs.pos(i).z()
       << std::endl;

  for (size_t i = 0; i < vertex_normals.size(); i++)
    os << "vn " << vertex_normals[i].x() << " " << vertex_normals[i].y() << " "
       << vertex_normals[i].z() << std::endl;
  ...
  for (size_t i = 0; i < vs.mesh().nt(); i++) {
    ...
    vpp.v = t[0];
    int vn0 = vertex_normal_index[vpp];
    vpp.v = t[1];
    int vn1 = vertex_normal_index[vpp];
    vpp.v = t[2];
    int vn2 = vertex_normal_index[vpp];
    os << "f " << t[0] + 1 << "//" << vn0 + 1 << " " << t[1] + 1 << "//"
       << vn1 + 1 << " " << t[2] + 1 << "//" << vn2 + 1 << std::endl;
  }
