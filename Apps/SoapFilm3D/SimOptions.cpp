...
void Options::addStringOption(const std::string& key,
                              const std::string& default_value) {
  assert(s_options.find(key) ==
         s_options.end());  // verify this option doesn't already exit
...
bool Options::parseOptionFile(const std::string& file, bool verbose) {
  ...
  while (!fin.eof()) {
    ...
    ss >> key;
    if (key == "#" || key == "" ||
        ss.eof())  // skip comment lines and empty lines
      continue;
  ...
  if (verbose) {
    for (std::map<std::string, Option>::iterator i = s_options.begin();
         i != s_options.end(); i++) {
...
const std::string& Options::strValue(const std::string& key) {
  ...
  assert(s_options[key].type ==
         STRING);  // verify this option has the correct type
...
int Options::intValue(const std::string& key) {
  assert(s_options.find(key) != s_options.end());  // verify this option exists
  assert(s_options[key].type ==
         INTEGER);  // verify this option has the correct type
...
double Options::doubleValue(const std::string& key) {
  assert(s_options.find(key) != s_options.end());  // verify this option exists
  assert(s_options[key].type ==
         DOUBLE);  // verify this option has the correct type
...
bool Options::boolValue(const std::string& key) {
  assert(s_options.find(key) != s_options.end());  // verify this option exists
  assert(s_options[key].type ==
         BOOLEAN);  // verify this option has the correct type
