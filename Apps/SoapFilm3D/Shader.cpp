...
void Shader::setUniform(const std::string & name, const Mat3 & m, bool transpose)
{
    Shader * current = currentlyActiveShader();
    assert(current);
...
void Shader::setUniform(const std::string & name, const Mat4 & m, bool transpose)
{
    Shader * current = currentlyActiveShader();
    assert(current);
...
bool Shader::loadFromFile(const std::string & name)
{
    ...
    {
        std::cout << "Error when loading shader " << name << ": files not found." << std::endl;
        return false;
    }
    ...
    for (std::map<std::string, GLuint>::iterator i = m_vertex_attributes.begin(); i != m_vertex_attributes.end(); i++)
...
{
    std::map<std::string, GLuint>::iterator entry = m_vertex_attributes.find(name);
...
void Shader::printProgramInfoLog(GLuint prog, const std::string & name, const std::string & stage)
{
...
void Shader::printShaderInfoLog(GLuint shader, const std::string & name, const std::string & stage)
{
    GLint infologLength = 0, charsWritten = 0;
